...
(function (global) {
  ...
  async function share(blob, filename, title) {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] }) && navigator.share) {
      try {
        ...
        // User cancelled or share unsupported under the hood — fall through to download.
        if (e && e.name === "AbortError") return;
      }
    }
    // Fallback: download.
    if (window.UI && UI.downloadFile) {
      // UI.downloadFile takes (filename, content, mime) and wraps in a Blob;
      // pass the blob directly so we don't double-wrap.
  ...
  global.ImageExport = { exportTreePng, exportProfileCard, share };
})(window);
