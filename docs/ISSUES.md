...
- ✅ **A child's two parents rendered as disconnected — the trunk dropped out of an empty gap unless the parents were also married.** The tree only drew a *couple* (adjacent placement + gold wedding-knot + a clean shared anchor for the child's trunk) when two people were mutual `spouses`. Two people who merely **share a child** (both in `child.parents`) but aren't each other's spouse got no adjacency and no joining mark, yet the child's trunk still anchored at their midpoint (`drawEdges`, the same-row 2-parent branch) — which landed in blank space between two visually-unrelated nodes, reading as a broken link. (User: *"why the links are incorrect"* / *"after adding as spouse it shows up"*.) Fixed **without touching the couple/knot path**: `computeLayout` now derives a co-parent map and `placeRow` pulls an unmarried co-parent adjacent into a separate `coParentAdjacency` set with its own `X_GAP_COPARENT` (−20 — closer than siblings so they read as a unit, wider than a married couple so the photos don't overlap). `drawEdges` then spans a **neutral, knot-less `.t-edge--coparent` bar** (structural ink, never gold — gold + knot stay reserved for marriage) between the two photo rings at portrait height; its midpoint equals the child's `anchorX`, so the trunk now descends from the centre of the join. Fades/brightens under lineage + compare focus like any other `.t-edge` (two-class `.is-faded`/`.is-lineage` win over the single-class variant). Sample tree unaffected — all 8 of its co-parent pairs are also spouses, so 0 new bars there. `CACHE_VERSION` → v110. (this commit)
