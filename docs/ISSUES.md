...
- 🟡 **Hardcoded UI strings that bypass i18n** (found in the 2026-08 subagent audit; all should route through `lib/core/i18n.js` so both EN + HI resolve — the new `tests/i18n-parity.mjs` guards the label file, but not string *usage*). Ranked by user impact:
  ...
  - **`inspector.js:472` hardcoded English month array.** `formatDateLong` builds `["Jan"…"Dec"][mo-1]`, so every date the inspector renders ("14 Aug 2026", the most-seen date surface in the app) shows English month abbreviations even when the UI is Hindi. Needs a 12-key i18n month table (or `Intl.DateTimeFormat` with the active locale). Higher user-visibility than most of this group — it's on every open profile — but grouped here because it's the same "route through i18n" fix.

- 🟡 **Archive-completion stats stranded on mobile.** `inspector.js:160-196` renders the family-archive completion bar (% of births/photos/descriptions filled across the tree) only in the inspector's empty-state. Like the family calendar was, that empty-state is desktop-only: the docked inspector becomes an off-screen drawer ≤1100px that opens *only* on person-select, so a phone user never sees the tree's completion progress. Same fix shape as the calendar (this commit surfaced the calendar as a folded Insights panel) — promote the completion bar into the mobile-reachable Insights view too.
