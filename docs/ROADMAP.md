...
>
> **Still NOT built (deferred, see [CLOUD-SYNC-PLAN.md](CLOUD-SYNC-PLAN.md) §"Deferred"):** the **public/unlisted token-based read path** (`trees.visibility` + `share_token` + the anyone-with-the-link RLS rule) and server-side viewer redaction. Sharing today is **invite-by-email only**; the "app link" in the share dialog is just the app URL, not an access-granting token. The effort-table row below stays un-struck for that reason.
>
