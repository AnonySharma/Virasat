...
| **8. Edges + polish** ✅ | PKCE `?code=` cleanup handled by the SDK's `detectSessionInUrl` (no app code — confirmed nothing else reads `location.search`); conflict-backup UX (the losing local snapshot rides the `virasat:cross-tab-conflict` event `detail`; banner's "Save my version" → `ExportImport.downloadBackupState`); `sync.*` i18n (EN+HI); keep-alive GitHub Actions cron (`.github/workflows/keep-alive.yml`, anon read every 3 days); `<meta name="robots" content="noindex, nofollow">` | — | 1.5 |
