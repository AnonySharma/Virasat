 ...
 *
 * The lexicon covers close-kin signatures that have ONE unambiguous word,
 * including the four cousin kinds Hindi names by the linking aunt/uncle
 * (chachera / fufera / mamera / mausera × bhai/bahen). Spouse's-siblings
 * (devar/jeth/saala…), second cousins and anything longer resolve to null —
 * the caller keeps its per-hop arrows / step count for those.
 */
(function (global) {
  "use strict";
  ...
  const T = {
    ...
    cousin:        { en: "cousin", hi: "" },
    // in-laws (the two unambiguous single-word ones)
  ...
  function forPath(path) {
    ...
    switch (sig) {
      ...
      default: return null; // cousins, spouse's-siblings, longer chains → no chip
    }

    if (!term) return null;
    // Recover the lexicon key for callers/tests that want a stable id.
    for (const k in T) { if (T[k] === term) { key = k; break; } }
    return { key: key, en: term.en, hi: term.hi };
  }
