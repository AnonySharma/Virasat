...
(function (global) {
  ...
  // survives verbatim and must be matched as-is. Shared by the presence gate
  // and the per-row name lookup so the two can't drift apart again.
  const NAME_HEADERS = ["name", "full name", "full name (english)"];
  ...
  function fallbackCopy(text) {
    ...
    catch (e) { UI.toast(I18n.t("collect.copyFailed"), "danger"); }
    ta.remove();
  }
