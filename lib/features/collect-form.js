...
(function (global) {
  ...
  function importCsvText(text) {
    // Read-only guard: this loops FamilyStore.addPerson, which returns null under
    // the store's read-only guard — the caller would then throw on newPerson.name.
    // Reachable only via edit-only menu entries, but guard here too so a stray
    // call (e.g. from the import delegator) warns cleanly instead of crashing.
