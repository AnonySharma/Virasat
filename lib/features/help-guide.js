 ...
 *
 * Public:
 *   HelpGuide.open()  — opens the guide modal; returns the UI.openModal handle
 *                       ({ close, modal }) so the hash router in app.js can tie
 *                       the #help route to it, or null if it couldn't open.
 */
(function (global) {
  "use strict";
  ...
  const SECTIONS = [
    {
      ...
      items: [
        ...
        ["fa-solid fa-id-card", "inspector", "inspectorHow"]
      ]
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        ["fa-solid fa-link", "navigate", "navigateHow"]
      ]
    }
  ];
  ...
  function open(opts) {
    if (!global.UI || !UI.openModal) return null;
    const o = opts || {};
    ...
    const handle = UI.openModal({
      ...
      // Fires on every close path (button, ×, Esc, backdrop) so a route owner
      // can sync the URL back off #help when the user dismisses the guide.
