...
(function (global) {
  ...
  function openExport() {
    ...
    const state = {
      ...
      // PNG-only view scope, meaningful only when the live tree is in Focus mode
      // (an ego-centric prune). The PNG clones the live tree DOM, so "current"
      // captures exactly the focused spine on screen; "whole" transiently flips
      // the live tree to full mode so the clone sees every node. "current" is a
      // faithful default — export what you're looking at. Ignored in full mode
      // (the two are then identical) and for JSON/backup (which read the store,
      // never the DOM).
      pngViewScope: "current"
    };
    // The live tree's display mode as the dialog opens. If it's focused, the PNG
    // export would otherwise silently capture only the on-screen spine, so we
    // surface an explicit "this view / whole tree" choice. Captured once here so
    // a transient flip we make for a "whole tree" preview doesn't change it.
    const liveTreeMode = (window.TreeView && TreeView.getTreeMode) ? TreeView.getTreeMode() : "full";
    const treeIsFocused = liveTreeMode === "focus";
    ...
    // ── Left rail: the format choice — an image to print/share, a data file to
    // re-import, or a full backup. Vertical radio-cards; picking one reshapes
    // the right pane (preview + options) instead of stacking three option sets.
    const FORMATS = [
    ...
    const formatCards = [];
    const rail = UI.el("div", { class: "exp2__rail", role: "radiogroup", "aria-label": I18n.t("exp.fmtLabel") });
    FORMATS.forEach((f) => {
      const card = UI.el("button", {
        type: "button", class: "exp2-fmt", role: "radio", "aria-checked": "false",
        onclick: () => { state.exportFormat = f.id; applyFormat(); }
      }, [
        UI.el("i", { class: f.icon + " exp2-fmt__icon", "aria-hidden": "true" }),
        UI.el("span", { class: "exp2-fmt__text" }, [
          UI.el("span", { class: "exp2-fmt__title" }, I18n.t(f.titleKey)),
          UI.el("span", { class: "exp2-fmt__desc" }, I18n.t(f.descKey))
        ])
      ]);
    ...
    // Right pane: a live preview on top, then the chosen format's options.
    const previewHost = UI.el("div", { class: "exp2__preview" });
    const optionsHost = UI.el("div", { class: "exp2__options" });

    // Compact switch row (icon + label + optional description + pill toggle),
    // mirroring the tree's View Options switches — the same features live there.
    function makeSwitch({ label, desc, icon, active, onChange }) {
      const cb = UI.el("input", { type: "checkbox" });
      /** @type {HTMLInputElement} */ (cb).checked = active !== false;
      cb.addEventListener("change", () => onChange(/** @type {HTMLInputElement} */ (cb).checked));
      return UI.el("label", { class: "exp-switch" }, [
        icon ? UI.el("i", { class: icon + " exp-switch__icon", "aria-hidden": "true" }) : null,
        UI.el("span", { class: "exp-switch__text" }, [
          UI.el("span", { class: "exp-switch__title" }, label),
          desc ? UI.el("span", { class: "exp-switch__desc" }, desc) : null
        ]),
        UI.el("span", { class: "exp-switch__pill" }, [cb, UI.el("span", { class: "exp-switch__track" })])
      ]);
    }

    // Small labeled group: a caption above its control(s).
    const group = (labelText, control) => UI.el("div", { class: "exp-group" }, [
      UI.el("div", { class: "field__label" }, labelText),
      control
    ]);

    // PNG quality picker — radio cards; each shows an estimated file size.
    ...
    // — Image options — "Shown in the image" switches (seeded from the live
    // View Options) + PNG quality. Relations only appears when a self is pinned.
    const imgSwitchDefs = [
    ...
    const imgSwitches = imgSwitchDefs.map((d) => makeSwitch({
      label: d.label, icon: d.icon, active: state.image[d.key],
      onChange: (on) => { state.image[d.key] = on; renderPreview(); }
    }));
    if (liveShow.relations) {
      imgSwitches.push(makeSwitch({
        label: I18n.t("exp.imgRelations"), icon: "fa-solid fa-people-arrows",
        active: state.image.relations,
        onChange: (on) => { state.image.relations = on; renderPreview(); }
      }));
    }
    const imageOptions = UI.el("div", { class: "exp2-panel" }, [
      group(I18n.t("exp.shownInImage"), UI.el("div", { class: "exp-switches" }, imgSwitches)),
      group(I18n.t("exp.pngQuality"), pngQualityHost)
    ]);

    // — Data options — "Include in export" switches (with descriptions) plus
    // Minimal mode, which greys the switches above (its own container only).
    const dataSwitches = UI.el("div", { class: "exp-switches" }, [
      makeSwitch({ label: I18n.t("exp.photos"), desc: I18n.t("exp.photosDesc"), icon: "fa-regular fa-image",
        active: state.data.photos, onChange: (on) => { state.data.photos = on; renderPreview(); } }),
      makeSwitch({ label: I18n.t("exp.dates"), desc: I18n.t("exp.datesDesc"), icon: "fa-regular fa-calendar",
        active: state.data.dates, onChange: (on) => { state.data.dates = on; renderPreview(); } }),
      makeSwitch({ label: I18n.t("exp.places"), desc: I18n.t("exp.placesDesc"), icon: "fa-solid fa-location-dot",
        active: state.data.places, onChange: (on) => { state.data.places = on; renderPreview(); } })
    ]);
    const minimalSwitch = makeSwitch({
      label: I18n.t("exp.minimalMode"), desc: I18n.t("exp.minimalModeDesc"), icon: "fa-solid fa-compress",
      active: false,
      onChange: (on) => { state.data.minimal = on; dataSwitches.classList.toggle("is-locked", on); renderPreview(); }
    });
    const dataOptions = UI.el("div", { class: "exp2-panel" }, [
      group(I18n.t("exp.includeInExport"), dataSwitches),
      minimalSwitch
    ]);

    // — Backup takes no options; its preview card carries the reassurance note.
    const backupOptions = UI.el("div", { class: "exp2-panel" }, []);

    // Lineage scope — applies to image + data (a backup is always whole-tree,
    // so applyFormat hides this row for a backup). Shown only when a person is
    // in focus as the dialog opens.
    let lineageRow = null;
    if (focusId) {
      lineageRow = makeSwitch({
        label: I18n.t("exp.onlyLineage", { name: focusName }), icon: "fa-solid fa-code-branch",
        active: true, onChange: (on) => { state.limitToLineage = on; renderPreview(); }
      });
      lineageRow.classList.add("exp-switch--scope");
    }

    // PNG view-scope row — only when the live tree is FOCUSED as the dialog
    // opens. Off = "this focused view" (default); on = "whole tree". Because the
    // PNG clones the live DOM, the only way to capture the whole tree from a
    // focused view is to transiently render it in full — syncLiveTreeScope does
    // that, and restoreLiveTreeMode puts the user's mode back on close.
    let pngScopeRow = null;
    if (treeIsFocused) {
      pngScopeRow = makeSwitch({
        label: I18n.t("exp.wholeTreeScope"), desc: I18n.t("exp.wholeTreeScopeDesc"),
        icon: "fa-solid fa-diagram-project", active: false,
        onChange: (on) => { state.pngViewScope = on ? "whole" : "current"; syncLiveTreeScope(); renderPreview(); }
      });
      pngScopeRow.classList.add("exp-switch--scope");
    }

    // Keep the LIVE tree's display mode in step with the chosen PNG scope so the
    // preview clone and the exported clone both see the right node set. Only ever
    // touches the tree when it was focused to begin with (treeIsFocused): "whole"
    // flips it to full, "current" flips it back to focus. A no-op otherwise. The
    // flip is transient (setTreeModeTransient never persists), so the user's real
    // preference is restored intact by restoreLiveTreeMode on close.
    function syncLiveTreeScope() {
      if (!treeIsFocused || !(window.TreeView && TreeView.setTreeModeTransient)) return;
      const want = state.exportFormat === "png" && state.pngViewScope === "whole" ? "full" : "focus";
      TreeView.setTreeModeTransient(want);
    }
    function restoreLiveTreeMode() {
      if (!treeIsFocused || !(window.TreeView && TreeView.setTreeModeTransient)) return;
      TreeView.setTreeModeTransient(liveTreeMode);
    }

    // People (and photo count) in the current export scope — honours the
    // lineage limit for image/data; a backup is always the whole tree.
    function scopedPeople() {
      const all = (window.FamilyStore ? FamilyStore.getPeople() : []) || [];
      if (state.exportFormat !== "backup" && state.limitToLineage && state.lineageFocusId
          && window.ImageExport && ImageExport.buildFocusSet) {
        const set = ImageExport.buildFocusSet(state.lineageFocusId);
        if (set) return all.filter((p) => set.has(p.id));
      }
      return all;
    }
    function estJsonSize() {
      try { return formatSize(new Blob([JSON.stringify(buildRedactedStateSync(dataOpts()))]).size); }
      catch (e) { return "—"; }
    }
    function contentsCard(icon, titleKey, stats, note) {
      return UI.el("div", { class: "exp-card" }, [
        UI.el("div", { class: "exp-card__head" }, [
          UI.el("i", { class: icon, "aria-hidden": "true" }),
          UI.el("span", null, I18n.t(titleKey))
        ]),
        UI.el("div", { class: "exp-card__stats" }, stats.map((t) => UI.el("span", { class: "exp-card__stat" }, t))),
        note ? UI.el("div", { class: "exp-card__note" }, note) : null
      ]);
    }

    // Paint the preview pane for the current format. Image → a live, scaled
    // clone of the tree (same trims as the PNG export, no rasterisation). Data /
    // Backup → a contents-summary card (counts + approximate size). Cheap enough
    // to re-run on every toggle.
    function renderPreview() {
      UI.clear(previewHost);
      const fmt = state.exportFormat;
      if (fmt === "png") {
        const svg = (window.ImageExport && ImageExport.buildPreviewSvg)
          ? ImageExport.buildPreviewSvg({
              includePhotos: state.image.photos, includeDates: state.image.dates,
              includePets: state.image.pets, includeAge: state.image.age,
              includeEras: state.image.eras, includeRelations: state.image.relations,
              focusId: state.limitToLineage ? state.lineageFocusId : null
            })
          : null;
        if (svg) {
          previewHost.appendChild(UI.el("div", { class: "exp2__canvas" }, [svg]));
          previewHost.appendChild(UI.el("div", { class: "exp2__note" }, I18n.t("exp.previewNote")));
        } else {
          previewHost.appendChild(UI.el("div", { class: "exp2__empty" }, I18n.t("exp.previewEmpty")));
        }
        return;
      }
      const ppl = scopedPeople();
      const photoN = ppl.filter((p) => p && (p.photo || p.photoId)).length;
      if (fmt === "json") {
        previewHost.appendChild(contentsCard("fa-solid fa-file-code", "exp.fmtDataTitle", [
          I18n.t("exp.cardPeople", { n: ppl.length }),
          state.data.photos ? I18n.t("exp.cardPhotos", { n: photoN }) : I18n.t("exp.cardNoPhotos"),
          "JSON ≈ " + estJsonSize()
        ]));
      } else {
        previewHost.appendChild(contentsCard("fa-solid fa-shield-halved", "exp.fmtBackupTitle", [
          I18n.t("exp.cardPeople", { n: ppl.length }),
          I18n.t("exp.cardPhotos", { n: photoN })
        ], I18n.t("exp.backupNote")));
      }
    }

    const body = UI.el("div", { class: "form-stack" }, [
      UI.el("p", { class: "exp2__intro" }, I18n.t("exp.body")),
      UI.el("div", { class: "exp2" }, [
        UI.el("div", { class: "exp2__railwrap" }, [
          UI.el("div", { class: "field__label" }, I18n.t("exp.fmtLabel")),
          rail
        ]),
        UI.el("div", { class: "exp2__main" }, [previewHost, optionsHost, lineageRow, pngScopeRow])
      ])
    ]);

    // Reflect the chosen format: light its rail card, swap in that format's
    // options, hide lineage scope for a backup, repoint Save, repaint preview.
    function applyFormat() {
      ...
      UI.clear(optionsHost);
      optionsHost.appendChild(fmt === "png" ? imageOptions : fmt === "json" ? dataOptions : backupOptions);
      if (lineageRow) lineageRow.style.display = fmt === "backup" ? "none" : "";
      // The whole-tree scope only makes sense for the PNG (the live-DOM clone);
      // JSON/backup read the store regardless of tree mode. Hide it elsewhere,
      // and re-sync the live tree so leaving PNG restores the focused view.
      if (pngScopeRow) pngScopeRow.style.display = fmt === "png" ? "" : "none";
      syncLiveTreeScope();
      saveIcon.className = fmt === "png" ? "fa-regular fa-image"
        : fmt === "backup" ? "fa-solid fa-shield-halved" : "fa-solid fa-file-code";
      saveLabel.textContent = fmt === "png" ? I18n.t("exp.savePng")
        : fmt === "backup" ? I18n.t("exp.saveBackup") : I18n.t("exp.saveJson");
      renderPreview();
    }
    ...
    const dlg = UI.openModal({
      ...
      // If we transiently flipped the live tree to full for a "whole tree" PNG,
      // put the user's real display mode back on every close path (Save, Cancel,
      // Escape, backdrop, X). Harmless no-op when we never flipped.
      onClose: () => restoreLiveTreeMode()
    });
    // The master-detail layout needs more room than the default 600px modal;
    // .modal--wide widens it on desktop and is a no-op on the mobile sheet.
    if (dlg && dlg.modal) dlg.modal.classList.add("modal--wide");
    ...
    // applyFormat() also paints the initial preview via renderPreview().
    applyFormat();
  }
