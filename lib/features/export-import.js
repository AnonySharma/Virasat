...
(function (global) {
  ...
  function openExport() {
    ...
    // image; they don't affect JSON (which carries data, not on-canvas chrome).
    // Relations is only offered when it could actually draw (self pinned +
    // "relation to me" on live), matching the live popover's own gating.
    const imgChips = [
      makeFieldChip({
    ...
    const imageChips = UI.el("div", { class: "field-chips" }, imgChips);
    ...
    const pngSection = UI.el("div", {
      ...
      // Image-only display features (mirror the live tree's View Options). These
      // trim on-canvas chrome from the PNG; they don't touch JSON, so they live
      // inside the PNG card rather than beside the data-field chips above.
      UI.el("div", { class: "field__label", style: { marginBottom: "4px" } }, I18n.t("exp.shownInImage")),
      imageChips,
      UI.el("div", { class: "field__label", style: { marginBottom: "4px", marginTop: "4px" } }, I18n.t("exp.pngQuality")),
      pngQualityHost
    ]);
  ...
  // shared vs solo trees); signed out there's only the single replace path.
  // Resolves to the chosen mode ("new" | "replace"); Cancel / Esc / backdrop /
  ...
  function confirmImportWithPreview(parsed, opts) {
    ...
    // Resolves to the chosen mode ("new" | "replace"); null on cancel/dismiss.
    return new Promise((resolve) => {
      ...
      cancelBtn.addEventListener("click", () => dlg.close());
      importBtn.addEventListener("click", () => { done(mode); dlg.close(); });
    });
  }
  ...
  function openImport() {
    ...
    const onChange = async () => {
      ...
      const mode = await confirmImportWithPreview(parsed, { currentCount, newCount, isShared, sharedWith });

      if (mode) {
        try {
          if (mode === "new") {
            // Add as a NEW tree (cloud only): provision an empty tree named from
            // the file's meta and switch to it. createTree arms the push pipeline
            // on the new row, so the replaceAll below persists the imported data
            // up as the new tree's first version — the current tree is untouched.
            const meta = (parsed && parsed.meta) || {};
            const title = String(meta.familyTitle || meta.familyName || "").trim();
