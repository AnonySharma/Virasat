...
(function (global) {
  ...
  //  - Photos: try to inline as a base64 data URL (portable across devices).
  //    If inlining fails (file:// fetch restrictions, IDB read errors,
  //    private browsing), KEEP the original photoUrl / photoId references
  //    so the backup is at least restorable on the same device. Returns a
  //    `{ state, photoFailures }` tuple so the caller can warn the user
  //    when photos couldn't be inlined.
  async function buildBackupState() {
    const src = FamilyStore.getState();
    let photoFailures = 0;
    let photoTotal = 0;
    ...
    async function inlinePhoto(rec) {
      ...
      if (rec.photoUrl) {
        try {
          const resp = await fetch(rec.photoUrl);
          if (resp.ok) {
            const blob = await resp.blob();
            return { ok: true, dataUrl: await PhotoStore.blobToDataUrl(blob) };
          }
          console.warn("Backup: photoUrl fetch", resp.status, rec.photoUrl);
        } catch (e) {
          // file:// fetch restrictions land here — not necessarily a bug,
          // just a constraint of the environment.
          console.warn("Backup: photoUrl fetch failed for", rec.photoUrl, e && e.message);
        }
      }
      return { ok: false };
    }
    ...
    const people = await Promise.all((src.people || []).map(async (p) => {
      ...
      const hasAnyPhoto = !!(p.photo || p.photoId || p.photoUrl);
      if (!hasAnyPhoto) return out;
      photoTotal++;
      const r = await inlinePhoto(p);
      if (r.ok) {
        out.photo = r.dataUrl;
        // Inlined photo is the source of truth — drop the stale references
        // so getUrlSync (photoUrl-first) doesn't shadow the inlined data.
        delete out.photoId;
        delete out.photoUrl;
      } else {
        // Keep original references so a same-device restore still works.
        photoFailures++;
      }
      return out;
    }));
  ...
  function openExport() {
    ...
    backupBtn.addEventListener("click", async () => {
      ...
      const origLabel = labelSpan.textContent;
      labelSpan.textContent = "Backing up…";
      try {
        const { state: backup, photoTotal, photoFailures } = await buildBackupState();
        const familyName = (FamilyStore.getFamilyTitle ? FamilyStore.getFamilyTitle() : null)
        ...
        if (photoFailures === 0) {
          UI.toast("Backup saved · " + filename, "success");
        } else if (photoFailures < photoTotal) {
          UI.toast("Backup saved, but " + photoFailures + " of " + photoTotal
            + " photos couldn't be inlined — file references kept as fallback.", "warning");
        } else {
          UI.toast("Backup saved without photos — none could be inlined. "
            + "Check that you're not running from file:// (fetch is restricted).", "warning");
        }
        dlg.close();
      } catch (e) {
        UI.toast("Backup failed: " + (e && e.message || "unknown"), "danger");
      } finally {
        pngBtn.disabled = false;
        jsonBtn.disabled = false;
        backupBtn.disabled = false;
