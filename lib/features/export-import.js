...
(function (global) {
  ...
  //  - photoId is replaced by an inlined base64 photo (data URL). photoIds
  //    are device-local IDB keys; on re-import migrateLegacy() generates
  //    fresh ones, so dropping the stale id is correct.
  //  - photoUrl (sample-data path) is also inlined so the file is
  //    self-contained on any device.
  async function buildBackupState() {
    ...
    const people = await Promise.all((src.people || []).map(async (p) => {
      ...
      if (window.PhotoStore && p.photoId) {
        try {
          ...
          if (blob) out.photo = await PhotoStore.blobToDataUrl(blob);
        } catch (_) {}
      } else if (p.photoUrl && !out.photo) {
        try {
          const resp = await fetch(p.photoUrl);
          if (resp.ok) out.photo = await PhotoStore.blobToDataUrl(await resp.blob());
        } catch (_) {}
      }
      return out;
    }));
    ...
    await Promise.all(Object.keys(rawM).map(async (k) => {
      ...
      delete out.photoId;
      if (window.PhotoStore && m.photoId) {
        try {
          const blob = await PhotoStore.get(m.photoId);
          if (blob) out.photo = await PhotoStore.blobToDataUrl(blob);
        } catch (_) {}
      }
      marriages[k] = out;
  ...
  function openExport() {
    ...
    backupBtn.addEventListener("click", async () => {
      ...
      try {
        const backup = await buildBackupState();
        const familyName = (FamilyStore.getFamilyTitle ? FamilyStore.getFamilyTitle() : null)
