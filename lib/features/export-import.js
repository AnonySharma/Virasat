...
(function (global) {
  ...
  function openExport() {
    ...
    const state = {
      ...
      // PNG display features, seeded from the live tree View Options.
      includePets: liveShow.pets,
      includeAge: liveShow.age,
      includeRelations: liveShow.relations,
      includeEras: liveShow.eras,
      format: FULL,
      pngQuality: "standard",
    ...
    // Data fields (Photos, Dates, Places) — the "what goes in" chips shared by
    // the image and data exports. Single instances, so a toggle sticks when the
    // format changes. Places only lands in the data file (the image never draws
    // a location), so it's hidden for the image format.
    const placesChip = makeFieldChip({
      label: I18n.t("exp.places"), icon: "fa-solid fa-location-dot",
      active: true,
      onChange: (on) => { state.includeLocations = on; refreshPreview(); }
    });
    const includeChips = UI.el("div", { class: "field-chips" }, [
      makeFieldChip({
    ...
    // "Shown in the image" — PNG-only display features that mirror the live
    // tree's View Options. Each chip trims that feature out of the exported
    // image; they don't affect the data file (which carries data, not on-canvas
    // chrome). Relations is only offered when it could actually draw (self
    // pinned + "relation to me" on live), matching the live popover's gating.
    const imgChips = [
      makeFieldChip({
        label: I18n.t("exp.imgPets"), icon: "fa-solid fa-paw",
        active: state.includePets,
        onChange: (on) => { state.includePets = on; }
      }),
      makeFieldChip({
        label: I18n.t("exp.imgAge"), icon: "fa-solid fa-hourglass-half",
        active: state.includeAge,
        onChange: (on) => { state.includeAge = on; }
      }),
      makeFieldChip({
        label: I18n.t("exp.imgEras"), icon: "fa-solid fa-layer-group",
        active: state.includeEras,
        onChange: (on) => { state.includeEras = on; }
      })
    ];
    ...
    if (liveShow.relations) {
      imgChips.push(makeFieldChip({
        ...
        active: state.includeRelations,
        onChange: (on) => { state.includeRelations = on; }
      }));
    }
    const imageExtras = UI.el("div", { class: "field-chips" }, imgChips);

    // Minimal-mode (data only): overrides the field chips and outputs just
    // id/name/parents/spouses/gender/timestamps. Sits beside the size estimate
    // so the format-vs-size trade-off reads in one place; muting the data-field
    // chips signals that they no longer apply.
    const minimalChip = makeFieldChip({
      ...
      onChange: (on) => {
        ...
        includeChips.classList.toggle("is-locked", on);
        refreshPreview();
      }
    });
    ...
    // Image: on-canvas features + quality. Data: minimal-mode + size estimate
    // (its data fields live in the shared chips above). Backup: a reassurance
    // note — it deliberately takes no options.
    ...
    const dataPanel = UI.el("div", { class: "exp-panel" }, [jsonRow]);
    ...
    const includeGroup = group(I18n.t("exp.includeInExport"), includeChips);
    ...
    // Reflect the chosen format: light up its card, show only its panel, hide
    // the data-field chips for a backup (which ignores them), hide Places for
    // the image, and repoint the single Save button's icon + label.
    function applyFormat() {
      ...
      includeGroup.style.display = isBackup ? "none" : "";
      placesChip.style.display = fmt === "json" ? "" : "none";
    ...
    async function exportPng() {
      try {
        ...
        const { blob, filename } = await ImageExport.exportTreePng({
          ...
          includePets: state.includePets,
          includeAge: state.includeAge,
          includeEras: state.includeEras,
          includeRelations: state.includeRelations,
