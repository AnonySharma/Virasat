...
(function (global) {
  ...
  //    can warn the user.
  async function buildBackupState(srcOverride) {
    ...
    const src = srcOverride || FamilyStore.getState();
    let photoFailures = 0;
    let photoTotal = 0;
      ...
      try {
        ...
        if (photoFailures === 0) {
          ...
          UI.toast("Backup saved, but " + photoFailures + " of " + photoTotal
            + " photos couldn't be read from storage — same-device restore will still work.", "warning");
        }
        dlg.close();
      } catch (e) {
