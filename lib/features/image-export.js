...
(function (global) {
  ...
  function liveSubtreeBBox(liveSvg, focusSet) {
    try {
      let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
      const nodes = liveSvg.querySelectorAll(".t-node");
      nodes.forEach((g) => {
  ...
  async function exportFullProfile(personId, { density = 2 } = {}) {
    ...
    ctx.textAlign = "left";
    const familyName = (FamilyStore.getState && FamilyStore.getState().meta && FamilyStore.getState().meta.familyName) || "Virasat";
