...
(function (global) {
  ...
  const CSS_VARS = [
    ...
    "--text", "--text-2", "--text-3", "--text-4", "--bg-elev",
    "--bone",
  ...
  // blobToDataUrl lives on PhotoStore — same primitive shared with
  // export-import. Local alias keeps the rest of this file readable.
  ...
  // Memoise per-export: when two people share the same photo (object URL or
  // file path), we read+encode the blob only once and reuse the data URL.
  async function inlineSvgImages(clonedSvg) {
    const images = Array.from(clonedSvg.querySelectorAll("image"));
    const cache = new Map();
    function cachedHref(href) {
      if (!cache.has(href)) cache.set(href, hrefToDataUrl(href));
    ...
    await Promise.all(images.map(async (img) => {
      ...
      if (!href) return;
      try {
        const dataUrl = await cachedHref(href);
        if (dataUrl) {
  ...
  function buildInlineStyle(vars) {
    ...
    const text4 = vars["--text-4"] || "#A39C89";
    const bone = vars["--bone"] || "#FFFCF5";
    return `
      .t-node-bg {
        fill: ${paper};
        stroke: ${paperLine};
      ...
      /* Unknown-parent placeholders — dashed, muted, so they read as "recorded
         as unknown" and never compete with real people in the printed sheet.
         CSS custom props don't survive SVG serialisation, so literals here. */
      .t-phantom-photo-bg { fill: ${paper}; }
      .t-phantom-ring {
        fill: none;
        stroke: ${text2};
        stroke-width: 1.75;
      ...
      /* The knot is a wedding-rings <image> emblem over a transparent hit
         circle — same as the live tree. The old rule painted the knot's inner
         circle as a gold-ringed paper disc, which hijacked that invisible hit
         target and drew a heavy ring the live tree never shows. Keep the hit
         circle transparent and give the emblem the same crisp white outline the
         live tree draws via drop-shadow, so the gold rings stay legible over
         any photo — and the export knot matches the screen exactly. */
      .t-couple-knot__hit { fill: transparent; stroke: none; }
      .t-couple-knot__icon {
        filter:
      ...
      /* On-node badges + gutter era labels. These carry ONLY a class in the
         live DOM — their colour lives in views.css, which does NOT travel with
         the detached clone we rasterise. Without these literals every badge
         fell back to SVG's default black fill (paw disc, story-count disc,
         relation pill, and the self gold ring all printed as black blobs).
         Mirror the on-screen palette so a kept badge looks identical in the PNG. */
      .t-node-pet-bg { fill: ${gold}; stroke: ${bone}; stroke-width: 2; }
      .t-node-pet-paw { fill: ${bone}; }
      .t-node-density-bg { fill: ${olive}; stroke: ${bone}; stroke-width: 2; }
      .t-node-density-num { fill: ${bone}; }
      .t-node-relpill-bg { fill: ${olive}; stroke: ${bone}; stroke-width: 1.5; }
      .t-node-relpill-text { fill: ${bone}; font-size: 10px; font-weight: 700; }
      /* Self gold ring — a thicker gold stroke on the pinned person's photo
         ring. Only present when the caller keeps relations (includeRelations). */
      .t-node.is-self .t-node-photo-ring { stroke: ${gold}; stroke-width: 3; }
      /* Left-gutter era axis — faint display-type decade markers, one per row.
         Kept only when includeEras; the bbox is widened to fit them so they
         don't clip. */
      .t-era-label {
        font-family: ${fontDisplay};
        font-size: 15px;
  ...
  function liveContentBBox(liveSvg, includeEras) {
    // Focus mode overlays folded-kin CLUSTER stacks (`.t-node-cluster`) and
    // their dashed tethers (`.t-cluster-edge`) into the node/edge groups, sitting
    // OUTSIDE the real node boxes (a node-width right, or a row-gap above/below
    // their anchor). trimTreeClone strips them from the export — a "+N, tap to
    // expand" affordance is meaningless in a flat image — so they must NOT widen
    // the measured frame, or the saved PNG (and the dialog preview, same path)
    // gets empty margins where the stacks were and the real tree sits off-centre.
    // Hide them just for the getBBox measure, then restore: `display:none` (not
    // `visibility`) is what excludes an element from getBBox, the measure is
    // synchronous, and the `finally` restores even if getBBox throws. In full
    // mode there are no clusters, so `toHide` is empty and this is a no-op.
    const toHide = liveSvg.querySelectorAll(".t-node-cluster, .t-cluster-edge");
    const prevDisplay = [];
    toHide.forEach((el) => { prevDisplay.push(el.style.display); el.style.display = "none"; });
    try {
      // Prefer the union of the content groups so we don't capture stray defs.
      // Eras are the left-gutter decade axis (`.tree-eras`); union them only
      // when the caller keeps them, so the frame widens leftward to fit the
      // labels instead of clipping them. They live in the same user-space as
      // the node/edge groups (no group transform, absolute label coords).
      const sel = includeEras ? ".tree-edges, .tree-nodes, .tree-eras" : ".tree-edges, .tree-nodes";
      const groups = liveSvg.querySelectorAll(sel);
  ...
  // ===== shared clone trimming =====
  // Apply the field-visibility + lineage-focus trims to a cloned tree SVG by
  // class — no rasterising, no var-resolution — so the exact same trim path is
  // shared by the PNG export (which then inlines + rasterises) and the live
  // Export-dialog preview (which just scales the styled clone). One trim path
  // guarantees the preview matches the saved image.
  /** @param {Element} clone
   *  @param {{includePhotos?:boolean,includeDates?:boolean,includeAge?:boolean,includePets?:boolean,includeEras?:boolean,includeRelations?:boolean,focusSet?:Set<string>|null}} [opts] */
  function trimTreeClone(clone, opts) {
    const o = opts || {};
    const includePhotos = o.includePhotos !== false;
    const includeDates = o.includeDates !== false;
    const includeAge = o.includeAge !== false;
    const includePets = o.includePets !== false;
    const includeEras = o.includeEras === true;
    const includeRelations = o.includeRelations === true;
    const focusSet = o.focusSet || null;

    // Drop the in-progress drawing animation class.
    clone.classList.remove("is-drawing");
    // Interactive-only chrome that must never appear in a saved image/preview:
    // the per-node add-relative "+" affordance, and (focus mode) the stacked
    // cluster nodes with their dashed tethers — a "+N, tap to expand" affordance
    // is meaningless in a flat image, so both the stack and its edge are dropped.
    Array.from(clone.querySelectorAll(".t-node-add")).forEach((n) => n.remove());
    Array.from(clone.querySelectorAll(".t-node-cluster, .t-cluster-edge")).forEach((n) => n.remove());
    // The "relation to me" overlay (gold self ring + kin pills) is a per-viewer
    // lens: NOT baked into shared art by default, so a shared image is identical
    // for everyone. Keep it only when relations are explicitly requested.
    // `.t-node-selfdeco` is the kin-pill wrapper; `.is-self` paints the ring.
    if (!includeRelations) {
      Array.from(clone.querySelectorAll(".t-node-selfdeco")).forEach((n) => n.remove());
      Array.from(clone.querySelectorAll(".t-node.is-self")).forEach((n) => n.classList.remove("is-self"));
    }
    // A hover lens may have hidden a node's dates row; restore it so visibility
    // follows the includeDates flag below, not that transient lens.
    Array.from(clone.querySelectorAll(".t-node-dates")).forEach((d) => { /** @type {HTMLElement} */ (d).style.display = ""; });
    // Left-gutter era axis — kept only when decade markers are requested.
    if (!includeEras) {
      const eraLayer = clone.querySelector(".tree-eras");
      if (eraLayer) eraLayer.remove();
    }
    ...
    if (focusSet) {
      ...
      // Unknown-parent placeholders (nodes + dashed links). Keep one only if a
      // child it belongs to is inside the focus set — else a lineage export
      // leaks a phantom for an unrelated child.
      Array.from(clone.querySelectorAll(".t-phantom-node, .t-phantom-edge")).forEach((n) => {
        const ids = (n.getAttribute("data-phantom-children") || "").split(",").filter(Boolean);
        if (!ids.some((id) => focusSet.has(id))) n.remove();
      });
      // Strip lineage-highlight classes so the output looks like a clean tree.
    ...
    // Field-visibility trims.
    if (!includePhotos) {
      // Remove only PERSON photos (the <image> inside each node); the rings
      // stay. Crucially do NOT touch the wedding-rings knot emblem (also an
      // <image>, but chrome) — a blanket `image` selector once wiped every knot.
      Array.from(clone.querySelectorAll(".t-node image")).forEach((img) => img.remove());
    }
    if (!includeDates) {
      Array.from(clone.querySelectorAll(".t-node-dates")).forEach((t) => t.remove());
    }
    if (!includeAge) {
      // Age badge is a pure overlay disc+number on the ring — trims cleanly.
      Array.from(clone.querySelectorAll(".t-node-density-bg, .t-node-density-num")).forEach((n) => n.remove());
    }
    if (!includePets) {
      // A pet node is identifiable DOM-only by its paw badge (`.t-node-pet-bg`).
      // Remove the whole pet node group AND every dashed pet riser
      // (`.t-edge--pet`); the human trunk is drawn from human children alone, so
      // nothing dangles. The bbox was measured with pets present, so a trimmed
      // pet may leave a little extra margin — acceptable, and far better than a
      // broken line.
      Array.from(clone.querySelectorAll(".t-node")).forEach((g) => {
        if (g.querySelector(".t-node-pet-bg")) g.remove();
      });
      Array.from(clone.querySelectorAll(".t-edge--pet")).forEach((e) => e.remove());
    }
    return clone;
  }

  // ===== buildPreviewSvg =====
  // A lightweight, self-contained <svg> for the Export dialog's LIVE preview:
  // clone the on-screen tree, apply the same trims as the PNG export, and frame
  // it with a viewBox = padded content bbox. No rasterising, no var-resolution,
  // no photo-inlining — the clone lives in the document, so global CSS styles it
  // and blob:/data: portrait hrefs resolve as-is. Returns an Element ready to
  // drop into a container, or null if the tree is empty. Mirrors exportTreePng's
  // framing minus the title band + paper background, which the dialog supplies.
  /** @param {{includePhotos?:boolean,includeDates?:boolean,includePets?:boolean,includeAge?:boolean,includeEras?:boolean,includeRelations?:boolean,focusId?:string|null}} [opts]
   *  @returns {Element|null} */
  function buildPreviewSvg(opts) {
    const o = opts || {};
    const liveSvg = /** @type {SVGSVGElement} */ (document.querySelector("#view-tree .tree-svg"));
    if (!liveSvg) return null;
    const people = (window.FamilyStore && FamilyStore.getPeople()) || [];
    if (!people.length) return null;

    let includeEras = o.includeEras === true;
    const focusSet = o.focusId ? buildFocusSet(o.focusId) : null;
    // Decade markers are a whole-tree gutter axis — meaningless on a subtree.
    if (focusSet) includeEras = false;

    let bbox = focusSet ? liveSubtreeBBox(liveSvg, focusSet) : liveContentBBox(liveSvg, includeEras);
    // Single-node lineage can fall through liveSubtreeBBox; frame the focus node.
    if (focusSet && (!bbox || bbox.w <= 0 || bbox.h <= 0)) {
      const focusNode = liveSvg.querySelector('.t-node[data-person-id="' + cssAttrEscape(o.focusId) + '"]');
      if (focusNode) {
        ...
        bbox = { x: (r.left - svgR.left) * sx + (vb ? vb.x : 0), y: (r.top - svgR.top) * sy + (vb ? vb.y : 0), w: r.width * sx, h: r.height * sy };
      }
    }
    if (!bbox || bbox.w <= 0 || bbox.h <= 0) return null;

    const clone = /** @type {SVGSVGElement} */ (liveSvg.cloneNode(true));
    trimTreeClone(clone, {
      includePhotos: o.includePhotos !== false,
      includeDates: o.includeDates !== false,
      includeAge: o.includeAge !== false,
      includePets: o.includePets !== false,
      includeEras: includeEras,
      includeRelations: o.includeRelations === true,
      focusSet: focusSet
    });

    // Frame the trimmed content: override the live pan/zoom viewBox with the
    // padded content box so the preview shows the whole framed tree, and let
    // the container size it (the viewBox does the scaling).
    const PAD = 40;
    clone.setAttribute("viewBox", (bbox.x - PAD) + " " + (bbox.y - PAD) + " " + (bbox.w + PAD * 2) + " " + (bbox.h + PAD * 2));
  ...
  // ===== exportTreePng =====
  /** @param {{familyName?: string, density?: number, includePhotos?: boolean, includeDates?: boolean, includePets?: boolean, includeAge?: boolean, includeEras?: boolean, includeRelations?: boolean, focusId?: string|null}} [opts] */
  async function exportTreePng({ familyName, density = 2, includePhotos = true, includeDates = true, includePets = true, includeAge = true, includeEras = false, includeRelations = false, focusId = null } = {}) {
    const liveSvg = /** @type {SVGSVGElement} */ (document.querySelector("#view-tree .tree-svg"));
    ...
    // Era labels are a whole-tree gutter axis (one decade per generation across
    // the full width), so they only make sense in a full export — a lineage
    // subset would show decade markers for rows it's cropping out. Force them
    // off when focusing, regardless of the chip.
    if (focusSet) includeEras = false;
    let bbox = focusSet
      ? liveSubtreeBBox(liveSvg, focusSet)
      : liveContentBBox(liveSvg, includeEras);
    ...
    // Clone the SVG, then apply every field/lineage trim. trimTreeClone is
    // shared with the live Export-dialog preview (buildPreviewSvg), so the
    // saved PNG always matches exactly what the dialog previewed.
    const clone = /** @type {Element} */ (liveSvg.cloneNode(true));
    trimTreeClone(clone, { includePhotos, includeDates, includeAge, includePets, includeEras, includeRelations, focusSet });

    // Resolve any `var(--xyz)` literals on every fill / stroke attribute and
    // `style` attribute — done AFTER the trims so only surviving nodes are
    // touched. Without this the rasteriser sees "var(--…)" and falls back to
    // black on attributes (why photos once rendered as solid black discs).
    ...
    // Inline every remaining <image> to a data URL — always, not only when
    // photos are on. When photos are off the only images left are the knot
    // emblems, and their href is a RELATIVE path (assets/wedding-rings.svg)
    // that resolves against nothing once the SVG is detached and rasterised —
    // so without inlining the rings vanished. inlineSvgImages memoises by href,
    // so the shared emblem path is fetched+encoded exactly once.
    const imageInlinePromise = inlineSvgImages(clone);
  ...
  // CSS.escape covers every special character; the manual fallback handles
  // the small set the round-1 helper considered (older browsers without
  // CSS.escape). Person ids are user-mintable via import so this matters
  // even though we generate them ourselves.
  ...
  async function exportFullProfile(personId, { density = 2 } = {}) {
    ...
    // Clamp the device-pixel density so neither axis exceeds MAX_PIXELS. A
    // content-rich profile (many stories/achievements) makes H unbounded, and
    // H * density can blow past the browser canvas cap (~4096px/side on iOS
    // Safari) → a blank/failed export. The other two export paths already clamp
    // to MAX_PIXELS; this one didn't. Scale k down so the whole poster still
    // renders, just at lower resolution, rather than clipping content.
    const longest = Math.max(W, H) * density;
    const k = longest > MAX_PIXELS ? density * (MAX_PIXELS / longest) : density;
    const canvas = document.createElement("canvas");
    canvas.width = Math.round(W * k);
    canvas.height = Math.round(H * k);
    const ctx = /** @type {CanvasRenderingContext2D} */ (canvas.getContext("2d"));
  ...
  // ===== exportMemorialPoster =====
  // A dignified A4-landscape keepsake for someone who has passed: the wide
  // hero crop (photoCropHero) at the top — finally honoured; it was captured
  // by the crop editor but rendered nowhere — under a double-ruled frame, with
  // the name, lifespan, and key dates centred beneath. Always a light parchment
  // sheet regardless of the app theme, so it prints and frames as a memento.
  async function exportMemorialPoster(personId, { density = 2 } = {}) {
    const person = window.FamilyStore && FamilyStore.getPerson(personId);
    if (!person) throw new Error("Person not found");

    const paper = "#F4EFE3";
    const paperEdge = "#EAE3D2";
    const ink = "#241E15";
    const accent = "#B89A5A";
    const accentDeep = "#856B33";
    const text2 = "#4A463E";
    const text3 = "#7A7363";
    ...
    const eyebrow = (window.I18n && I18n.t("profile.inMemoriam")) || "In loving memory";
    const bornLabel = (window.I18n && I18n.t("profile.born")) || "Born";
    const diedLabel = (window.I18n && I18n.t("profile.died")) || "Died";
    ...
    // Parchment ground with a barely-there vignette so the sheet doesn't read
    // as flat white when printed.
    ctx.fillStyle = paper;
    ctx.fillRect(0, 0, W, H);
    const vign = ctx.createRadialGradient(W / 2, H / 2, H * 0.3, W / 2, H / 2, H * 0.75);
    vign.addColorStop(0, "rgba(0,0,0,0)");
    vign.addColorStop(1, "rgba(120, 104, 70, 0.06)");
    ctx.fillStyle = vign;
    ctx.fillRect(0, 0, W, H);

    // Double-ruled memorial frame.
    ctx.strokeStyle = accent;
    ctx.lineWidth = 2;
    ...
    // Eyebrow — "In loving memory", gold, letter-spaced, with a small feather.
    ctx.textAlign = "center";
    ctx.fillStyle = accentDeep;
    ctx.font = '600 20px "Inter", -apple-system, sans-serif';
    ctx.fillText(spaced(eyebrow.toUpperCase(), 3), centerX, 122);

    // Feather glyph rule above the photo (a quiet flourish; falls back to a
    // plain divider if the icon font isn't ready).
    drawCenteredRule(ctx, centerX, 150, 120, accent);

    // ===== Hero band (honours photoCropHero) =====
    const bandW = 640, bandH = Math.round(bandW * 9 / 16); // 640 × 360
    const bandX = centerX - bandW / 2;
    const bandY = 176;
    const bandR = 10;

    // Backing + gold ring.
    drawRoundedRect(ctx, bandX, bandY, bandW, bandH, bandR);
    ...
    if (photoImg) {
      ...
      const ih = photoImg.naturalHeight || photoImg.height;
      if (iw > 0 && ih > 0) {
        // Mirror crop-editor.apply() exactly so the poster frames the photo the
        // same way the user composed the wide hero crop. Fall back to the
        // avatar crop, then to centre, when no hero crop was set.
        const crop = person.photoCropHero || person.photoCropAvatar || { x: 50, y: 50, scale: 1 };
        const scale = Math.max(1, crop.scale || 1);
        const cover = Math.max(bandW / iw, bandH / ih);
        const dw = iw * cover * scale;
        const dh = ih * cover * scale;
        const tx = -(dw - bandW) * ((crop.x != null ? crop.x : 50) / 100);
        const ty = -(dh - bandH) * ((crop.y != null ? crop.y : 50) / 100);
        ctx.drawImage(photoImg, bandX + tx, bandY + ty, dw, dh);
      }
    } else {
      // No photograph — a large monogram on parchment reads with dignity.
      const initials = (FamilyStore.initials && FamilyStore.initials(displayName)) || "?";
      ctx.fillStyle = "rgba(184, 154, 90, 0.12)";
      ctx.fillRect(bandX, bandY, bandW, bandH);
      ctx.fillStyle = accent;
      ctx.font = '600 140px "Fraunces", Georgia, serif';
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(initials, centerX, bandY + bandH / 2 + 4);
      ctx.textBaseline = "alphabetic";
      ctx.textAlign = "center";
    }
    ctx.restore();

    // Gold ring around the band.
    drawRoundedRect(ctx, bandX + 0.5, bandY + 0.5, bandW - 1, bandH - 1, bandR);
    ctx.strokeStyle = accent;
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    const nameLines = wrapText(ctx, displayName, innerW, 2);
    nameLines.forEach((line, i) => ctx.fillText(line, centerX, y + i * 62));
    y += (nameLines.length - 1) * 62;

    // Alternate-script name (Hindi under a Latin name or vice-versa), quiet.
    const altName = (window.I18n && I18n.getLang() === "hi")
      ? (person.name && person.name !== displayName ? person.name : "")
      : (person.name_hi && person.name_hi !== displayName ? person.name_hi : "");
    if (altName) {
      y += 40;
      ctx.fillStyle = text3;
      ctx.font = '400 24px "Fraunces", Georgia, serif';
      ctx.fillText(altName, centerX, y);
    }

    // ===== Lifespan =====
    const lifespan = memorialLifespan(person);
    if (lifespan) {
      y += 52;
      ctx.fillStyle = accentDeep;
      ctx.font = '400 30px "Fraunces", Georgia, serif';
      ctx.fillText(lifespan, centerX, y);
    }

    // Divider knot.
    y += 26;
    drawCenteredRule(ctx, centerX, y, 150, accent);

    // ===== Key dates (Born / Died · place) =====
    const bornLine = memorialDateLine(bornLabel, person.birthDate, person.birthDatePrecision, birthPlace);
    const diedLine = memorialDateLine(diedLabel, person.deathDate, person.deathDatePrecision, deathPlace);
    y += 42;
    ctx.font = '400 22px "Inter", -apple-system, sans-serif';
    ctx.fillStyle = text2;
    if (bornLine) { ctx.fillText(bornLine, centerX, y); y += 34; }
    if (diedLine) { ctx.fillText(diedLine, centerX, y); y += 34; }

    // Occupation as a quiet epitaph line, if there's room and it exists.
    if (occupation && y < H - 130) {
      y += 12;
      ctx.fillStyle = text3;
      ctx.font = 'italic 22px "Fraunces", Georgia, serif';
      const occLines = wrapText(ctx, occupation, innerW - 80, 2);
      occLines.forEach((line, i) => ctx.fillText(line, centerX, y + i * 28));
    }
    ...
    ctx.textAlign = "center";
    ctx.fillText(spaced(String(familyName).toUpperCase(), 2), centerX, H - 74);

    const blob = await canvasToBlob(canvas, "image/png");
    return {
      blob,
      filename: "memorial-" + slugify(displayName) + ".png"
    };
  }

  // Letter-space a string by inserting thin gaps between characters — canvas
  // has no letter-spacing before recent Chrome, so we space manually for the
  // engraved-caps look the memorial eyebrow/footer want.
  function spaced(s, gapPx) {
    const chars = String(s || "").split("");
    if (chars.length < 2) return s;
    // A hair space between letters approximates tracking without measuring.
    const sep = gapPx >= 3 ? "  " : " ";
    return chars.join(sep);
  }

  // Short centred divider with a small diamond in the middle — the memorial
  // "knot" motif, drawn with primitives so it needs no icon font.
  function drawCenteredRule(ctx, cx, cy, width, color) {
    const half = width / 2;
    const gap = 10;
    ctx.save();
  ...
  // "1935 – 2018 · age 83" style, month-agnostic — full-date lines carry the
  // precise dates below.
  function memorialLifespan(person) {
    const by = (window.FamilyStore && FamilyStore.getYear) ? FamilyStore.getYear(person.birthDate) : null;
    const dy = (window.FamilyStore && FamilyStore.getYear) ? FamilyStore.getYear(person.deathDate) : null;
  ...
  // "12 March 1935" / "March 1935" / "1935" from a YYYY[-MM[-DD]] string.
  function memorialFullDate(s) {
    if (!s) return "";
    const m = String(s).match(/^(\d{4})(?:-(\d{2}))?(?:-(\d{2}))?$/);
    if (!m) return String(s);
    const y = +m[1];
    if (!m[2]) return String(y);
    const mo = MONTHS_LONG[(+m[2]) - 1] || "";
    if (!m[3]) return mo + " " + y;
    return (+m[3]) + " " + mo + " " + y;
  }

  // "Born · 12 March 1935 · Lahore", honouring date precision. Returns "" when
  // there's neither a date nor a place to show for that side.
  function memorialDateLine(label, date, precision, place) {
    let dateStr = memorialFullDate(date);
    if (dateStr && precision && precision !== "exact" && window.I18n) {
      const key = precision === "about" ? "date.circa"
        : precision === "before" ? "date.before"
        : precision === "after" ? "date.after" : null;
      if (key) dateStr = I18n.t(key, { year: dateStr });
    }
    const parts = [label];
    if (dateStr) parts.push(dateStr);
    if (place) parts.push(place);
    if (parts.length === 1) return ""; // label alone — nothing recorded
    return parts.join(" · ");
  }

  // ===== save helpers =====
  // Always-download. The header "Save PNG" expects a file in the user's
  // Downloads folder, not the OS share sheet. UI.downloadFile handles the
  // blob→anchor→revoke dance (it takes an already-built Blob as-is).
  function download(blob, filename) {
    global.UI.downloadFile(filename, blob);
  }

  // Save the image. On touch devices, offer the OS share sheet (so a photo can
  // go straight to Messages / Photos / WhatsApp); everywhere else, download it.
  //
  // Why gate on a coarse pointer: desktop Safari/Chrome now report
  // canShare({files}) === true, but the desktop share sheet has no "save to
  // disk" target (only Mail / Messages / AirDrop). Routing desktop through it
  // meant the file never landed in Downloads even though the button "worked" —
  // so on a fine pointer we skip the sheet entirely and download.
  //
  // Returns true when a file was actually saved (downloaded, or a share the
  // user completed) and false when the user dismissed the share sheet — the
  // caller uses this so a cancelled share doesn't toast "saved".
    ...
    if (coarse && navigator.canShare && navigator.share) {
      ...
      if (navigator.canShare({ files: [file] })) {
        try {
          ...
          // User dismissed the sheet — don't claim success, and don't then
          // dump a surprise download on top of the cancel.
          if (e && e.name === "AbortError") return false;
          // A real share failure (not a cancel) → fall through to download.
        }
      }
    }
  ...
  // buildFocusSet is exposed so the JSON / backup export paths scope a
  // lineage-only export to the SAME set the PNG path frames — otherwise the
  // "Only X's lineage" chip would apply to one button and silently lie for
  // the others.
  global.ImageExport = { exportTreePng, buildPreviewSvg, exportProfileCard, exportFullProfile, exportMemorialPoster, share, download, buildFocusSet };
})(window);
