...
(function (global) {
  ...
  async function exportTreePng({ familyName, density = 2, includePhotos = true, includeDates = true, includePets = true, includeAge = true, includeEras = false, includeRelations = false, focusId = null } = {}) {
    ...
    // Strip interactive-only chrome that must never appear in a saved image —
    // the per-node add-relative "+" affordance, and the left-gutter era axis
    // (an on-canvas orientation aid; the export frames only nodes+edges, so a
    // gutter label would fall outside the captured box and clip anyway).
    Array.from(clone.querySelectorAll(".t-node-add")).forEach((n) => n.remove());
    // The "relation to me" overlay (the gold self ring + kin pills) is a
    // per-viewer local lens: by default it's NOT baked into the shared art, so a
    // shared image is identical for every viewer regardless of who they pinned
    // as themselves. When the user explicitly asks to include relations in this
    // image (the Relations export chip), keep the pills + self ring so the PNG
    // matches the tree they're looking at. `.t-node-selfdeco` is the kin pill
    // wrapper; `.is-self` paints the gold photo ring on the pinned person.
    if (!includeRelations) {
    ...
    // A chip may have hidden a node's dates row; restore it for the image so the
    // export shows dates per the user's showDates setting, not this lens.
    Array.from(clone.querySelectorAll(".t-node-dates")).forEach((d) => { /** @type {HTMLElement} */ (d).style.display = ""; });
    // Left-gutter era axis. Historically always stripped (an on-canvas aid, not
    // the art). Now optional: when includeEras is on we keep the layer AND widen
    // the captured bbox leftward to fit the labels (below), so they don't clip.
    if (!includeEras) {
    ...
    if (focusSet) {
      ...
      // Unknown-parent placeholders (nodes + their dashed links). Keep one only
      // if at least one of the children it belongs to is inside the focus set —
      // otherwise a lineage export leaks a phantom for an unrelated child.
    ...
    if (!includePhotos) {
      // Remove only PERSON photos — the <image> inside each node. The photo
      // rings stay, just the portraits go. Crucially do NOT touch the
      // wedding-rings knot emblem (also an <image>, but chrome not a photo):
      // the old blanket `querySelectorAll("image")` wiped it too, so turning
      // photos off silently deleted every couple knot.
    ...
    if (!includeAge) {
      // Age badge is a pure overlay disc+number on the photo ring — removing it
      // leaves no gap, so this trims cleanly. It's cloned from the live tree,
      // which draws the badge, so trimming here is all the PNG path needs.
    ...
    if (!includePets) {
      ...
      // so we don't need the store here. Remove the pet's whole node group AND
      // every dashed pet riser (`.t-edge--pet`) — those serve only pets, and the
      // human trunk/rail is drawn from the human children alone (tree-view.js),
      // so no human connector dangles. The captured bbox was measured with pets
      // present, so a trimmed pet may leave a little extra margin — acceptable,
      // and far better than a broken line.
      Array.from(clone.querySelectorAll(".t-node")).forEach((g) => {
  ...
  // the others.
  global.ImageExport = { exportTreePng, exportProfileCard, exportFullProfile, exportMemorialPoster, share, download, buildFocusSet };
})(window);
