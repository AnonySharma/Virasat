...
(function (global) {
  ...
  // ===== exportTreePng =====
  /** @param {{familyName?: string, density?: number, includePhotos?: boolean, includeDates?: boolean, focusId?: string|null}} [opts] */
  async function exportTreePng({ familyName, density = 2, includePhotos = true, includeDates = true, focusId = null } = {}) {
    ...
    // per-viewer local lens, not part of the shared art — never bake it into an
    // export. Remove the pill/decoration elements AND the .is-self class that
    // paints the gold photo ring, so a shared image is identical for every
    // viewer regardless of who they pinned as themselves.
