...
(function (global) {
  ...
  function liveContentBBox(liveSvg, includeEras) {
    ...
    // their connectors (`.t-cluster-edge`) into the node/edge groups, sitting
    // in the anchor's adjacent generation row (a sibling slot beside its real
    // kin, or centred on the anchor). trimTreeClone strips them from the export
    // — a "+N, tap to expand" affordance is meaningless in a flat image — so
    // they must NOT widen
  ...
  function trimTreeClone(clone, opts) {
    ...
    // cluster nodes with their connectors — a "+N, tap to expand" affordance
