...
(function (global) {
  ...
  // A capped list of event rows plus a "+N more" control that opens the full set
  // in a modal — the panel/section stays a fixed-height glance. `moreTitle` is
  // the modal heading. Returns the list container (never null; may be empty).
  function renderComingUpList(evs, max, moreTitle) {
    ...
    if (hidden) {
      const moreBtn = UI.el("button", { class: "coming-up__more", type: "button" }, [
        UI.el("span", null, I18n.t("insights.comingUpMore", { n: hidden })),
