...
(function (global) {
  ...
  // itself first so the tree reveal is visible. `lead` promotes the row into the
  // list's focal point — the soonest event — trading the small type glyph for
  // the person's avatar and carrying a kind badge, so the next date reads at a
  // glance instead of as one row among many.
  function renderEventRow(ev, onClick, lead) {
    const meta = eventMeta(ev);
    const kindIcon = ev.kind === "death" ? "fa-solid fa-feather" : "fa-solid fa-cake-candles";
    const glyph = lead && UI.avatar
      ? UI.el("div", { class: "coming-up__lead-avatar" }, [
          UI.avatar(ev.person, "sm"),
          UI.el("i", { class: kindIcon + " coming-up__lead-badge coming-up__lead-badge--" + ev.kind, "aria-hidden": "true" })
        ])
      : UI.el("i", { class: kindIcon + " coming-up__icon coming-up__icon--" + ev.kind, "aria-hidden": "true" });
    const row = UI.el("button", {
      class: "coming-up__row" + (lead ? " coming-up__row--lead" : ""), type: "button",
  ...
  // The calendar-export (.ics) + same-day-reminder buttons for the events modal
  // (the one home for "keep these dates") — a calendar file that works everywhere
  // and an opt-in reminder that needs this device's permission. `rerender`
  // repaints the host so a toggled reminder pill reflects its new state. Returns
  // an array of button elements; the modal places them in its always-visible
  // footer and supplies the explanatory hint alongside.
  ...
  function renderEventsModal(title, evs) {
    ...
    // The "keep these dates" controls live in the modal footer — always visible
    // beside Close, above the scrolling row list, rather than buried beneath it.
    // The action buttons repaint in place so a toggled reminder pill reflects its
    // new state without tearing down the open modal; the hint sits on its own line.
    const actions = UI.el("div", { class: "coming-up__actions" });
    function paintActions() {
      while (actions.firstChild) actions.removeChild(actions.firstChild);
      renderReminderActions(paintActions).forEach((n) => actions.appendChild(n));
    }
    paintActions();
    ...
    const footer = UI.el("div", { class: "coming-up__footer" }, [
      UI.el("p", { class: "coming-up__hint" }, I18n.t("anniv.remindHint")),
      UI.el("div", { class: "coming-up__footer-row" }, [actions, closeBtn])
    ]);
    handle = UI.openModal({
      title: title,
      body: UI.el("div", { class: "coming-up__list coming-up__list--modal" }, rows),
      footer: [footer]
    });
  ...
  function renderComingUpList(evs, max, moreTitle) {
    const cap = max || 6;
    // The soonest event leads — render it as the emphasised lead row.
    const rows = evs.slice(0, cap).map((ev, i) => renderEventRow(ev, null, i === 0));
  ...
  function renderCalendar(rerender) {
    ...
    if (!isCurrentMonth) {
      const todayBtn = UI.el("button", { class: "cal-nav__today", type: "button" }, [
        ...
        UI.el("span", null, I18n.t("insights.calendarToday"))
      ]);
