 ...
 *
 * CLOUD (Phase 3): when cloud sync is live (Auth.isCloud() && an active tree),
 ...
 * from cache offline, and Phase 4 migration keeps photoIds verbatim. The
...
(function (global) {
  ...
  function delBlob(id) {
    ...
    // genuinely orphaned, so removing the bucket object stops it leaking.
    // Fire-and-forget: a viewer's RLS rejection or a network blip is
    // swallowed (removePhoto never throws), and a re-surfaced photo (LWW
    // delete/edit race) degrades to initials via getUrl→null, not a broken img.
    removePhoto(id);
  ...
  global.PhotoStore = {
    ...
    clearAll,
    migrateLegacy,
    // Cloud Storage adapter (Phase 3). Exposed for cloud-store wiring + tests;
    // no-ops when cloud is off. Not called directly by the view layer.
    uploadPhoto, downloadPhoto, removePhoto,
