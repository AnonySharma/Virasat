...
(function (global) {
  ...
  let dirtyHook = null;
  // Read-only guard for viewer-role shared trees (Phase 6). When true, every
  ...
  function deletePerson(id) {
    ...
    // single owner of delete-time cleanup — callers (inspector, tree-view,
    // people-view) used to each do their own partial cleanup (or none), which
    // leaked blobs. Now every path through here frees the same set.
    const orphanedBlobs = [];
    const gone = getPerson(id);
    if (gone && gone.photoId) orphanedBlobs.push(gone.photoId); // the person's own photo
    ...
    // with no edges (see audit tree-view #1).
  ...
  function relationLabel(fromId, toId) {
    ...
    // picker). This used to compare against "male"/"female", which never
    // matched, so every gendered label silently fell through to the neutral
    // form ("spouse"/"parent"/"child") in the path-finder.
