...
(function (global) {
  ...
  function collectPhotoIds(st) {
    ...
    if (Array.isArray(st.people)) {
      st.people.forEach((p) => { if (p && p.photoId) ids.add(p.photoId); });
    }
  ...
  function purgeLocalTree(treeId) {
    ...
    // Free only the blobs the doomed tree EXCLUSIVELY owned. Fire-and-forget,
    // guarded like deletePerson's cleanup (data-store loads before photo-store;
    // headless/test runs have no IDB).
    if (typeof window !== "undefined" && window.PhotoStore && PhotoStore.delete) {
      doomed.forEach((id) => { if (id && !keep.has(id)) PhotoStore.delete(id).catch(() => {}); });
    }
  }
