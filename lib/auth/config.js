...
(function (global) {
  ...
  // Typed so the isConfigured() method assigned below type-checks: an object
  // literal alone infers only its three data fields, so a later `.isConfigured`
  // assignment trips ts(2339). Optional (?) because it's attached after the
  // literal — callers already guard with `typeof cfg.isConfigured === "function"`.
  /** @type {{ supabaseUrl: string, supabaseAnonKey: string, bucket: string, isConfigured?: () => boolean }} */
  const config = {
