 ...
 *
 * Virasat ships LOCAL-FIRST. With the empty credentials below,
 * isConfigured() returns false, Auth.ready() resolves { cloud: false }, and
 * the app boots exactly as the offline, single-device PWA it has always
 * been: no network calls, no sign-in screen, no behavioural change, and the
 * Supabase SDK is never even downloaded.
 *
 * To enable cloud sync, paste your Supabase project's URL and anon (public)
 * key below. The anon key is SAFE to commit and serve publicly — it only
 * grants what the database's Row-Level Security policies allow. RLS, not
 * this file, is the security boundary. (Never put the service_role key
 * here.) Full setup lives in docs/CLOUD-SYNC-PLAN.md.
 */
(function (global) {
  "use strict";
