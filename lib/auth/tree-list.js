...
(function (global) {
  ...
  function treeRow(tree, ctx) {
    ...
    if (tree.role === "owner" && global.Sharing && typeof global.Sharing.open === "function") {
      ...
      sh.addEventListener("click", (e) => { e.stopPropagation(); global.Sharing.open(tree.id, tree.title, tree.role); });
      actions.push(sh);
    }
    // Delete: owner only. A shared-in member (viewer/editor) can't delete the
    // tree, but gets a "Leave" action instead — the owner-independent way out.
    if (tree.owned) {
      ...
      const leave = global.UI.el("button", {
        type: "button", class: "tree-row__action tree-row__action--danger", "aria-label": t("tree.leave"), title: t("tree.leave")
      }, [global.UI.el("i", { class: "fa-solid fa-right-from-bracket", "aria-hidden": "true" })]);
      leave.addEventListener("click", (e) => { e.stopPropagation(); ctx.leave(tree); });
      actions.push(leave);
    }
  ...
  function open() {
    ...
    const ctx = {
      switchTo: async (tree) => {
        dlg.close();
        try {
          // Drop the previous tree's People search/filter BEFORE the switch
          // hydrates the new tree — switchTree's hydrateFromRemote fires a
          // notifyAll → render, and we want that render to already see cleared
          // filters (otherwise Tree B paints through Tree A's leftover query,
          // hiding most of it with no visible explanation). reset() itself does
          // not render, so this is a pure state pre-clear.
          if (global.PeopleView && global.PeopleView.reset) global.PeopleView.reset();
      ...
      leave: async (tree) => {
        const ok = await global.UI.confirm({
          title: t("tree.leave"),
          message: t("tree.leaveConfirm", { name: tree.title }),
          confirmLabel: t("tree.leave"),
          danger: true
        });
        if (!ok) return;
        try {
          const wasActive = tree.active;
          const res = await cloud().leaveTree(tree.id);
          if (wasActive && global.Inspector && global.Inspector.clear) global.Inspector.clear();
          if (global.UI.toast) global.UI.toast(t("tree.left"), "success");
          // Leaving your only tree drops to the first-run state, same as deleting
          // the last owned one — re-show the "create your first tree" screen.
          if (res && res.emptied && global.FirstRun && typeof global.FirstRun.show === "function") {
            dlg.close();
            global.FirstRun.show().then(() => {
              if (global.location && global.location.reload) global.location.reload();
          ...
          if (global.UI.toast) global.UI.toast(t("tree.leaveError"), "danger");
          console.error("tree leave failed:", e);
        }
      }
    };
