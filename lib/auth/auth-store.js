...
(function (global) {
  ...
  // Deliberately self-contained (not folded into a UI helper) so this core auth
  // module carries no dependency on the UI layer, and so the undefined return —
  // which Supabase reads as "use the default redirect" — stays exact.
  function redirectTo() {
  ...
  // Update the signed-in user's own profile. Today only the display first
  // name (user_metadata.first_name, mirrored to `name` so Supabase templates
  // and the OAuth `name` field stay on one key — same convention as sign-up).
  // Patches the cached session's user + notifies so the greeting/avatar update
  // without a reload; Supabase also emits USER_UPDATED which notifies again.
  /** @param {{ firstName?: string }} [patch] */
  ...
  // True when this account can sign in with an email+password (so a password
  // change is meaningful). A Google-only user has no "email" identity — the
  // account UI hides the change-password section for them. `identities` isn't
  // always present on a restored session, so fall back to app_metadata.
  function hasPassword() {
    const u = (currentSession && currentSession.user) || null;
    if (!u) return false;
    const ids = Array.isArray(u.identities) ? u.identities : null;
    if (ids) return ids.some((i) => i && i.provider === "email");
    const am = u.app_metadata || {};
    const provs = Array.isArray(am.providers) ? am.providers
      : (am.provider ? [am.provider] : []);
    return provs.indexOf("email") !== -1;
  }

  // Change the signed-in user's password. Supabase's updateUser({password})
  // does NOT require the current one, so we verify it ourselves by
  // re-authenticating first — otherwise anyone on an unlocked session could
  // change it. A successful sign-in just refreshes the same session (no reboot:
  // the boot gate runs once, and CloudStore doesn't subscribe to auth changes).
  // On a wrong current password we throw a coded error the UI maps to friendly
  // copy rather than leaking Supabase's raw "Invalid login credentials".
  ...
  async function updatePassword({ currentPassword, newPassword } = {}) {
    ...
    const { error: reauthErr } = await c.auth.signInWithPassword({
      email, password: String(currentPassword || "")
    });
    if (reauthErr) {
      const e = new Error("current password incorrect");
      /** @type {any} */ (e).code = "current-password";
      throw e;
    }
    const { data, error } = await c.auth.updateUser({ password: String(newPassword || "") });
    if (error) throw error;
    if (data && data.user && currentSession) currentSession.user = data.user;
    notify();
    return data;
