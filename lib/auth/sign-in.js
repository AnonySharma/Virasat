  ...
  function show() {
    return new Promise((resolve) => {
      ...
      const passwordInput = make("input", {
        class: "input input--with-affix", type: "password", autocomplete: "current-password",
        "data-i18n-placeholder": "auth.passwordPlaceholder",
        placeholder: t("auth.passwordPlaceholder", "Your password"), required: true
      });
      // Reveal/hide toggle. tabindex=-1 so keyboard users tab straight from the
      // password field to Submit (the eye is a mouse convenience); the state is
      // still exposed via aria-pressed + a translated aria-label. Flipping the
      // input `type` keeps the field's value and cursor intact.
      const pwToggle = make("button", {
        class: "input-affix", type: "button", tabindex: "-1",
        "aria-pressed": "false", "aria-label": t("auth.showPassword", "Show password")
      }, [make("i", { class: "fa-solid fa-eye", "aria-hidden": "true" })]);
      pwToggle.addEventListener("click", () => {
        const revealed = passwordInput.getAttribute("type") === "text";
        passwordInput.setAttribute("type", revealed ? "password" : "text");
        pwToggle.setAttribute("aria-pressed", revealed ? "false" : "true");
        pwToggle.setAttribute("aria-label",
          t(revealed ? "auth.showPassword" : "auth.hidePassword", revealed ? "Show password" : "Hide password"));
        const icon = pwToggle.querySelector("i");
        if (icon) icon.className = revealed ? "fa-solid fa-eye" : "fa-solid fa-eye-slash";
        passwordInput.focus();
      });
      const passwordControl = make("div", { class: "input-affix-wrap" }, [passwordInput, pwToggle]);
      ...
      // Separates the primary passwordless CTA from the password fallback below
      // it. Only shown in sign-in mode (sign-up always needs a password, so the
      // password path isn't a "fallback" there). Mirrors the "or" Google divider.
      const passwordDivider = make("div", { class: "signin__divider signin__divider--pw" }, [
        ti(make, "span", null, "auth.orPassword", "or use a password")
      ]);
      ...
      function applyMode() {
        ...
        // Button weighting flips with mode. Sign-in leads with the passwordless
        // magic link (primary) and demotes the password submit to a secondary
        // action; sign-up needs a password, so the "Create account" submit is
        // primary and the magic link + its divider are hidden.
        const signup = mode === "signup";
        magicBtn.hidden = signup;
        passwordDivider.hidden = signup;
        magicBtn.classList.toggle("btn--primary", !signup);
        submitBtn.classList.toggle("btn--primary", signup);
        errorBox.className = "signin__error";
        setError("");
      }

      // — Handlers —
      // Email-first, magic-link-primary layout. The lowest-friction path for the
      // elder audience (a one-tap link, no password to remember) gets top weight:
      // shared email → magic-link (primary in sign-in) → "or use a password" →
      // password field → password submit (secondary in sign-in). Sign-up flips
      // it: the password submit becomes "Create account" (primary) and the magic
      // button + its divider hide, since sign-up collects a first name + password.
      const form = make("form", { class: "signin__form" }, [
        ...
        passwordDivider,
        make("label", { class: "field" }, [
          ti(make, "span", "field__label", "auth.password", "Password"), passwordControl
        ]),
        submitBtn
      ]);
      ...
      // A footer link that NAVIGATES to a standalone page (privacy.html /
      // terms.html) — a real <a> so middle-click, open-in-new-tab and copy-link
      // work, and Google's OAuth consent screen can point at the same URL.
      // Opens in a NEW TAB so a visitor reading the terms mid-sign-in doesn't
      // lose the sign-in screen (rel=noopener for safety).
      function footAnchor(labelKey, labelFb, href) {
        return make("a", { class: "landing__foot-link", href: href, target: "_blank", rel: "noopener" }, [
          ti(make, "span", null, labelKey, labelFb)
        ]);
      }
      // Render an array of already-translated paragraph strings as legal prose.
      // Delegates to the shared UI.legalProse so the landing modals, the in-app
      // About route, and the standalone privacy.html / terms.html pages all use
      // one renderer. Kept as a thin local wrapper (same name) so the callers
      // below are untouched, with an inline fallback for the impossible case
      // that dom.js hasn't loaded when the landing paints.
      function prose(paragraphs) {
        if (global.UI && typeof UI.legalProse === "function") return UI.legalProse(paragraphs);
        return (paragraphs || []).map((line) => make("p", { class: "legal__p" }, line));
      }
      ...
      // The About copy lives in i18n (EN + HI) — see landing.aboutBody. This
      // one-line fallback only fires in the impossible case that I18n failed to
      // load entirely; the real content is always the translated array.
      // (Privacy + Terms are standalone pages now — see the footer below.)
      ...
      // The onboarding chrome (lang + theme) is mounted once at the .signin root
      // (below), pinned to the PAGE's top-right, so it stays in the same corner
      // whether the visitor is on the landing intro or has stepped into the auth
      // card — never attached to the card itself.
        ...
        make("div", { class: "landing__inner" }, [
          ...
          make("footer", { class: "landing__footer" }, [
            make("nav", { class: "landing__foot-nav", "aria-label": t("landing.footNavLabel", "About and legal") }, [
              // About stays an in-app modal (only meaningful next to a running
              // tree); Privacy + Terms navigate to their standalone public
              // pages — the canonical URLs Google's OAuth consent screen links.
              ...
              footAnchor("landing.footPrivacy", "Privacy", "privacy.html"),
              make("span", { class: "landing__foot-dot", "aria-hidden": "true" }, "·"),
              footAnchor("landing.footTerms", "Terms", "terms.html")
            ]),
      ...
      // One onboarding chrome for the whole screen, pinned to the PAGE top-right
      // (the .signin root is a fixed, full-viewport layer). It stays in the exact
      // same corner across both the landing and the auth card, instead of the old
      // pair of instances that jumped from page-corner to card-corner on step-in.
      const pageChrome = (global.UI && UI.onboardChrome)
        ? make("div", { class: "onboard-chrome-slot onboard-chrome-slot--page" }, [UI.onboardChrome()])
        : null;

      root = make("div", { class: "signin signin--landing", role: "dialog", "aria-modal": "true", "aria-label": t("app.title", "Virasat") }, [pageChrome, landing, card]);
