...
(function (global) {
  ...
  // Wall-clock ceilings for the create flow. createTree chains several network
  // round-trips (insert → flush → loadRole → loadInto → subscribe); any one can
  // hang indefinitely against a paused/cold Supabase project or a dead socket.
  // A cold start is only "multi-second", so 20s clears a slow-but-working boot
  // while still bounding a truly stuck call. The seed flush is best-effort, so
  // it gets a shorter leash and its timeout is swallowed.
  const CREATE_TIMEOUT_MS = 20000;
  const FLUSH_TIMEOUT_MS = 10000;
  const TIMEOUT_TAG = "virasat:timeout";

  // Race a promise against a timeout that REJECTS (tagged), so a hung network
  // call surfaces as an ordinary failure instead of freezing the screen. The
  // timer is cleared on settle so a fast success leaves no dangling rejection.
  function withTimeout(promise, ms) {
    let timer = null;
    const timeout = new Promise((_, reject) => {
      timer = setTimeout(() => reject(new Error(TIMEOUT_TAG)), ms);
    });
    ...
    return new Promise((resolve) => {
      ...
      async function commit(name, seed, origin) {
        ...
        try {
          ...
          await withTimeout(CloudStore.createTree(name, family), CREATE_TIMEOUT_MS);
          if (seed) {
            ...
            // Photos in `seed` live in this device's IDB (same-device blobs) or
            // arrive inline as base64. migrateLegacy folds the base64 into IDB;
            // backfillToCloud then pushes every referenced blob to the bucket so
            // the tree's photos follow it to other devices (only fileToPhotoId
            // uploads on the hot path, and the seed never went through it).
            // Fire-and-forget: the create UX must not wait on N photo uploads.
            ...
            if (CloudStore.flush) { try { await withTimeout(CloudStore.flush(), FLUSH_TIMEOUT_MS); } catch (_) {} }
          }
          done();
        } catch (e) {
          console.error("first-run create failed:", e);
          const timedOut = e && e.message === TIMEOUT_TAG;
          setError(timedOut
            ? tf("firstRun.createTimeout", "This is taking longer than expected — the server may be waking up. Please try again, or sign out and back in.")
            : tf("firstRun.createError", "Couldn't create the tree. Check your connection and try again."));
          busy = false;
          setBusyLabel(false);
        }
      }
