...
(function (global) {
  ...
  // backup first" flow is a fast-follow; for now the banner + toast tell the user to
  ...
  async function start() {
    ...
    subscribe(treeId);
    startPolling();
    try { if (global.addEventListener) global.addEventListener("online", onOnline); } catch (_) {}
  }
  ...
  function stop() {
    ...
    unsubscribe();
    stopPolling();
    try { if (global.removeEventListener) global.removeEventListener("online", onOnline); } catch (_) {}
    started = false;
    treeId = null;
    userId = null;
    currentRole = null;
