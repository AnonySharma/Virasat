...
(function (global) {
  ...
  // The no-login, view-only link for an unlisted tree: the app's own URL plus
  // the tree id and its secret share token. Anyone opening it sees the tree
  // read-only (PublicView boot path) — the token is what gates access, so treat
  // it like a password. Empty string if either part is missing.
  function publicLinkUrl(treeId, token) {
    if (!treeId || !token) return "";
    try {
      return global.location.origin + global.location.pathname
        + "?v=" + encodeURIComponent(treeId) + "&k=" + encodeURIComponent(token);
    } catch (_) { return ""; }
  }
  function copyLink(text, btn) {
    // Copy via the shared UI.copyText (async Clipboard API + textarea fallback);
    // this function owns the success toast + button "copied" flash, and the
    // failure toast when neither copy path works.
    global.UI.copyText(text).then((ok) => {
      if (!ok) {
  ...
  function open(treeId, treeTitle, role) {
    if (!treeId || !db()) {
      if (global.UI && global.UI.toast) global.UI.toast(t("tree.needSignIn"), "warning");
      return;
    }
    // Only the owner may manage sharing. Trust the role the caller already knows
    // for THIS tree (tree-list has it per-row) — CloudStore.isOwner() reflects
    // only the globally-active tree, so re-deriving it here wrongly locked the
    // owner out of a second owned tree opened from the switcher without switching
    // to it first. Fall back to the active-tree owner check for older callers
    // that pass no role. The invite/revoke RPCs are owner-checked server-side
    // regardless, so this gate is purely about showing the right UI.
    const c = global.CloudStore;
    const activeOwner = role
      ? role === "owner"
      : !!(c && c.activeTreeId && c.activeTreeId() === treeId && c.isOwner && c.isOwner());
    ...
    // — View-only link (owner only) — anyone-with-the-link, no sign-in required.
    // Its own bordered block so it reads as a distinct, more-exposed sharing mode
    // than per-email invites. Populated async by refreshLink() once we read the
    // tree's current visibility/token.
    const linkBlock = global.UI.el("div", { class: "share-publink" });
    if (activeOwner) {
      bodyChildren.push(linkBlock);
    }
    ...
    // — View-only link state + rendering —————————————————————————————————————
    // shared === current on/off; token === the secret. Both read from the tree
    // row (owner can SELECT it) and updated in place by the RPCs so we never
    // need a full dialog rebuild.
    ...
    async function loadLinkState() {
      try {
        const res = await db().from("trees")
          .select("visibility, share_token")
          .eq("id", treeId)
          .single();
        if (res.error) throw res.error;
        linkShared = (res.data && res.data.visibility) === "unlisted";
        linkToken = (res.data && res.data.share_token) || null;
      } catch (e) {
        linkShared = false; linkToken = null;
        console.error("link state load failed:", e);
      }
    }

    function renderLink() {
      if (!activeOwner) return;
      global.UI.clear(linkBlock);

      const head = global.UI.el("div", { class: "share-publink__head" }, [
        global.UI.el("div", { class: "share-publink__title-wrap" }, [
          global.UI.el("i", { class: "fa-solid fa-link share-publink__icon", "aria-hidden": "true" }),
          global.UI.el("span", { class: "share-publink__title" }, t("share.linkShareTitle"))
        ]),
        (function () {
          // A plain checkbox-as-switch: on turns the tree unlisted + mints a
          // token, off returns it to private (killing the link immediately).
          const sw = global.UI.el("input", {
            type: "checkbox", class: "share-publink__switch",
            "aria-label": t("share.linkShareToggle")
          });
          /** @type {HTMLInputElement} */ (sw).checked = linkShared;
          /** @type {HTMLInputElement} */ (sw).disabled = linkBusy;
          sw.addEventListener("change", () => setShared(/** @type {HTMLInputElement} */ (sw).checked));
          return sw;
        })()
      ]);
      linkBlock.appendChild(head);

      // Always-visible explanation of what this mode is.
      linkBlock.appendChild(global.UI.el("p", { class: "share-publink__desc" }, t("share.linkShareDesc")));

      if (!linkShared) return;   // off: nothing more to show

      // The exposure warning — blunt on purpose. Whole-tree, view-only, no login;
      // private contact fields are stripped server-side but everything else is
      // visible to anyone holding the link.
      linkBlock.appendChild(global.UI.el("p", { class: "share-publink__warn" }, [
        global.UI.el("i", { class: "fa-solid fa-triangle-exclamation", "aria-hidden": "true" }),
        " " + t("share.linkShareWarn")
      ]));

      const url = publicLinkUrl(treeId, linkToken);
      if (url) {
        const linkInput = global.UI.el("input", {
          class: "input share-link__input", type: "text", readonly: "readonly", value: url,
          "aria-label": t("share.linkShareUrlLabel")
        });
        linkInput.addEventListener("focus", () => /** @type {HTMLInputElement} */ (linkInput).select());
        const copyBtn = global.UI.el("button", {
          type: "button", class: "btn btn--icon share-link__copy",
          "aria-label": t("share.copyLink")
        }, [global.UI.el("i", { class: "fa-solid fa-copy", "aria-hidden": "true" })]);
        copyBtn.addEventListener("click", () => copyLink(url, copyBtn));
        linkBlock.appendChild(global.UI.el("div", { class: "share-link__row" }, [linkInput, copyBtn]));
      }

      // Rotate — invalidate the old link and mint a new one (the "I shared it too
      // widely" escape hatch). Ghost + small, under the URL row.
      const rotate = global.UI.el("button", {
        type: "button", class: "btn btn--ghost btn--sm share-publink__rotate", disabled: linkBusy ? "disabled" : null
      }, [
        global.UI.el("i", { class: "fa-solid fa-rotate", "aria-hidden": "true" }),
        global.UI.el("span", null, t("share.linkRotate"))
      ]);
      rotate.addEventListener("click", rotateToken);
      linkBlock.appendChild(rotate);
    }

    async function setShared(on) {
      if (linkBusy) return;
      linkBusy = true; renderLink();
      try {
        const res = await db().rpc("set_tree_share", { p_tree: treeId, p_on: !!on });
        if (res.error) throw res.error;
        const out = res.data || {};
        linkShared = out.visibility === "unlisted";
        linkToken = out.share_token || null;
        if (global.UI.toast) global.UI.toast(t(on ? "share.linkOn" : "share.linkOff"), "success");
      } catch (e) {
        if (global.UI.toast) global.UI.toast(t("share.linkError"), "danger");
        console.error("set_tree_share failed:", e);
        await loadLinkState();   // resync the switch to the real state
      } finally {
        linkBusy = false; renderLink();
      }
    }
    ...
    async function rotateToken() {
      if (linkBusy) return;
      const ok = await global.UI.confirm({
        title: t("share.linkRotate"),
        message: t("share.linkRotateConfirm"),
        confirmLabel: t("share.linkRotate"),
        danger: true
      });
      if (!ok) return;
      linkBusy = true; renderLink();
      try {
        const res = await db().rpc("rotate_share_token", { p_tree: treeId });
        if (res.error) throw res.error;
        const out = res.data || {};
        linkShared = out.visibility === "unlisted";
        linkToken = out.share_token || null;
        if (global.UI.toast) global.UI.toast(t("share.linkRotated"), "success");
      } catch (e) {
        if (global.UI.toast) global.UI.toast(t("share.linkError"), "danger");
        console.error("rotate_share_token failed:", e);
      } finally {
        linkBusy = false; renderLink();
      }
    }
    ...
    if (activeOwner) {
      ...
      // Paint the link block immediately (shows the toggle in a loading-safe
      // off state), then fill in the real visibility/token once read.
      renderLink();
      loadLinkState().then(renderLink);
    }
    refresh();
  }
