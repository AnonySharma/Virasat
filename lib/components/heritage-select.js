 ...
 *     onChange: (v) => …,
 *     placeholder: "Choose…",
 *     ariaLabel: "Father"   // accessible name (this is a non-native combobox)
 *   });
...
(function (global) {
  ...
  function create(opts) {
    ...
    // A <label> wrapping this non-native combobox <div> does not reliably
    // produce an accessible name in any browser, so callers pass an explicit
    // ariaLabel for screen readers (relation/gender/precision pickers).
    ...
    // Strategy: commit selection on mousedown inside the menu, via event
    // delegation (mousedown is the most reliable trigger across browsers and
    // CDP). We set `justPicked` when we commit so the wrapper's click handler
    // — which fires AFTER mousedown — recognises that it was a "pick" gesture
    // and skips its open/close toggle. This avoids the menu re-opening
