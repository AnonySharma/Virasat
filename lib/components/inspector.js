...
(function (global) {
  ...
  function renderHighlights() {
    ...
    wrap.appendChild(UI.el("div", { class: "highlights__intro" }, [
      ...
      UI.el("p", { class: "highlights__hint" }, I18n.t("highlights.hint"))
    ]));

    // Collect the candidate highlights, then show ONE as a hero + the rest as a
    // compact row — a single focal point rather than four equal-weight cards.
    // On a small tree the same person often wins several categories (oldest AND
    // most stories AND next birthday); stacking four near-identical cards read as
    // padding, so we dedupe by person and keep only the first mention of each.
    /** @type {Array<{person:any,label:string,footer:string,icon:string}>} */
    const candidates = [];
    function consider(person, label, footer, icon) {
      if (person) candidates.push({ person: person, label: label, footer: footer || "", icon: icon });
    }

    // Oldest known ancestor — earliest non-null birth year. Listed first so it's
    // the natural hero: the root the whole tree descends from.
    /** @type {any} */
    let oldest = null;
    ppl.forEach((p) => {
    ...
    if (oldest) {
      ...
      consider(oldest, I18n.t("highlights.oldest"), I18n.t("highlights.born", { year: y != null ? y : "—" }), "fa-solid fa-tree");
    }

    // Next upcoming anniversary (first hit from upcomingAnniversaries(60)) —
    // timely, so it ranks just under the ancestor.
    if (FamilyStore.upcomingAnniversaries) {
      const evs = FamilyStore.upcomingAnniversaries(60);
      if (evs.length) {
        const ev = evs[0];
        const days = ev.daysAway === 0 ? I18n.t("highlights.today")
          : ev.daysAway === 1 ? I18n.t("highlights.tomorrow")
          : I18n.t("highlights.inDays", { n: ev.daysAway });
        const label = ev.kind === "death" ? I18n.t("highlights.nextMemorial") : I18n.t("highlights.nextBirthday");
        consider(ev.person, label, days, ev.kind === "death" ? "fa-solid fa-feather" : "fa-solid fa-cake-candles");
      }
    }
    ...
    if (mostStories) {
      ...
      consider(mostStories, I18n.t("highlights.mostStories"), I18n.t(n === 1 ? "highlights.storyOne" : "highlights.storyMany", { n }), "fa-solid fa-book-open");
    }
    ...
    if (latest) {
      ...
      consider(latest, I18n.t("highlights.latest"), when ? I18n.t("highlights.added", { when }) : "", "fa-solid fa-user-plus");
    }

    // Dedupe by person: keep the first (highest-priority) mention of each.
    const seen = {};
    /** @type {Array<{person:any,label:string,footer:string,icon:string}>} */
    const unique = [];
    candidates.forEach((c) => {
      const id = c.person && c.person.id;
    ...
    if (hero) {
      wrap.appendChild(UI.el("button", {
        class: "highlights__hero", type: "button",
        onclick: () => revealInTree(hero.person.id)
      }, [
        UI.el("div", { class: "highlights__label" }, [
          UI.el("i", { class: hero.icon, "aria-hidden": "true" }),
          UI.el("span", null, hero.label)
        ]),
        UI.el("div", { class: "highlights__hero-person" }, [
          UI.avatar(hero.person, "lg"),
          UI.el("div", { class: "highlights__hero-name" },
            FamilyStore.getField(hero.person, "name") || hero.person.name)
        ]),
        hero.footer ? UI.el("div", { class: "highlights__footer" }, hero.footer) : null
      ]));
    }

    // Everyone else: a compact, quieter row under the hero.
    const more = unique.slice(1);
    if (more.length) {
      wrap.appendChild(UI.el("div", { class: "highlights__more" }, more.map((c) =>
        UI.el("button", {
          class: "highlights__mini", type: "button", title: c.label + (c.footer ? " · " + c.footer : ""),
          onclick: () => revealInTree(c.person.id)
        }, [
          UI.avatar(c.person, "sm"),
          UI.el("div", { class: "highlights__mini-body" }, [
            UI.el("div", { class: "highlights__mini-label" }, [
              UI.el("i", { class: c.icon, "aria-hidden": "true" }),
              UI.el("span", null, c.label)
            ]),
            UI.el("div", { class: "highlights__mini-name" },
              FamilyStore.getField(c.person, "name") || c.person.name)
          ])
        ])
      )));
    }
    ...
    contentEl.appendChild(UI.el("div", { class: "inspector-actions" }, [
      ...
      deceased ? iconAction("memorial", "fa-solid fa-feather", I18n.t("inspector.actMemorial"), async () => {
        ...
        try {
          ...
          await ImageExport.share(blob, filename, displayName);
          UI.toast(I18n.t("inspector.memorialSaved"), "success");
        } catch (e) {
  ...
  function openStoryEditor(p, existing) {
    ...
    const titleInput = UI.el("input", {
      ...
      placeholder: I18n.t("inspector.storyTitlePlaceholder"),
      oninput: (e) => { draft.title = e.target.value; }
    });
    const bodyInput = UI.el("textarea", {
      class: "textarea",
      style: { minHeight: "180px" },
      placeholder: I18n.t("inspector.storyBodyPlaceholder"),
      oninput: (e) => { draft.body = e.target.value; }
    }, draft.body);
    const tagsInput = UI.el("input", {
      class: "input", type: "text",
      value: draft.tags.join(", "),
      placeholder: I18n.t("inspector.storyTagsPlaceholder"),
      oninput: (e) => {
    ...
    const body = UI.el("div", { class: "form-stack" }, [
      ...
      buildPromptHelper(draft, titleInput, bodyInput),
    ...
    const saveBtn = UI.saveBtn(isEdit ? I18n.t("actions.save") : I18n.t("inspector.addStory"));
    ...
    const dlg = UI.openModal({
      title: isEdit ? I18n.t("inspector.storyEditTitle") : I18n.t("inspector.storyNewTitle"),
      body, footer
    });
