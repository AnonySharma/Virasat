...
(function (global) {
  ...
  function renderHighlights() {
    ...
    // Family calendar — the whole-family month grid marking every birthday &
    // memorial, plus the year's upcoming events. Both are built by the
    // Anniversaries feature module (shared with the Insights "Coming up" band),
    // so the sidebar renders the same surfaces without duplicating markup. Guard
    // for a minimal build where the module didn't load.
    const anniv = window.Anniversaries;
    if (anniv && anniv.renderCalendar) {
      // The calendar's prev/next/"this month" nav mutates state inside the
      // module; a full inspector re-render repaints it in place.
      wrap.appendChild(UI.el("div", { class: "highlights__section" }, [
        UI.el("div", { class: "highlights__section-head" }, [
          UI.el("i", { class: "fa-regular fa-calendar-days", "aria-hidden": "true" }),
          UI.el("span", null, I18n.t("insights.calendarTitle"))
        ]),
        anniv.renderCalendar(() => render())
      ]));

      const evs = anniv.events();
      if (evs.length) {
        // Next event — the soonest birthday/anniversary/memorial, promoted as a
        // hero card (events() is sorted by daysAway, so [0] is next).
        const ev = evs[0];
        const days = anniv.relLabel(ev.daysAway);
        ...
        wrap.appendChild(UI.el("button", {
          ...
          onclick: () => revealInTree(ev.person.id)
        }, [
          UI.el("div", { class: "highlights__label" }, [
            UI.el("i", { class: ev.kind === "death" ? "fa-solid fa-feather" : "fa-solid fa-cake-candles", "aria-hidden": "true" }),
            UI.el("span", null, label)
          ]),
          UI.el("div", { class: "highlights__hero-person" }, [
            UI.avatar(ev.person, "lg"),
            UI.el("div", { class: "highlights__hero-name" },
              FamilyStore.getField(ev.person, "name") || ev.person.name)
          ]),
          UI.el("div", { class: "highlights__footer" }, anniv.dateLabel(ev.date) + " · " + days)
        ]));

        // Coming up — the rest of the year ahead as clickable rows, with "+N
        // more" opening the full list in a modal (skip the event already shown
        // as the hero above so it isn't listed twice).
        ...
        if (rest.length) {
          wrap.appendChild(UI.el("div", { class: "highlights__section" }, [
            ...
            anniv.renderComingUpList(rest, 4, I18n.t("insights.comingUpTitle"))
          ]));
        }
      }
    }
