...
(function (global) {
  ...
  function open(initialFromId, initialToId) {
    ...
    const from = initialFromId || ppl[0].id;
    // Default the second picker to any person who isn't the first — otherwise a
    // seeded `from` that happens to sort first would collide with the default
    // `to` and open on the "same person" empty state.
    let to = initialToId;
    if (!to) { const other = ppl.find((p) => p.id !== from); to = other ? other.id : from; }
    const state = { from: from, to: to };
    ...
    function refresh() {
      ...
      // Header — N hops, plus (when we can name it) the exact kinship term for
      // how the destination relates to the source: "chacha", "bua", "dada",
      // "chachera bhai". English "uncle/aunt/cousin" collapses side + seniority;
      // the vernacular word is the whole point of this chip. Null for shapes
      // outside the close-kin lexicon (second cousins, longer chains) — those
      // keep the per-hop arrows below.
      const hops = path.length - 1;
      const header = UI.el("div", { class: "path-finder__count" },
        hops === 1 ? I18n.t("path.stepOne") : I18n.t("path.stepMany", { n: hops }));
      const kin = window.KinTerms ? KinTerms.label(path) : null;
      if (kin) {
        const nameOf = (pid) => {
          const p = FamilyStore.getPerson(pid);
          return p ? (FamilyStore.getField(p, "name") || p.name) : "";
        };
        header.appendChild(UI.el("span", {
          class: "path-finder__kin",
          // Screen readers get the full sentence ("B is A's chacha"); sighted
          // users just see the word in the chip.
          "aria-label": I18n.t("path.kinAria", { from: nameOf(state.from), to: nameOf(state.to), term: kin })
        }, [
