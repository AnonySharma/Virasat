...
(function (global) {
  ...
  function open(initialFromId, initialToId) {
    ...
    const closeBtn = UI.el("button", { class: "btn", type: "button" }, "Close");
  ...
  // dropdown styling matches the rest of the form.
  function makePersonSelect(people, value, onChange) {
    ...
    if (window.HeritageSelect && HeritageSelect.create) {
      return HeritageSelect.create({ options: opts, value, onChange });
    }
    ...
    return { node: sel, setValue: (v) => { sel.value = v; } };
  }
