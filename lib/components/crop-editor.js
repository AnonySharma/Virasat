...
(function (global) {
   ...
   * @param {{ x?: number, y?: number, scale?: number }} [opts.value]
   * @param {(v: { x: number, y: number, scale: number }) => void} opts.onChange
   */
  function buildFrame({ shape, ratio, label, photoUrl, value, onChange }) {
    const frame = el("div", { class: "crop-frame crop-frame--" + shape });
    // tabindex/role/aria-label make the focal-point surface keyboard-operable:
    // the zoom slider was already reachable, but the drag-to-reposition gesture
    // was mouse/touch-only. Arrow keys nudge the focal point (keydown handler
    // below) through the same apply()/onChange() pipeline the pointer drag uses.
    const inner = el("div", {
      class: "crop-frame__inner",
      tabindex: "0",
      role: "application",
      "aria-label": I18n.t("crop.dragAria", { label })
    });
    ...
    const zoomInput = el("input", {
      ...
      "aria-label": I18n.t("crop.zoomAria", { label })
    });
    ...
    // along each axis — a naive "1 px drag → 1/frame.width %" mapping does
    // nothing on a 16:9 hero frame when the image already covers the short
    // axis (no slack → drag has no visible effect).
    //
    // With object-fit: cover, the rendered image dimensions are
    ...
    // Keyboard nudge: arrows shift the focal point by a fixed step (Shift = a
    // coarser jump). Sign matches the drag (Right/Down reveals more of that
    // edge). Runs through the same apply()/onChange() as the pointer drag, so a
    // keyboard-only user can compose either crop without a mouse.
    function onKeyNudge(e) {
      ...
      e.preventDefault();
      state.x = clamp(state.x + dx, 0, 100);
      state.y = clamp(state.y + dy, 0, 100);
      apply();
      onChange(snapshot());
    }
    inner.addEventListener("keydown", onKeyNudge);
  ...
  function open(photoUrl, opts = {}) {
    return new Promise((resolve) => {
      let result = /** @type {{avatar: any, hero: any}} */ ({ avatar: null, hero: null });
      let dlgRef = null;

      const avatarLabel = I18n.t("crop.avatar");
      const heroLabel = I18n.t("crop.hero");
      ...
      // Snapshot the opening framing so beforeClose can tell whether the user
      // actually changed anything before warning about a discard. `result` is
      // kept live by each frame's onChange (drag/zoom) and by Reset.
      const openSnapshot = JSON.stringify(result);
      let committed = false;
      ...
      const saveBtn = el("button", { class: "btn btn--primary", type: "button" }, [
        ...
        el("span", null, I18n.t("crop.save"))
      ]);

      dlgRef = openModal({
        title: I18n.t("crop.title"),
        body,
        footer: [cancelBtn, saveBtn],
        onClose: () => { /* cancel via close-X */ if (dlgRef && !dlgRef.__resolved) { dlgRef.__resolved = true; resolve(null); } },
        // Unsaved-changes guard, mirroring the person form: if the framing was
        // touched and not saved, veto the dismiss (Cancel, close-X, Escape,
        // backdrop) and confirm the discard first. Dragging both focal points
        // and tuning both zooms is deliberate work; a stray Escape shouldn't
        // silently throw it away (the app has no undo).
        beforeClose: () => {
          if (committed) return true;                          // a save is never "discarded"
          // Read the live frame values so an in-progress drag/zoom counts.
          result.avatar = avatarFrame.getValue();
          result.hero = heroFrame.getValue();
          if (JSON.stringify(result) === openSnapshot) return true;  // untouched → let it close
          if (discardConfirming) return false;                 // confirm already open
          discardConfirming = true;
          UI.confirm({
            title: I18n.t("form.discardTitle"),
            message: I18n.t("crop.discardMsg"),
            confirmLabel: I18n.t("actions.discard"),
            danger: true
          }).then((ok) => {
            discardConfirming = false;
            if (ok) dlgRef.close(true);                         // force past this guard
          });
          return false;
        }
      });
      cancelBtn.addEventListener("click", () => { dlgRef.close(); });
