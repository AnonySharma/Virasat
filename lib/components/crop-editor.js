...
(function (global) {
  ...
  function buildFrame({ shape, ratio, label, photoUrl, value, onChange }) {
    ...
    function onPointerMove(e) {
      ...
      // 1 px pointer move ≈ (100 / pixel-extent / scale) % focal shift.
      const dxPct = ((e.clientX - dragStart.x) / r.width) * 100 / state.scale;
      const dyPct = ((e.clientY - dragStart.y) / r.height) * 100 / state.scale;
