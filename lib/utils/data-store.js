...
(function (global) {
  ...
  function buildGenerations() {
    ...
    // Pull pets to the deepest generation among their owners. Without this
    // they default to 0 (no parents → root row) and the layout sticks them
    // on the top generation, far away from their humans.
    state.people.forEach((p) => {
      if (!p.isPet || !(p.petOwners || []).length) return;
      const ownerGens = p.petOwners.map((oid) => gen.get(oid)).filter((g) => g != null);
      if (ownerGens.length) gen.set(p.id, Math.max(...ownerGens));
    });
    return gen;
  }
