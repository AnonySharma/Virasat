...
(function (global) {
  ...
  function openModal({ title, body, footer, onClose }) {
    ...
    const onKey = (e) => {
      ...
      if (active && active.closest && active.closest(".hdp__pop, .hsel__menu, .modal-portal-exempt")) {
        return;
      }
  ...
  // + Save pair the same way: ghost button with xmark + "Cancel", primary
  // button with floppy-disk + "Save". Centralise so the icon/label set is
  // uniform — and so a future redesign that changes "save" to a different
  // affordance lands in one place.
  ...
  global.UI = { el, clear, clamp, avatar, toast, openModal, confirm, emptyState, cancelBtn, saveBtn, field, downloadFile, pastelFor };
})(window);
