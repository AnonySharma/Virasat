...
(function () {
  ...
  const OVERLAY_ROUTES = { help: openHelpRoute, about: openAbout };
  ...
  // #help → the HelpGuide modal, wired to the route's onClose.
  function openHelpRoute(opts) {
    if (!(window.HelpGuide && HelpGuide.open)) return null;
    return HelpGuide.open(opts);
  }
  ...
  // Route through the overlay router so the guide gets its own #help URL
  // (shareable/bookmarkable) and the Back button closes it.
  document.getElementById("tool-help")?.addEventListener("click", () => {
    openOverlayRoute("help", false);
  });
  ...
  // "Reset everything" lives in the rail Tools for a local-only user, but once
  // signed in it moves into the account page (a destructive action belongs
  // behind the account surface, not one slip from Add). Hide the rail row —
  // and its now-lone divider — when there's an account to house it. Local-only
  // users still get the rail row (the account page is unreachable for them).
  function refreshRailReset() {
    const signedIn = isSignedIn();
    const row = document.getElementById("tool-reset");
    if (row && row.parentElement) row.parentElement.hidden = signedIn;
    // The "Try sample family" row and the hairline divider share this danger
    // cluster; the divider would dangle if Reset were the only thing hidden,
    // but sample stays, so only the Reset <li> toggles. The divider is kept.
  }
  ...
  function openAccountModal() {
    ...
    // — Reset — (editors only; the store no-ops a viewer's clearAll anyway)
    if (canEdit) {
  ...
  // chrome. The two header buttons + the keyboard shortcuts share doUndo/doRedo,
  // which step the store, toast (the reverted change may be off-screen — a
  // profile field, a node the other side of the canvas — so a silent swap would
  // read as "nothing happened"), and refresh the disabled state. The store's
  // subscribe fires on every change, so refreshUndoRedo re-syncs the buttons
  // after any edit, undo, redo, tree switch, or remote hydrate.
  ...
  function refreshUndoRedo() {
    if (undoBtn) undoBtn.disabled = !(window.FamilyStore && FamilyStore.canUndo && FamilyStore.canUndo());
    if (redoBtn) redoBtn.disabled = !(window.FamilyStore && FamilyStore.canRedo && FamilyStore.canRedo());
  }
  function doUndo() {
  ...
  if (window.FamilyStore && FamilyStore.subscribe) FamilyStore.subscribe(refreshUndoRedo);
  refreshUndoRedo();
  ...
  function bootWithCloud() {
    ...
    CloudStore.start()
      ...
      .then(() => maybeFirstRun())
      .then(() => { setupAccountMenu(); renderViewerBanner(); refreshShareButton(); refreshShellTitle(); refreshTreesRail(); refreshRailReset(); refreshLegacyCta(); refreshSyncPip(); bootApp(); });
