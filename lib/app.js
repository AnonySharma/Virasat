...
(function () {
  ...
  function renderViewerBanner() {
    ...
    // Read-only has two sources: a real viewer-role member, or an owner/editor
    // self-previewing. They need different copy (and the preview one an exit),
    // so key the banner on which it is and rebuild if that flips (the two share
    // #viewer-banner so the lineage cue's "sit below the banner" logic still
    // finds exactly one). data-kind lets an already-correct banner short-circuit.
    const preview = !!(window.CloudStore && CloudStore.isPreviewing && CloudStore.isPreviewing());
    const kind = preview ? "preview" : "viewer";
    if (existing) {
    ...
    msg.textContent = I18n.t(preview ? "preview.bannerMsg" : "share.viewerNote");
    banner.appendChild(ico);
    banner.appendChild(msg);
