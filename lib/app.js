...
(function () {
  ...
  if (window.I18n) {
    ...
    I18n.onChange(() => {
      ...
      refreshSyncPip();
      // The Preview button's label + the preview/viewer banner copy are built
      // imperatively (state-dependent), so applyToDOM can't reach them either.
      // renderViewerBanner short-circuits when its kind is unchanged, so drop the
      // stale-language banner first to force a rebuild in the new language.
  ...
  function refreshShareButton() {
    ...
    const canShare = !!(window.CloudStore && CloudStore.isActive && CloudStore.isActive()
      ...
      // Share is owner-only — a real viewer never sees it, so hide it while
      // previewing-as-viewer too, or the preview shows an affordance the guest
      // it's imitating wouldn't have. Returns on exit (refreshed on every store
      // change, which setPreview triggers).
      && !(CloudStore.isPreviewing && CloudStore.isPreviewing()));
    if (btn) btn.hidden = !canShare;
    if (phoneBtn) phoneBtn.hidden = !canShare;
  }

  // Preview-as-viewer — let an owner/editor see their own tree exactly as a
  // view-only guest does (edit affordances gone, viewer read-only guard on).
  // Flips CloudStore.setPreview, which re-applies the read-only guard + repaints;
  // nothing is persisted. The toggle is its own control, so it stays visible in
  // preview (edit-only affordances don't) to offer the way back out.
  function togglePreview() {
    if (!window.CloudStore || typeof CloudStore.setPreview !== "function") return;
    CloudStore.setPreview(!(CloudStore.isPreviewing && CloudStore.isPreviewing()));
  }
  // Reveal the Preview button for an owner OR editor of a live cloud tree (a real
  // viewer is already read-only — nothing to preview). Gated on canEdit(), the
  // stable role, NOT FamilyStore.isReadOnly() — which is true mid-preview and
  // would hide the button's own exit. Label/icon flip to reflect the live state.
  function refreshPreviewButton() {
    const btn = document.getElementById("preview-btn");
    if (!btn) return;
    const canPreview = !!(window.CloudStore && CloudStore.isActive && CloudStore.isActive()
      && CloudStore.canEdit && CloudStore.canEdit()
      && CloudStore.activeTreeId && CloudStore.activeTreeId());
    btn.hidden = !canPreview;
    if (!canPreview) return;
    const on = !!(CloudStore.isPreviewing && CloudStore.isPreviewing());
    const icon = btn.querySelector("i");
    if (icon) icon.className = (on ? "fa-solid fa-eye-slash" : "fa-solid fa-eye");
    const label = btn.querySelector(".btn__label-md");
    if (label) label.textContent = I18n.t(on ? "preview.exit" : "preview.enterShort");
    const aria = I18n.t(on ? "preview.exit" : "preview.enter");
  ...
  function refreshShellTitle() {
    ...
    const inCloud = !!(window.CloudStore && CloudStore.isActive && CloudStore.isActive()
      && CloudStore.activeTreeId && CloudStore.activeTreeId());
    // A no-login shared-link visitor has no CloudStore session but DID hydrate a
    // named tree — show its family title just like a signed-in member would.
    const inPublic = !!(window.PublicView && PublicView.isActive && PublicView.isActive());
    if ((inCloud || inPublic) && window.FamilyStore && FamilyStore.getFamilyTitle) {
  ...
  function openKebabMenu(anchor) {
    ...
    if (isSignedIn()) {
      ...
      // Preview-as-viewer — owner/editor only. The desktop header Preview button
      // is in .app-header__actions (display:none ≤768px), so the kebab is the
      // phone path. Gated on canEdit() (the stable role), so the row survives
      // into preview to offer the way out; label flips with the live state.
      if (window.CloudStore && CloudStore.canEdit && CloudStore.canEdit()
          && CloudStore.activeTreeId && CloudStore.activeTreeId()) {
        const previewing = !!(CloudStore.isPreviewing && CloudStore.isPreviewing());
        menu.appendChild(row(
          previewing ? "fa-solid fa-eye-slash" : "fa-solid fa-eye",
          I18n.t(previewing ? "preview.exit" : "preview.enter"),
          togglePreview
        ));
      }
  ...
  // Clear this device's localStorage by key prefix. ALWAYS preserves
  // "virasat.activeTreeId" (so a reset never bounces a multi-tree user onto a
  // different tree) and the Supabase "sb-*" auth session (never matched by our
  // prefixes) — so a signed-in user stays logged in and re-hydrates from the
  // cloud on the reload that follows.
  function clearLocalByPrefix(prefixes) {
    try {
      Object.keys(localStorage).forEach((k) => {
        if (k === "virasat.activeTreeId") return;
        if (prefixes.some((p) => k.indexOf(p) === 0)) localStorage.removeItem(k);
      });
    } catch (_) {}
  }

  // Reset preferences — restore on-device settings (all "virasat.*" keys: view
  // toggles, theme, contrast, filters, one-time hints) to defaults, then reload
  // so each module re-reads them. Never touches the tree ("familyTree.*") or the
  // auth session, so it's safe whether signed in or local-only.
  async function resetPreferencesFlow() {
    const ok = await UI.confirm({
      title: I18n.t("auth.resetPrefsTitle"),
      message: I18n.t("auth.resetPrefsBody"),
      confirmLabel: I18n.t("auth.resetPrefsConfirm")
    });
    if (!ok) return;
    clearLocalByPrefix(["virasat."]);
    location.reload();
  }

  // Clear cached data & re-sync — signed-in only. Flush any pending edits to the
  // cloud first (so nothing unsaved is lost), then drop this device's local copy
  // AND settings ("virasat.*" + "familyTree.*"), keep the auth session, and
  // reload: boot re-downloads the tree from the account (CloudStore.loadInto →
  // FamilyStore.hydrateFromRemote). Nothing in the cloud is deleted.
  async function clearCacheAndResyncFlow() {
    const ok = await UI.confirm({
      title: I18n.t("auth.clearCacheTitle"),
      message: I18n.t("auth.clearCacheBody"),
      confirmLabel: I18n.t("auth.clearCacheConfirm")
    });
    if (!ok) return;
    try { if (window.CloudStore && CloudStore.flush) await CloudStore.flush(); } catch (_) {}
    clearLocalByPrefix(["virasat.", "familyTree."]);
    location.reload();
  }
  ...
  function openAccountMenu(anchor) {
    ...
    if (signedIn) {
      ...
      // One identity line + the email beneath. Greet by first name when we have
      // one; otherwise the neutral "Signed in as" label. (Previously showed both
      // the greeting AND the label — a redundant third line that added clutter.)
      const firstName = (window.Auth && Auth.getFirstName && Auth.getFirstName()) || "";
      const who = document.createElement("div");
      who.className = firstName ? "kebab-menu__meta-greeting" : "kebab-menu__meta-label";
      who.textContent = firstName ? I18n.t("auth.greeting", { name: firstName }) : I18n.t("auth.signedInAs");
    ...
    // signed in or not — this is why the menu is always present. It groups with
    // Account + Your trees as the "things you can open" cluster (no divider
    // between them); offline it's simply the first row.
    {
    ...
    // — Reset — signed OUT only. Device-local maintenance ("Reset preferences",
    // "Clear cached data & re-sync") now lives in the Account settings modal's
    // "This device" section (see openAccountModal), which decluttered the menu.
    // Offline there's no account surface, so "Reset preferences" (restore
    // on-device settings to defaults) stays here; "Clear cached data & re-sync"
    // is cloud-only, so it's never offered offline.
    if (!signedIn) {
      const rDiv = document.createElement("div");
      rDiv.className = "kebab-menu__divider";
      menu.appendChild(rDiv);
      const rp = document.createElement("button");
      rp.type = "button";
      rp.className = "kebab-menu__item";
      rp.setAttribute("role", "menuitem");
      const rpIco = document.createElement("i");
      rpIco.className = "fa-solid fa-arrow-rotate-left kebab-menu__icon";
      rpIco.setAttribute("aria-hidden", "true");
      const rpLab = document.createElement("span");
      rpLab.textContent = I18n.t("auth.resetPrefs");
      rp.appendChild(rpIco);
      rp.appendChild(rpLab);
      rp.addEventListener("click", () => { menu.remove(); resetPreferencesFlow(); });
      menu.appendChild(rp);
    }
  ...
  function openAccountModal() {
    ...
    // — This device — on-device maintenance that used to sit as two verbose
    // rows in the account menu (which made it feel cluttered). "Reset
    // preferences" restores local settings (view toggles, theme, contrast,
    // one-time hints) to defaults; "Clear cached data & re-sync" drops this
    // device's local copy and re-downloads it from the account. Neither deletes
    // anything from the account itself — that's the danger section below.
    {
      const resetPrefsBtn = UI.el("button", { class: "btn btn--sm", type: "button" }, [
        UI.el("i", { class: "fa-solid fa-arrow-rotate-left", "aria-hidden": "true" }),
        UI.el("span", null, I18n.t("auth.resetPrefs"))
      ]);
      resetPrefsBtn.addEventListener("click", () => resetPreferencesFlow());
      const deviceActions = [resetPrefsBtn];
      // Re-sync pulls from the cloud, so it's a signed-in-only action (the modal
      // is cloud-only anyway, but guard so a future local-only account surface
      // wouldn't offer it).
      if (isSignedIn()) {
        const clearCacheBtn = UI.el("button", { class: "btn btn--sm", type: "button" }, [
          UI.el("i", { class: "fa-solid fa-cloud-arrow-down", "aria-hidden": "true" }),
          UI.el("span", null, I18n.t("auth.clearCache"))
        ]);
        clearCacheBtn.addEventListener("click", () => clearCacheAndResyncFlow());
        deviceActions.push(clearCacheBtn);
      }
      sections.push(UI.el("section", { class: "account-section" }, [
        UI.el("h3", { class: "account-section__title" }, I18n.t("auth.secDevice")),
        UI.el("p", { class: "account-section__note" }, I18n.t("auth.deviceDesc")),
        UI.el("div", { class: "account-section__actions" }, deviceActions)
      ]));
    }
  ...
  function renderViewerBanner() {
    ...
    // Read-only has three sources: a no-login visitor on a shared link, a real
    // viewer-role member, or an owner/editor self-previewing. They need different
    // copy (and the preview one an exit), so key the banner on which it is and
    // rebuild if that flips (all three share #viewer-banner so the lineage cue's
    // "sit below the banner" logic still finds exactly one). data-kind lets an
    // already-correct banner short-circuit.
    const isPublic = !!(window.PublicView && PublicView.isActive && PublicView.isActive());
    const preview = !!(window.CloudStore && CloudStore.isPreviewing && CloudStore.isPreviewing());
    const kind = isPublic ? "public" : (preview ? "preview" : "viewer");
    if (existing) {
      if (existing.getAttribute("data-kind") === kind) return;   // already correct
      existing.remove();
    }
    const banner = document.createElement("div");
    banner.id = "viewer-banner";
    banner.className = "viewer-banner" + (preview ? " viewer-banner--preview" : "");
    ...
    // Public link visitor gets "you're viewing a shared tree" copy; an invited
    // viewer-member gets "you have view-only access"; a self-preview gets the
    // preview note + an exit.
    msg.textContent = I18n.t(isPublic ? "publicView.bannerMsg" : (preview ? "preview.bannerMsg" : "share.viewerNote"));
    banner.appendChild(ico);
    banner.appendChild(msg);
    // Preview is self-invoked, so it carries its own way out — the edit-only
    // affordances (including the header Preview button on phone widths) are
    // hidden while previewing, so this exit lives on the banner itself.
    if (preview) {
      const exit = document.createElement("button");
      exit.type = "button";
      exit.className = "btn btn--sm btn--ghost viewer-banner__exit";
      const exitIco = document.createElement("i");
      exitIco.className = "fa-solid fa-eye-slash";
      exitIco.setAttribute("aria-hidden", "true");
      const exitLab = document.createElement("span");
      exitLab.textContent = I18n.t("preview.exit");
      exit.appendChild(exitIco);
      exit.appendChild(exitLab);
      exit.addEventListener("click", () => {
        if (window.CloudStore && CloudStore.setPreview) CloudStore.setPreview(false);
      });
  ...
  function bootWithCloud() {
    ...
    CloudStore.start()
      ...
      .then(() => maybeFirstRun())
      .then(() => { setupAccountMenu(); renderViewerBanner(); refreshShareButton(); refreshPreviewButton(); refreshShellTitle(); refreshTreesRail(); refreshDestructiveTools(); refreshLegacyCta(); refreshSyncPip(); bootApp(); });
  ...
  // Public share link (?v=<treeId>&k=<token>) — a no-login, read-only visitor.
  // This is checked FIRST, before the sign-in gate, so someone opening a shared
  // link never sees a sign-in wall: PublicView loads the tree read-only via the
  // anon RPC and we boot straight over it. On failure (bad/rotated token, SDK
  // blocked) PublicView paints its own "not available" screen and we do NOT
  // fall through to the normal app (which would flash the sign-in gate).
  function tryPublicView() {
    if (!(window.PublicView && PublicView.isPublicView && PublicView.isPublicView())) return false;
    showSplash();
    PublicView.boot()
      .then((ok) => {
        hideSplash();
        if (!ok) return;   // PublicView already showed its error screen — stay put.
        bootApp();
        // Paint the read-only chrome for the anonymous visitor: the viewer
        // banner (public variant) + the family name in the shell title. The
        // cloud-gated controls (share/preview/sync-pip/trees rail) stay hidden
        // on their own since CloudStore never started.
        renderViewerBanner();
        refreshShellTitle();
      })
      .catch((e) => { hideSplash(); console.error("public view boot failed:", e); bootApp(); });
    return true;
  }
  ...
  if (tryPublicView()) {
    // handled — public read-only path owns boot
  } else if (window.Auth && typeof Auth.ready === "function") {
    Auth.ready().then((res) => {
