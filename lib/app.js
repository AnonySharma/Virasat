...
(function () {
  ...
  function filterListFacet(people, dim, field, title, options, icon, repaint, chip) {
    ...
    const rows = shown.map((x) => filterRow(dim, x.o.value, x.o.label, x.count, { icon, chip }));
  ...
  function renderFilterModalBody() {
    ...
    if (idx.hasGender) {
      ...
      addGroup(filterGroup(railT("gender"), codes.map((code) => {
        ...
        return (count > 0 || filterState.gender === code)
          ? filterRow("gender", code, gv(code), count, { icon: icons[code], chip: true })
          : null;
      }), { chip: true }));
    }
    ...
    if (idx.decades.length) {
      addGroup(filterGroup(railT("era"), idx.decades
        ...
        .map((x) => filterRow("era", x.d, railT("eraDecade", { d: x.d }), x.count, { icon: "fa-solid fa-calendar-day", chip: true })),
      { chip: true }));
    }

    // Birthplace + Occupation (capped, frequency-sorted).
    addGroup(filterListFacet(people, "place", "birthPlace", railT("birthplace"), idx.places, "fa-solid fa-location-dot", repaint, true));
    addGroup(filterListFacet(people, "occupation", "occupation", railT("occupation"), idx.occupations, "fa-solid fa-briefcase", repaint, true));
    ...
    addGroup(filterGroup(railT("maintenance"), needsDefs.map(([field, icon, label]) => {
      ...
      return (count > 0 || filterState.missing === field)
        ? filterRow("missing", field, label, count, { icon, danger: true, chip: true })
        : null;
    }), { chip: true }));
