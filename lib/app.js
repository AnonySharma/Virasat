...
(function () {
  ...
  function filterMatchesExcept(p, exclude) {
    if (!p) return false;
    const s = filterState;
    if (exclude !== "status") {
  ...
  function renderFilterPanel() {
    ...
    // Prune selections whose value no longer exists, so the panel can't strand
    // the family behind an invisible filter.
    if (filterState.place && !idx.places.some((o) => o.value === filterState.place)) filterState.place = "";
    if (filterState.occupation && !idx.occupations.some((o) => o.value === filterState.occupation)) filterState.occupation = "";
    if (filterState.era && !idx.decades.includes(filterState.era)) filterState.era = "";
    if (filterState.gender && !idx.hasGender) filterState.gender = "";
    saveFilter();

    while (host.firstChild) host.removeChild(host.firstChild);

    const t = (k, p) => I18n.t("rail." + k, p);
    function countFor(dim, test) {
      let n = 0;
      for (const p of people) if (filterMatchesExcept(p, dim) && test(p)) n++;
      return n;
    }
    function row(dim, value, label, count, opts) {
      opts = opts || {};
      const active = filterState[dim] === value;
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "rail-item rail-item--filter" + (active ? " is-active" : "") + (opts.danger ? " rail-item--needs" : "");
      ...
      btn.addEventListener("click", () => {
        ...
        const cur = filterState[dim];
        const next = cur === value ? (dim === "status" ? "all" : "") : value;
        applyFilter({ [dim]: next });
      });
      return btn;
    }
    function group(title, rows) {
      ...
      wrap.className = "rail-fgroup";
      const h = document.createElement("h4");
      h.className = "rail-fgroup__title";
      h.textContent = title;
      wrap.appendChild(h);
    ...
    // drop zero-count options (unless selected), sort by frequency, and show
    // the top few behind a "Show all (N)" toggle.
    function listFacet(dim, field, title, options, icon) {
      const scored = options
        .map((o) => ({ o, count: countFor(dim, (p) => facetKey(p, field) === o.value) }))
        .filter((x) => x.count > 0 || filterState[dim] === x.o.value)
      ...
      const rows = shown.map((x) => row(dim, x.o.value, x.o.label, x.count, { icon }));
      const g = group(title, rows);
      if (g && scored.length > CAP) {
        const more = document.createElement("button");
        more.type = "button";
        more.className = "rail-showmore";
        more.textContent = expanded ? t("showLess") : t("showMore", { n: scored.length });
        more.addEventListener("click", () => { filterExpanded[dim] = !expanded; renderFilterPanel(); });
        g.appendChild(more);
      }
      return g;
    }

    // Clear-all — only when something is narrowed.
    const activeN = filterActiveCount();
    if (activeN > 0) {
    ...
    addGroup(group(t("status"), [
      row("status", "all", t("all"), countFor("status", () => true), { icon: "fa-solid fa-circle-dot" }),
      row("status", "alive", t("living"), countFor("status", (p) => FamilyStore.isAlive(p)), { icon: "fa-solid fa-leaf", iconColor: "var(--sage)" }),
      row("status", "deceased", t("deceased"), countFor("status", (p) => FamilyStore.isDeceased(p)), { icon: "fa-regular fa-circle" })
    ]));

    // Gender — only when at least one person has one on file, and only rows
    // that match somebody (or are the active pick).
    if (idx.hasGender) {
      const gv = (code) => I18n.t("form.gender" + code.toUpperCase());
      addGroup(group(t("gender"), [
        row("gender", "m", gv("m"), countFor("gender", (p) => p.gender === "m"), { icon: "fa-solid fa-mars" }),
        row("gender", "f", gv("f"), countFor("gender", (p) => p.gender === "f"), { icon: "fa-solid fa-venus" }),
        row("gender", "o", gv("o"), countFor("gender", (p) => p.gender === "o"), { icon: "fa-solid fa-genderless" })
      ].filter((btn, i) => {
        const codes = ["m", "f", "o"];
        return countFor("gender", (p) => p.gender === codes[i]) > 0 || filterState.gender === codes[i];
      })));
    }

    // Era (birth decade) — drop zero-count decades unless one is the active
    // pick, matching Gender and the capped facets (no dead-end rows).
    if (idx.decades.length) {
      addGroup(group(t("era"), idx.decades
        .map((d) => ({ d, count: countFor("era", (p) => personDecade(p) === d) }))
        .filter((x) => x.count > 0 || filterState.era === x.d)
        .map((x) => row("era", x.d, t("eraDecade", { d: x.d }), x.count, { icon: "fa-solid fa-calendar-day" }))
      ));
    }

    // Birthplace + Occupation (capped, frequency-sorted).
    const placeGroup = listFacet("place", "birthPlace", t("birthplace"), idx.places, "fa-solid fa-location-dot");
    if (placeGroup) host.appendChild(placeGroup);
    const occGroup = listFacet("occupation", "occupation", t("occupation"), idx.occupations, "fa-solid fa-briefcase");
    if (occGroup) host.appendChild(occGroup);

    // Needs attention — folded in from the old standalone rail block. Counts
    // are context-aware (via countFor) like every other facet here, so the
    // number shown always matches what a click surfaces even when another
    // filter is active; a zero-count row is dropped unless it's the active pick.
    const needsDefs = [
      ["birth", "fa-regular fa-calendar-xmark", I18n.t("rail.needsBirth")],
      ["photo", "fa-regular fa-image", I18n.t("rail.needsPhoto")],
      ["description", "fa-regular fa-pen-to-square", I18n.t("rail.needsDescription")]
    ];
    addGroup(group(I18n.t("rail.maintenance"), needsDefs.map(([field, icon, label]) => {
      const count = countFor("missing", (p) => personMissing(p, field));
      return (count > 0 || filterState.missing === field)
        ? row("missing", field, label, count, { icon, danger: true })
        : null;
    })));
  }
