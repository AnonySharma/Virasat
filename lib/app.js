(function () {
  ...
  function bootApp() {
    ...
    // First-run sample offer — only on a genuinely empty tree.
  ...
  // unchanged. When cloud is on: boot if signed in, else show the sign-in gate
  // and boot once a session is established.
  if (window.Auth && typeof Auth.ready === "function") {
    Auth.ready().then((res) => {
      if (!res || !res.cloud || res.session) { bootApp(); return; }
      if (window.SignIn && typeof SignIn.show === "function") {
        SignIn.show().then(() => bootApp());
      } else {
