(function () {
  ...
  function refreshAnniversaries() {
    const block = document.getElementById("rail-anniversaries-block");
    const list = document.getElementById("rail-anniversaries");
    if (!block || !list || !FamilyStore.upcomingAnniversaries) return;
    const events = FamilyStore.upcomingAnniversaries(30);
    if (!events.length) { block.hidden = true; return; }
    block.hidden = false;
    list.innerHTML = "";
    events.slice(0, 6).forEach((ev) => {
      const card = document.createElement("button");
      card.className = "rail-anniv__row" + (ev.kind === "death" ? " rail-anniv__row--memorial" : "");
      card.type = "button";
      card.title = (ev.kind === "death" ? "Death anniversary — " : "Birthday — ") + ev.date;
      card.addEventListener("click", () => {
        if (window.Inspector) Inspector.show(ev.person.id);
      });
      const ico = document.createElement("i");
      ico.className = ev.kind === "death" ? "fa-solid fa-feather rail-anniv__icon" : "fa-solid fa-cake-candles rail-anniv__icon";
      const name = document.createElement("span");
      name.className = "rail-anniv__name";
      name.textContent = (FamilyStore.getField(ev.person, "name") || ev.person.name);
      const days = document.createElement("span");
      days.className = "rail-anniv__days";
      days.textContent = ev.daysAway === 0 ? "today"
        : ev.daysAway === 1 ? "tomorrow"
        : "in " + ev.daysAway + " d";
      card.appendChild(ico); card.appendChild(name); card.appendChild(days);
  ...
  function refreshMaintenance() {
    ...
    function row(field, count, icon, label) {
      ...
      btn.type = "button";
      btn.innerHTML = '<i class="' + icon + '"></i><span>' + label + '</span><span class="rail-count">' + count + '</span>';
      btn.addEventListener("click", () => {
