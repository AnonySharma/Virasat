...
(function () {
  ...
  function setupAccountMenu() {
    ...
    btn.setAttribute("title", email || I18n.t("auth.account"));
    btn.appendChild(UI.avatar({ name: email || "?" }, "sm"));
  ...
  // Keep the banner in sync when the role changes (tree switch) without a full
  // reboot — cloud-store toggles body.is-viewer, so re-evaluate on store change.
  if (window.FamilyStore && FamilyStore.subscribe) FamilyStore.subscribe(renderViewerBanner);
  ...
  function bootWithCloud() {
    ...
    CloudStore.start()
      ...
      .then(() => maybeFirstRun())
      .then(() => { setupAccountMenu(); renderViewerBanner(); bootApp(); });
