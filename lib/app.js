...
(function () {
  ...
  function pruneFilterState(people) {
  ...
  function filterCountFor(people, dim, test) {
    let n = 0;
    for (const p of people) if (filterMatchesExcept(p, dim) && test(p)) n++;
    return n;
  }
  // One selectable facet row (reused by the rail Status list and the modal).
  ...
  function filterListFacet(people, dim, field, title, options, icon, repaint) {
    const scored = options
      .map((o) => ({ o, count: filterCountFor(people, dim, (p) => facetKey(p, field) === o.value) }))
    ...
    const rows = shown.map((x) => filterRow(dim, x.o.value, x.o.label, x.count, { icon }));
    const g = filterGroup(title, rows);
  ...
  function filterModalActiveCount() {
    let n = 0;
    if (filterState.gender) n++;
    if (filterState.place) n++;
    if (filterState.occupation) n++;
  ...
  function renderFilterModalBody() {
    ...
    if (!host) return;
    const people = FamilyStore.getPeople();
    const idx = pruneFilterState(people);
    while (host.firstChild) host.removeChild(host.firstChild);
    ...
    if (idx.hasGender) {
      ...
      addGroup(filterGroup(railT("gender"), codes.map((code) => {
        const count = filterCountFor(people, "gender", (p) => p.gender === code);
        return (count > 0 || filterState.gender === code)
          ? filterRow("gender", code, gv(code), count, { icon: icons[code] })
          : null;
      })));
    }
    ...
    if (idx.decades.length) {
      addGroup(filterGroup(railT("era"), idx.decades
        .map((d) => ({ d, count: filterCountFor(people, "era", (p) => personDecade(p) === d) }))
        .filter((x) => x.count > 0 || filterState.era === x.d)
        .map((x) => filterRow("era", x.d, railT("eraDecade", { d: x.d }), x.count, { icon: "fa-solid fa-calendar-day" }))
      ));
    }

    // Birthplace + Occupation (capped, frequency-sorted).
    addGroup(filterListFacet(people, "place", "birthPlace", railT("birthplace"), idx.places, "fa-solid fa-location-dot", repaint));
    addGroup(filterListFacet(people, "occupation", "occupation", railT("occupation"), idx.occupations, "fa-solid fa-briefcase", repaint));
    ...
    addGroup(filterGroup(railT("maintenance"), needsDefs.map(([field, icon, label]) => {
      const count = filterCountFor(people, "missing", (p) => personMissing(p, field));
      return (count > 0 || filterState.missing === field)
        ? filterRow("missing", field, label, count, { icon, danger: true })
        : null;
    })));
  ...
  function openFilterModal() {
    ...
    const clearBtn = UI.el("button", {
      ...
      onclick: () => { filterExpanded.place = false; filterExpanded.occupation = false; applyFilter({ ...FILTER_DEFAULT }); }
    }, [
    ...
    const doneBtn = UI.el("button", {
      ...
      onclick: () => { if (filterModal) filterModal.close(); }
    }, [UI.el("span", null, I18n.t("actions.done") || "Done")]);
    filterModal = UI.openModal({
      title: railT("moreFilters"),
      body: filterModalBody,
      footer: [clearBtn, doneBtn],
      onClose: () => { filterModal = null; filterModalBody = null; }
    });
  }
