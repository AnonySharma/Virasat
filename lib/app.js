...
(function () {
  ...
  function setupAccountMenu() {
    ...
    // Signal that an account menu exists: CSS then hides the header appearance
    // toggles ([data-appearance-toggle]) because Theme + High-contrast now live
    // inside this menu. Only reached when signed in, so local-only mode never
    // sets it and keeps its header toggles (it has no avatar to host them).
  ...
  function openAccountMenu(anchor) {
    ...
    // avatar there). Each row clicks the hidden header button so applyTheme /
    // applyContrast stay the single source of truth — the same indirection the
    // phone kebab uses. Labels reflect the CURRENT state (read at open time) so
    // the row says what the tap will DO ("Dark mode" when currently light).
    {
      const aDiv = document.createElement("div");
      aDiv.className = "kebab-menu__divider";
      menu.appendChild(aDiv);
      const row = (icon, label, targetId) => {
        const item = document.createElement("button");
        item.type = "button";
        item.className = "kebab-menu__item";
        item.setAttribute("role", "menuitem");
        ...
        item.appendChild(ic);
        item.appendChild(lab);
        item.addEventListener("click", () => { menu.remove(); document.getElementById(targetId)?.click(); });
        return item;
      };
      const isDark = document.documentElement.getAttribute("data-theme") === "dark";
      menu.appendChild(row(
