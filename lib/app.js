...
(function () {
  ...
  function openAccountMenu(anchor) {
    ...
    // — Reset — device-local maintenance, just above Sign out. "Reset
    // preferences" restores on-device settings to defaults (everyone). Signed
    // in, also offer "Clear cached data & re-sync", which rebuilds the local
    // copy from the cloud. Neither deletes anything from the account.
    {
      ...
      if (signedIn) {
        ...
        cc.type = "button";
        cc.className = "kebab-menu__item";
        cc.setAttribute("role", "menuitem");
        const ccIco = document.createElement("i");
        ccIco.className = "fa-solid fa-cloud-arrow-down kebab-menu__icon";
        ccIco.setAttribute("aria-hidden", "true");
        const ccLab = document.createElement("span");
        ccLab.textContent = I18n.t("auth.clearCache");
        cc.appendChild(ccIco);
        cc.appendChild(ccLab);
        cc.addEventListener("click", () => { menu.remove(); clearCacheAndResyncFlow(); });
        menu.appendChild(cc);
      }
    }
