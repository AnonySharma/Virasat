...
(function (global) {
  ...
  function showNodeMenu(person, pos, ev) {
    ...
    items.push(
      divider(),
      // "Focus descendants" — sticks the current person as the lineage focus
      // root and closes the inspector. The dim/highlight effect persists
      // across panel-close, banner shows "Viewing as: X · Reset". If the user
      // taps the same item on a person already focused, treat it as Reset.
      menuItem(
  ...
  global.TreeView = { mount, render, setFilter, applyHighlightClasses, revealPerson };
