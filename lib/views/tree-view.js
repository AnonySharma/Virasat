...
(function (global) {
  ...
  function drawClusters(positions) {
    ...
    buckets.forEach((bucket) => {
      ...
      // real siblings. If the anchor already has visible kin in that row, sit
      // the stack one sibling-slot to their right; otherwise centre it under
      // (down) / over (up) the anchor — a real single child sits there too.
      let cx, cy;
      if (bucket.side === "down") {
        cx = clusterColumnX(positions, bucket.anchorId, anchor, "down", R);
        cy = anchor.y + NODE_H + Y_GAP + PHOTO_CY;      // child-row photo centre
      } else if (bucket.side === "up") {
        cx = clusterColumnX(positions, bucket.anchorId, anchor, "up", R);
        cy = anchor.y - NODE_H - Y_GAP + PHOTO_CY;      // parent-row photo centre
      } else {
        cx = anchor.x + NODE_W + X_GAP_SIBLING + R;     // beside the anchor, same row
        cy = anchor.y + PHOTO_CY;
      }
      drawClusterConnector(bucket, anchor, cx, cy, R);
      appendClusterNode(bucket, cx, cy, anchor);
    });
  }

  // Column for a cluster stack: if the anchor has visible kin already occupying
  // the target generation row (real children for a down-bucket, visible parents
  // for an up-bucket), drop the stack one sibling gap to the right of the
  // right-most of them so it never lands on top of a real node. With no visible
  // kin in that row, centre it on the anchor — exactly where a real lone child
  // would sit, so the solid connector reads as a normal single branch.
  function clusterColumnX(positions, anchorId, anchor, side, R) {
    let rightmostX = -Infinity;
    positions.forEach((pos) => {
      const p = pos.person;
      const inRow = side === "down"
        ? (p.parents || []).includes(anchorId) || (p.isPet && (p.petOwners || []).includes(anchorId))
        : (anchor.person.parents || []).includes(p.id);
      if (inRow && pos.x > rightmostX) rightmostX = pos.x;
    });
    if (rightmostX > -Infinity) return rightmostX + NODE_W + X_GAP_SIBLING + R;
    return anchor.x + NODE_W / 2;
  }

  // The connector from an anchor to its cluster stack — the SAME solid
  // orthogonal skeleton real kin get (trunk from the anchor's edge, a rail at
  // the mid-line, a riser into the stack, corners rounded with the R=10 fillet),
  // so a folded sub-branch is drawn in the tree's own structural vocabulary.
  //   down → trunk from the anchor's BOTTOM edge down into the stack's top,
  //   up   → trunk from the anchor's TOP edge up into the stack's bottom,
  ...
  function drawClusterConnector(bucket, anchor, cx, cy, R) {
    const ax = anchor.x + NODE_W / 2;   // anchor's photo column
    let d;
    if (bucket.side === "side") {
      ...
      const startY = down ? anchor.y + NODE_H : anchor.y;  // anchor bottom / top edge
      const endY = down ? cy - R : cy + R;                 // stack top / bottom edge
      d = orthoConnector(ax, startY, cx, endY);
    }
