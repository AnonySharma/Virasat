 ...
 *     getLineageFocus, clearLineageFocus, getTreeMode, setTreeModeTransient }.
...
(function (global) {
  ...
  // True once the user has explicitly re-centred the focus view THIS session
  // (a tap on a node, or a "reveal in tree" from another view). Until then
  // resolveFocusId treats the centre as unpinned and defaults to the marked
  // "self" — so a fresh load, or a session where nobody was tapped, centres on
  // "you" rather than on a stale persisted centre. Reset on every tree switch.
  let focusUserChosen = false;
  ...
  function syncFocusToActiveTree() {
    ...
    if (tid !== focusTreeId) {
      focusTreeId = tid;
      focusId = readFocusId();
      // A new tree hasn't been interacted with yet — its centre is unpinned, so
      // resolveFocusId defaults to that tree's "self" rather than any centre
      // carried over (or persisted from a prior session on this tree).
      focusUserChosen = false;
    }
  }

  // Resolve the person the focus view should centre on. Preference order (D2):
  //   1. an explicit re-center made THIS session — a tap or "reveal in tree"
  //      (focusUserChosen) — so a chosen centre survives subsequent re-renders
  //   2. the pinned "self" (SelfAnchor) — the default centre: "focus on you"
  //   3. a root (someone with no parents) — the natural top of a lineage
  //   4. a centre persisted from a prior session (weaker than self/root, but a
  //      better guess than an arbitrary pick when neither exists)
  //   5. the current inspector selection, then simply the first non-pet person
  ...
  function resolveFocusId() {
    ...
    if (focusUserChosen && has(focusId)) return focusId;
    try {
  ...
  // exactly ONE visible anchor — its nearest kept neighbour, or the ego itself
  // when it has no kin path to the spine at all (a not-yet-linked relative, a
  // separate branch). So the union of all clusters' members === the folded set,
  // always. Nothing drops, even from a disconnected component.
  //
  ...
  function bucketizeFolded(keep, foldedIds) {
    ...
    const gen = FamilyStore.buildGenerations();
    // Nearest visible anchor for every folded id, in ONE multi-source BFS: seed
    // the frontier with all kept (spine) people at once, each carrying its own
    // id as the anchor tag, then flood outward over the kin graph. The first
    // wave to reach a folded person tags them with the kept person that wave
    // started from — i.e. their nearest visible relative. This is O(V+E) total
    // (same adjacency as the DOI walk) rather than a separate, capped BFS per
    // folded person, and — crucially — it has no hop cap, so no connected
    // relative is ever left anchor-less by distance alone.
    const anchorOf = new Map(); // foldedId -> nearest kept anchorId
    ...
    while (frontier.length) {
      ...
      for (const cur of frontier) {
        for (const nid of focusNeighbors(cur.id)) {
    ...
    foldedIds.forEach((fid) => {
      // A folded person the waves never reached sits in a component with no
      // visible person — a not-yet-linked relative, a separate branch. They must
      // still be shown (the focus view promises nothing is silently dropped), so
      // hang them off the ego, the one person guaranteed to be on the spine.
      const anchorId = anchorOf.get(fid) || focusId;
      if (!anchorId) return; // no ego at all (empty tree) — nothing to anchor to
  ...
  //
  // The stack lands as if it were one more node in the anchor's ADJACENT
  // generation — down-bucket → the child row, up-bucket → the parent row — and
  // is joined by the SAME solid orthogonal trunk→rail→riser connector that real
  // kin use (drawClusterConnector). So a "+N" of folded descendants reads
  // honestly as a collapsed sub-branch sitting among the real siblings, not as
  // a lone child dangling on a bespoke dashed tether (the earlier design, which
  // was pixel-identical to a parent→child edge and misread as "one child").
  function drawClusters(positions) {
    ...
    const R = PHOTO_R * 0.7; // stack ring radius — must match appendClusterNode
    buckets.forEach((bucket) => {
      ...
      if (!anchor) return;
      // Place the stack in the anchor's adjacent generation row, at the same
      // photo-centre height a real node there would have, so it lines up with
      // real siblings. Sit it one sibling-slot to the right of the RIGHT-MOST
      // real node already in that row (counting a child's married-in spouse,
      // who sits there too — see clusterRowRightmostX); with no real kin in the
      // row, centre it on the trunk — a real lone child sits there too.
      if (bucket.side === "side") {
        const cx = anchor.x + NODE_W + X_GAP_SIBLING + R; // beside the anchor, same row
        const cy = anchor.y + PHOTO_CY;
        drawClusterConnector(bucket, anchor, cx, cy, R, null);
        appendClusterNode(bucket, cx, cy, anchor);
        return;
      }
      // DOWN → more children of the anchor's PARENT-UNIT; hang the stack off the
      // same couple trunk the real children use (coupleAnchorPoint mirrors
      // drawEdges' anchorX) so its connector merges with the sibling rail rather
      // than dropping from one partner's photo column as a stray second line.
      // UP → folded ancestors: a plain child→parent riser from the anchor.
      const down = bucket.side === "down";
      const trunk = down
        ? coupleAnchorPoint(positions, anchor)
        : { x: anchor.x + NODE_W / 2, yTop: anchor.y, yBottom: anchor.y + NODE_H };
      const rightmostX = clusterRowRightmostX(positions, anchor, bucket.side);
      const cx = rightmostX != null
        ? rightmostX + NODE_W + X_GAP_SIBLING + R
        : trunk.x;                                        // no real kin — centre on the trunk
      const cy = down
        ? anchor.y + NODE_H + Y_GAP + PHOTO_CY            // child-row photo centre
        : anchor.y - NODE_H - Y_GAP + PHOTO_CY;           // parent-row photo centre
      drawClusterConnector(bucket, anchor, cx, cy, R, trunk);
      appendClusterNode(bucket, cx, cy, anchor);
    });
  }

  // The couple trunk-point a DOWN cluster hangs off — identical to the anchorX
  // drawEdges computes for a real child group (lines "pp.length === 2 …"), so
  // the cluster connector merges into the SAME trunk the anchor's real children
  // descend from instead of dropping as a stray second line from one partner's
  // photo column. When the anchor is coupled (a spouse placed in the same row)
  // that's the midpoint of the couple's outer photo edges; otherwise the
  // anchor's own photo column. yBottom/yTop are the unit's outer edges.
  function coupleAnchorPoint(positions, anchor) {
    const spouses = anchor.person.spouses || [];
    for (const sid of spouses) {
      const sp = positions.get(sid);
      if (sp && sp.rowIdx === anchor.rowIdx) {
        const left  = anchor.x < sp.x ? anchor : sp;
        const right = anchor.x < sp.x ? sp : anchor;
        return {
          x: (left.x + NODE_W + right.x) / 2,
          yTop: Math.min(anchor.y, sp.y),
          yBottom: Math.max(anchor.y, sp.y) + NODE_H
        };
      }
    }
    return { x: anchor.x + NODE_W / 2, yTop: anchor.y, yBottom: anchor.y + NODE_H };
  }

  // Right-most x of the real nodes already filling the cluster's target row, so
  // the stack sits one sibling-slot past them and never overlaps. For a DOWN
  // bucket that row is the anchor-couple's children — INCLUDING each child's
  // married-in spouse, who sits beside them on the couple gap; the "+N" used to
  // land on that spouse (e.g. Lalpati) because a blood-child-only test missed
  // her. For an UP bucket it's the anchor's visible parents. Returns null when
  // the row has no real node — a lone folded branch, centred on the trunk.
  function clusterRowRightmostX(positions, anchor, side) {
    if (side !== "down") {
      let up = -Infinity;
      positions.forEach((pos) => {
        if ((anchor.person.parents || []).includes(pos.person.id) && pos.x > up) up = pos.x;
      });
      return up > -Infinity ? up : null;
    }
    // DOWN: the couple = anchor + its same-row spouse(s); the row = their blood
    // children plus the placed spouses those children married in.
    const coupleIds = new Set([anchor.person.id]);
    (anchor.person.spouses || []).forEach((sid) => coupleIds.add(sid));
    const bloodChildIds = new Set();
    positions.forEach((pos) => {
      const p = pos.person;
      const isChild = (p.parents || []).some((pid) => coupleIds.has(pid))
        || (p.isPet && (p.petOwners || []).some((pid) => coupleIds.has(pid)));
      if (isChild) bloodChildIds.add(p.id);
    });
    let rightmostX = -Infinity;
    positions.forEach((pos) => {
      const p = pos.person;
      const inRow = bloodChildIds.has(p.id)
        || (p.spouses || []).some((sid) => bloodChildIds.has(sid)); // married-in spouse
      if (inRow && pos.x > rightmostX) rightmostX = pos.x;
    });
    return rightmostX > -Infinity ? rightmostX : null;
  }

  // The connector from an anchor to its cluster stack — the SAME solid
  // orthogonal skeleton real kin get (trunk from the couple/anchor edge, a rail
  // at the mid-line, a riser into the stack, corners rounded with the R=10
  // fillet), so a folded sub-branch is drawn in the tree's own structural
  // vocabulary.
  //   down → trunk from the couple's BOTTOM edge down into the stack's top,
  //          joining the sibling rail at the SAME height real children use so
  //          the "+N" reads as the right-most sibling riser off a shared rail,
  //   up   → riser from the anchor's TOP edge up into the stack's bottom,
  //   side → a plain horizontal link (same generation, no parent/child implied).
  // `trunk` is the {x, yTop, yBottom} from coupleAnchorPoint (null for side).
  // Class stays .t-cluster-edge (styled solid, ink) so the PNG-export strip,
  // the export-measure hide, and the lineage-fade rule all keep matching it.
  function drawClusterConnector(bucket, anchor, cx, cy, R, trunk) {
    let d;
    if (bucket.side === "side") {
      const x1 = anchor.x + NODE_W / 2 + PHOTO_R;  // right edge of anchor photo
      d = `M ${x1} ${cy} L ${cx - R} ${cy}`;
    } else if (bucket.side === "down") {
      // Match drawEdges' rail height (midpoint of couple bottom and child-row
      // top) so this rail overlays+extends the real sibling rail instead of
      // running parallel ~20px below it.
      const railY = (trunk.yBottom + (cy - PHOTO_CY)) / 2;
      d = orthoConnector(trunk.x, trunk.yBottom, cx, cy - R, railY);
    } else { // up
      d = orthoConnector(trunk.x, trunk.yTop, cx, cy + R);
    }
    edgesG.appendChild(svgEl_("path", { class: "t-cluster-edge", d }));
  }

  // Orthogonal trunk→rail→riser path between two points, corners filleted with
  // the same R=10 radius drawEdges uses. Straight vertical when aligned; a
  // rail at the mid-line otherwise. Works both downward (y2 > y1) and upward.
  // railYOverride pins the rail to a specific height (a DOWN cluster passes the
  // real sibling rail's Y so its rail overlays theirs); default is the midpoint.
  function orthoConnector(x1, y1, x2, y2, railYOverride) {
    if (x1 === x2) return `M ${x1} ${y1} L ${x2} ${y2}`;
    const railY = railYOverride != null ? railYOverride : (y1 + y2) / 2;
    const r = Math.min(10, Math.max(2, Math.abs(x2 - x1) / 2));
    const vsign = y2 > y1 ? 1 : -1;   // trunk/riser direction
    const hsign = x2 > x1 ? 1 : -1;   // rail direction
    return `M ${x1} ${y1}`
      + ` L ${x1} ${railY - vsign * r}`
      + ` Q ${x1} ${railY}, ${x1 + hsign * r} ${railY}`
      + ` L ${x2 - hsign * r} ${railY}`
      + ` Q ${x2} ${railY}, ${x2} ${railY + vsign * r}`
      + ` L ${x2} ${y2}`;
  }

  // One stacked cluster: fanned photo-rings (up to 3) behind a count badge.
  // The connector back to the anchor is drawn separately (drawClusterConnector)
  // so this owns only the stack itself. Keyboard + pointer reachable; tap/Enter
  // expands it. aria-label names the count and anchor so a screen-reader user
  // knows what's hidden and where.
  ...
  function recenterFocus(personId) {
    ...
    hideClusterPeek();
    focusId = personId;
    focusUserChosen = true;  // an explicit tap pins this centre over the self default
    writeFocusId(focusId);
  ...
  function revealPerson(personId) {
    ...
    // Focus mode: the target may be folded away into a cluster — not on the
    // drawn spine, so there's no node to pan to (lastPositions has no entry).
    // Re-center the focus view on them first (D2: "reveal" makes them the new
    // ego), which re-prunes, re-lays-out, and tweens them into the middle. Then
    // skip the snap-pan below so it doesn't fight that in-flight tween. No-op in
    // full mode, and when they're already the centre, so those fall through to
    // the normal pan/select path unchanged.
    let recentred = false;
    if (treeMode === "focus" && focusId && personId !== focusId
        && !(lastPositions && lastPositions.get && lastPositions.get(personId))) {
      const target = FamilyStore.getPerson(personId);
      // Only re-center on someone who'd actually be drawn — a hidden pet (pets
      // toggled off) isn't in the layout for a reason, so don't make it the ego.
      if (target && (showPets || !target.isPet)) {
        recenterFocus(personId);
        recentred = true;
      }
    }
    ...
    const pos = lastPositions && lastPositions.get ? lastPositions.get(personId) : null;
    if (!recentred && pos && svgEl && isFinite(viewBox.w) && viewBox.w > 0) {
      const nodeCx = pos.x + NODE_W / 2;
      const nodeCy = pos.y + NODE_H / 2;
  ...
  // Current display mode ("full" | "focus"). Read by the export dialog to decide
  // whether to offer a PNG scope choice (focused view vs whole tree).
  function getTreeMode() { return treeMode; }
  // Transiently switch display mode WITHOUT persisting the choice — used by the
  // export dialog to render the whole tree for a "Whole tree" PNG while the
  // user's actual mode preference (in localStorage) is untouched, then restore
  // it on close. Re-renders synchronously so the caller can clone the fresh DOM.
  // Returns the mode that was in effect before, so the caller can restore it.
  //
  // Preserves the user's expanded-cluster set across the round-trip: full-mode
  // render() clears expandedClusters (they're meaningless without a focus), but
  // a TRANSIENT flip is invisible bookkeeping for the export preview and must
  // not disturb what the user had opened. Snapshot on the way out of focus,
  // restore on the way back. focusId is untouched by the flip to full and
  // re-resolves to the same person on the way back, so the saved cluster keys
  // (anchorId#side) stay valid; render() drops any that no longer bucket.
  let transientExpandedBackup = null;
  function setTreeModeTransient(mode) {
    const prev = treeMode;
    const next = mode === "focus" ? "focus" : "full";
    if (next === prev) return prev;
    if (prev === "focus" && next === "full") {
      transientExpandedBackup = new Set(expandedClusters);
    }
    treeMode = next;
    if (treeMode === "focus") {
      syncFocusToActiveTree();
      focusId = resolveFocusId();
      // Restore before render() so the revived clusters lay out as real nodes.
      if (transientExpandedBackup) {
        expandedClusters = transientExpandedBackup;
        transientExpandedBackup = null;
      }
    } else {
      hideClusterPeek();
    }
  ...
  global.TreeView = { mount, render, setFilter, applyHighlightClasses, revealPerson, getLineageFocus, clearLineageFocus, getTreeMode, setTreeModeTransient };
