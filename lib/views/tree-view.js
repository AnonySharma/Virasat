...
(function (global) {
  ...
  function appendSelfBadge(g) {
    // Bottom-left of the photo — mirrors the bottom-right add-"+" affordance and
    // stays a corner disc (never over the face), same idiom as the pet/story
    // badges. Plain SVG glyph so it rasterises in PNG/poster exports too.
    const bx = NODE_W / 2 - PHOTO_R + 4;
    const by = PHOTO_CY + PHOTO_R - 4;
    const grp = svgEl_("g", { class: "t-node-selfdeco t-node-self-badge" }, [
      svgEl_("title", null, I18n.t("inspector.youAreSelf") || "This is you"),
      svgEl_("circle", { class: "t-node-self-badge-bg", cx: bx, cy: by, r: 11 }),
      svgEl_("text", {
        class: "t-node-self-badge-glyph", "text-anchor": "middle",
        x: bx, y: by + 4,
        "font-family": '"Font Awesome 6 Free"', "font-weight": "900", "font-size": "11"
      }, "") // fa-user — a gold person disc = "this one is you"
    ]);
    g.appendChild(grp);
  }
  function appendRelPill(g, text, fullTitle) {
    ...
    if (fullTitle) label.appendChild(svgEl_("title", null, fullTitle));
    grp.appendChild(label);
    // Measure to size the rounded backing; fall back to a char estimate when
    // getComputedTextLength is unavailable (headless test shim / hidden view).
    let w = 0;
    try { w = label.getComputedTextLength ? label.getComputedTextLength() : 0; } catch (_) { w = 0; }
    if (!w) w = String(text).length * 7;
    const padX = 8, h = 15;
    const rw = Math.min(NODE_W - 4, w + padX * 2);
  ...
  function decorateSelf(force) {
    ...
    nodesG.querySelectorAll(".t-node").forEach((g) => {
      const id = g.getAttribute("data-person-id");
      if (!id) return;
      if (id === selfId) { g.classList.add("is-self"); appendSelfBadge(g); return; }
