...
(function (global) {
  ...
  function mount(root) {
    ...
    function renderTitle() {
      ...
      const fam = (FamilyStore.getFamilyName ? FamilyStore.getFamilyName() : "Family");
      const familyAccent = UI.el("span", { class: "view-head__title-accent" }, fam);
      titleEl.appendChild(familyAccent);
      titleEl.appendChild(document.createTextNode(" family tree"));
    ...
    function openRename() {
      const current = FamilyStore.getFamilyName();
      ...
      const body = UI.el("div", { class: "form-stack" }, [
        UI.el("p", { style: { margin: 0, color: "var(--text-3)", fontSize: "13px" } },
          'This is the surname or family handle that appears in the tree title and on exported files. The full title shown is "{name} family tree".'),
        UI.field("Family name", input)
      ]);
      ...
      const dlg = UI.openModal({ title: "Rename family", body, footer: [cancelBtn, saveBtn] });
      cancelBtn.addEventListener("click", () => dlg.close());
      saveBtn.addEventListener("click", () => {
        FamilyStore.setFamilyName(input.value);
