...
(function (global) {
  ...
  function mount(root) {
    ...
    function openRename() {
      ...
      const saveBtn = UI.el("button", { class: "btn btn--primary", type: "button" }, [
        UI.el("i", { class: "fa-solid fa-floppy-disk", "aria-hidden": "true" }),
        UI.el("span", null, "Save")
      ]);
  ...
  function topoSignature(people, marriages, viewOpts) {
    ...
    people.slice().sort((a, b) => a.id.localeCompare(b.id)).forEach((p) => {
      parts.push(p.id
        ...
        + "|" + ((p.photo || p.photoId || p.photoUrl) ? "p" : "_"));
    });
      ...
      const empty = UI.el("div", {
        ...
        UI.el("div", { class: "empty", style: { pointerEvents: "auto", background: "var(--bg-elev)" } }, [
          UI.el("div", { class: "empty__icon" }, [
            UI.el("i", { class: "fa-solid fa-tree" })
          ]),
          UI.el("div", { class: "empty__title" }, I18n.t("tree.emptyTitle")),
          UI.el("div", { class: "empty__text" }, I18n.t("tree.emptyText")),
          // Inline CTA — the rail is hidden on phones, so "Add people from
          // the rail" gives mobile users no path. A button right here in
          // the empty state is a one-tap rescue for any size.
          UI.el("button", {
            ...
            UI.el("i", { class: "fa-solid fa-user-plus", "aria-hidden": "true" }),
            UI.el("span", null, I18n.t("actions.addFirst") || "Add your first relative")
          ])
        ])
      ]);
  ...
  function drawEdges(positions) {
    ...
    groups.forEach(({ parents: pp, children: cc }) => {
      ...
      if (orderedHumans.length) {
        ...
        // Shared horizontal rail across the human children, extended to the
        // trunk so the T-junction draws cleanly.
        const minX = Math.min(anchorX, ...humanTops.map((c) => c.x));
        const maxX = Math.max(anchorX, ...humanTops.map((c) => c.x));
  ...
  function applyHighlightClasses() {
    ...
    updateLineageBanner(selectedId);
  }
