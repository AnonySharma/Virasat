...
(function (global) {
  ...
  function drawEdges(positions) {
    ...
    // Couple marker — a short gold horizontal bar connecting the two cards
    // at the BOTTOM (below the dates). Parent-child verticals also drop from
    // this same Y, so a "marriage" reads as one short low bar tethered to
    // both partners, while a "sibling rail" reads as a long horizontal at a
    // different Y (halfway down to the next generation). No more confusion.
    positions.forEach((pos, id) => {
      ...
      person.spouses.forEach((sid) => {
        ...
        const leftCenter  = left.x + NODE_W / 2;
        const rightCenter = right.x + NODE_W / 2;
        const cx = (leftCenter + rightCenter) / 2;
        const cy = Math.max(left.y, right.y) + NODE_H;  // beneath both cards

        // Single horizontal couple bar between the two card centres,
        // skipping a small gap in the middle for the gold knot.
        edgesG.appendChild(svgEl_("path", {
          class: "t-edge t-edge--couple",
          d: `M ${leftCenter} ${cy} L ${cx - 8} ${cy}`
        }));
        edgesG.appendChild(svgEl_("path", {
          class: "t-edge t-edge--couple",
          d: `M ${cx + 8} ${cy} L ${rightCenter} ${cy}`
        }));

        // Gold knot in the middle (two interlocking circles).
