...
(function (global) {
  ...
  const NODE_W = 160;
  const NODE_H = 150;
  // Couples render with overlapping card boxes so the two PHOTOS just touch
  // (~2px between rings). The card has no visible rectangle — just photo,
  // name, dates — so the overlap is harmless visually as long as the names
  // below don't smash together. Names are centred at NODE_W/2 of each card.
  // (NODE_W - 2*PHOTO_R) = 76; X_GAP_COUPLE of -74 leaves photo edges 2px
  // apart, with 86px between name centres — enough for ~7-letter first names
  // before they overlap.
  const X_GAP_COUPLE  = -74;
  ...
  // Tiny modal showing marriage details for a couple. Triggered by clicking
  // the gold knot between two partners.
  function showMarriageModal(a, b) {
    ...
    const partnerLine = (p, name, life) => UI.el("div", {
      ...
      UI.avatar(p, "md"),
      UI.el("div", { style: { display: "flex", flexDirection: "column", gap: "2px" } }, [
        UI.el("div", { style: { fontFamily: "var(--font-display)", fontSize: "18px", fontWeight: "500", color: "var(--text)" } }, name),
        UI.el("div", { style: { fontSize: "12px", color: "var(--text-3)", fontVariantNumeric: "tabular-nums" } }, life)
      ])
    ]);

    const body = UI.el("div", { class: "form-stack" }, [
      UI.el("p", {
        style: { margin: 0, color: "var(--text-3)", fontSize: "13px" }
      }, "Click either partner to open their profile."),
      partnerLine(a, nameA, lifeA),
      UI.el("div", {
        style: { textAlign: "center", color: "var(--gold)", fontSize: "20px", margin: "-4px 0" }
      }, [ UI.el("i", { class: "fa-solid fa-link" }) ]),
      partnerLine(b, nameB, lifeB),
      UI.el("p", {
        style: { margin: "8px 0 0", fontSize: "12px", color: "var(--text-4)", fontStyle: "italic" }
      }, "Marriage dates and stories aren't recorded yet.")
    ]);

    const dlg = UI.openModal({ title: "Marriage", body });
  }
  ...
  // Compact date string for the tree only — no spaces around the en-dash,
  // a hairspace before/after, and "·" instead of "present" for living people
  // so couples' dates don't collide when their photos sit close.
  ...
  function applyHighlightClasses() {
    ...
    if (edgesG) {
      const allEdges = edgesG.querySelectorAll("path, g.t-couple-knot, g.t-couple-knot circle");
      allEdges.forEach((n) => {
