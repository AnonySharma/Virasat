...
(function (global) {
  ...
  function mount(root) {
    ...
    function openRename() {
      ...
      const saveBtn = UI.el("button", { class: "btn btn--primary", type: "button" }, "Save");
  ...
  function render() {
    ...
    if (rootEl && rootEl.__subEl) {
      ...
      // metric, not just a roster size: it tells the user how much of the
      // soul of the tree is captured beyond names and dates.
  ...
  function computeLayout(people) {
    ...
    function placeRow(g) {
      ...
      const order = [];
      const pairs = new Set();
      // Pets shouldn't compete for primary placement — they piggyback on
      // their first owner. Skip them in the main pass and add them right
      // after their owner lands.
      const pets = row.filter((p) => p.isPet);
      const humans = row.filter((p) => !p.isPet);
      humans.forEach((p) => {
        ...
        if (spouseInGen) {
          ...
          // Drop pets owned by the just-placed couple right after the spouse,
          // so they read as "and their dog" without disrupting sibling order.
          pets.forEach((pet) => {
            if (placed.has(pet.id)) return;
            const owners = pet.petOwners || [];
            if (owners.includes(p.id) || owners.includes(spouseInGen)) {
              order.push(pet.id); placed.add(pet.id);
            }
          });
        } else {
          pets.forEach((pet) => {
            if (placed.has(pet.id)) return;
            if ((pet.petOwners || []).includes(p.id)) {
              order.push(pet.id); placed.add(pet.id);
            }
          });
        }
      });
      // Any remaining pets (owners not in this generation, or no owner at
      // all) — append at the end of the row so they still get drawn.
      pets.forEach((pet) => { if (!placed.has(pet.id)) { order.push(pet.id); placed.add(pet.id); } });
