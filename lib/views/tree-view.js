...
(function (global) {
  ...
  // Pet visibility — when off, pets and their tethers are stripped from
  // the layout entirely (not just CSS-hidden) so the tree reflows tighter.
  // Persists across sessions; default ON.
  let showPets = (function () {
    try { return localStorage.getItem("virasat.showPets") !== "0"; }
    catch (_) { return true; }
  })();
  function setShowPets(v) {
    showPets = !!v;
    try { localStorage.setItem("virasat.showPets", showPets ? "1" : "0"); } catch (_) {}
    render();
  }
  ...
  function mount(root) {
    ...
    const petsBtn = UI.el("button", {
      class: "btn" + (showPets ? "" : " is-off"),
      type: "button",
      "aria-label": showPets ? "Hide pets" : "Show pets",
      title: showPets ? "Hide pets" : "Show pets",
      onclick: () => setShowPets(!showPets)
    }, [UI.el("i", { class: showPets ? "fa-solid fa-paw" : "fa-solid fa-paw fa-paw--off" })]);
  ...
  function drawNodes(positions) {
    positions.forEach((pos, id) => {
      ...
      // an archive map, not just a relationship diagram.
