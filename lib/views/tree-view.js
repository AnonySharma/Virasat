...
(function (global) {
  ...
  let focusId = readStr("virasat.focusId", null) || null;
  ...
  //   4. the current inspector selection, then simply the first person
  ...
  function setTreeMode(mode) {
    ...
    if (treeMode === "focus") {
      ...
      writeStr("virasat.focusId", focusId);
    }
  ...
  function render() {
    ...
    // In focus mode, keep focusId pointing at a living person. If the centre was
    // deleted (or was never set), re-resolve to a sensible default so the focus
    // view always has a valid ego. Persist any change so a reload is consistent.
    if (treeMode === "focus") {
      const resolved = resolveFocusId();
      if (resolved !== focusId) { focusId = resolved; writeStr("virasat.focusId", focusId); }
    }
