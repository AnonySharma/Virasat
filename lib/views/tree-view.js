...
(function (global) {
  ...
  let showStoryCount = readBool("virasat.showStoryCount", true);
  ...
  function setShowStoryCount(v) { showStoryCount = !!v; writeBool("virasat.showStoryCount", showStoryCount); render(); }
  ...
  function mount(root) {
    ...
    function renderTitle() {
      ...
      eyebrowEl.textContent = earliest != null
        ? "Established c. " + (Math.floor(earliest / 10) * 10)
        : "";
    }
    function openRename() {
      ...
      const body = UI.el("div", { class: "form-stack" }, [
        UI.el("p", { style: { margin: 0, color: "var(--text-3)", fontSize: "13px" } },
          "The full title shown in the header and printed on every exported file. Type whatever you'd like — a surname, a phrase, the family motto."),
        UI.field("Tree title", input)
      ]);
      const cancelBtn = UI.cancelBtn("Cancel");
      const saveBtn = UI.saveBtn("Save");
      const dlg = UI.openModal({ title: "Rename tree", body, footer: [cancelBtn, saveBtn] });
  ...
  // mutable cosmetic bits (name, dates, story-count chip, photo) on the
  ...
  function topoSignature(people, marriages, viewOpts) {
    ...
    people.slice().sort((a, b) => a.id.localeCompare(b.id)).forEach((p) => {
      parts.push(p.id
        ...
        + "|" + (p.isPet ? "1" : "0")
        + "|" + (p.deathDate ? "1" : "0")
        // Story-count presence (0 vs >0) drives whether the density chip
        // exists in the DOM — bust the cache when it crosses that boundary.
        + "|" + ((p.stories || []).length ? "s" : "_")
  ...
  function updateSubtitle(people) {
    ...
    const memoryStr = memoryCount === 0 ? null
      : memoryCount === 1 ? "1 memory"
      : memoryCount + " memories";
    rootEl.__subEl.textContent = [memberStr, genStr, memoryStr].filter(Boolean).join(" · ");
  }
  ...
  function softUpdateNodes(people) {
    ...
    nodesG.querySelectorAll(".t-node").forEach((g) => {
      ...
      if (nameEl) {
        ...
        // Tree shows only the first word; full name lives in the inspector
        // and the SVG <title> tooltip. Keep parity with the initial render
        // path (line ~965) so a soft update doesn't swap first→full and
        // suddenly fill the canvas with long names after import.
        const firstName = String(displayName).split(/\s+/)[0] || displayName;
        const truncated = truncate(firstName, 14);
      ...
      // Density chip — text only; the chip's existence is structural
      // (added on full render based on storyCount > 0). For soft updates
      // we just refresh the number.
      const densityNum = g.querySelector(".t-node-density-num");
      if (densityNum) {
        const next = String((p.stories || []).length);
  ...
  function render() {
    ...
    const people = showPets ? allPeople : allPeople.filter((p) => !p.isPet);
    ...
    const sig = topoSignature(people, state.marriages || {},
      ...
      // showStoryCount + showDates add/remove SVG children per node.
      [showPets ? 1 : 0, showStoryCount ? 1 : 0, showDates ? 1 : 0].join(","));
    const sigStable = sig === lastTopoSig && nodesG.childElementCount > 0;
    lastTopoSig = sig;

    if (sigStable) {
      // Topology hasn't changed — patch cosmetic bits in place. Header /
      // density chip / story-count toggle / dates toggle still need a
      // refresh, but no layout, no DOM rebuild, no edges.
  ...
  function drawEdges(positions) {
    ...
    positions.forEach((pos, id) => {
      ...
      person.spouses.forEach((sid) => {
        ...
        // Wider invisible hit target so the knot is easy to click.
    ...
    groups.forEach(({ parents: pp, children: cc }) => {
      ...
      if (orderedHumans.length) {
        ...
        edgesG.appendChild(svgEl_("path", { class: "t-edge", d: trunk, "data-edge-ids": groupIds }));
        ...
        // Shared horizontal rail. Clip the extreme ends to where the
        // leftmost / rightmost children's risers actually start curving;
        // otherwise the straight rail draws past the curve's bend point
        // and you see a visible horizontal stub poking out of the corner.
        // The trunk side (anchorX) gets a hard T-junction with the rail —
        // no curve there — so anchorX itself is never clipped.
        const childMinX = Math.min(...humanTops.map((c) => c.x));
        const childMaxX = Math.max(...humanTops.map((c) => c.x));
        const railLeft = childMinX < anchorX
          ? childMinX + riserRadius(childMinX)
          : anchorX;
        const railRight = childMaxX > anchorX
          ? childMaxX - riserRadius(childMaxX)
          : anchorX;
        if (railLeft < railRight) {
          const rail = `M ${railLeft} ${railY} L ${railRight} ${railY}`;
          edgesG.appendChild(svgEl_("path", { class: "t-edge", d: rail, "data-edge-ids": groupIds }));
        }
  ...
  // ===== Pet tethers =====
  // Pets are bonded to their owners via dashed gold curves rather than the
  // formal parent / child edges. The curve reads as "this beagle belongs
  // here" without claiming biological lineage.
  function drawPetTethers(positions) {
    positions.forEach((pos, id) => {
      const pet = pos.person;
      if (!pet || !pet.isPet) return;
      const owners = (pet.petOwners || []).filter((oid) => positions.has(oid));
      if (!owners.length) return;
      // Tether to the first known owner only — multiple pets per owner is
      // common but multiple tethers from one pet would clutter the canvas.
      const ownerPos = positions.get(owners[0]);
      if (!ownerPos) return;
      const px = pos.x + NODE_W / 2;
      const py = pos.y + PHOTO_CY;
      const ox = ownerPos.x + NODE_W / 2;
      const oy = ownerPos.y + PHOTO_CY;
      // Cubic Bezier with vertical handles so the curve sweeps gently between
      // the two photos rather than running through whatever's in between.
      const dx = ox - px;
      const dy = oy - py;
      const c1x = px + dx * 0.15;
      const c1y = py + dy * 0.6;
      const c2x = ox - dx * 0.15;
      const c2y = oy - dy * 0.6;
      edgesG.appendChild(svgEl_("path", {
        class: "t-pet-tether",
        d: `M ${px} ${py} C ${c1x} ${c1y}, ${c2x} ${c2y}, ${ox} ${oy}`,
        "data-pet-id": pet.id,
        "data-owner-id": owners[0]
      }));
    });
  }
  ...
  function drawNodes(positions) {
    positions.forEach((pos, id) => {
      ...
      // Story-density badge — top-left of the photo ring. Surfaces how
      // much soul (stories) is captured per person so the tree reads as
      // an archive map, not just a relationship diagram. Toggled in the
      // View Options popover.
      const storyCount = (p.stories || []).length;
      if (showStoryCount && storyCount > 0) {
  ...
  function showMarriageModal(a, b) {
      ...
      if (!has) {
        bodyHost.appendChild(UI.el("div", {
          ...
          UI.el("p", { style: { margin: 0, fontSize: "13.5px" } },
            "No marriage details recorded yet."),
          UI.el("p", { style: { margin: "4px 0 0", fontSize: "12px" } },
            "Tap Edit to add the date, place, photo, or story.")
        ]));
      } else {
        ...
        if (existing.date)  dl.appendChild(row("Date",  existing.date));
        if (existing.place) dl.appendChild(row("Place", existing.place));
    ...
    function renderEditMode() {
      ...
      }, [UI.el("i", { class: "fa-solid fa-image" }), UI.el("span", null, "Add photo")]);
      ...
      bodyHost.appendChild(partnersRow());
      bodyHost.appendChild(UI.field("Marriage date", dateEl));
      bodyHost.appendChild(UI.field("Place", placeInput));
      bodyHost.appendChild(UI.field("Photo", photoRow));
      bodyHost.appendChild(UI.field("Story / memories", storyInput, "How you met, the wedding day, anything you want to remember."));
      ...
      }, [UI.el("i", { class: "fa-solid fa-floppy-disk" }), UI.el("span", null, "Save")]);
  ...
  function showNodeMenu(person, pos, ev) {
    ...
    const menu = UI.el("div", { class: "tree-node-menu", role: "menu", style: { left: left + "px", top: top + "px", transform: "translateX(-50%)" } }, [
      ...
      // "Focus this lineage" — sticks the current person as the lineage
      // focus root and closes the inspector. The dim/highlight effect
      // persists across panel-close, banner shows "Viewing as: X · Reset".
      // If the user taps the same item on a person already focused, treat
      // it as Reset for symmetry.
      menuItem(
        lineageFocusId === person.id
          ? (I18n.t("tree.resetView") || "Reset view")
          : (I18n.t("tree.focusLineage") || "Focus this lineage"),
        "fa-solid fa-route",
        null,
        () => {
          dismissMenu();
          if (lineageFocusId === person.id) {
            lineageFocusId = null;
          } else {
            lineageFocusId = person.id;
            // Close the inspector so the dimmed tree is visible. Mobile
            // users couldn't see the highlight at all otherwise — the
            // inspector covers the whole screen at narrow widths.
  ...
  function openViewOptions(anchorBtn) {
    ...
    pop.appendChild(UI.el("div", { class: "tree-view-options__head" }, "Tree options"));
    pop.appendChild(row("Show pets",        "fa-solid fa-paw",         showPets,        setShowPets));
    pop.appendChild(row("Show story count", "fa-solid fa-book-open",   showStoryCount,  setShowStoryCount));
    pop.appendChild(row("Show dates",       "fa-regular fa-calendar",  showDates,       setShowDates));
  ...
  // the visual root of a subtree, then fades everything else. We deliberately
  // do NOT walk ancestors here — when you focus on a person you usually want
  // them at the top of the highlight, with their descendants below. Ancestors
  // and siblings stay rendered (faded) so context isn't lost, but no bright
  // line ever appears *above* the focus root.
  ...
  function applyHighlightClasses() {
    ...
    const lineage = focusRoot ? lineageOf(focusRoot) : null;
    ...
    if (edgesG) {
      ...
      allEdges.forEach((edge) => {
        ...
        const allIn = lineage && ids.length > 0 && ids.every((id) => lineage.has(id));
  ...
  function ensureBanner() {
    ...
    const resetBtn = UI.el("button", {
      ...
      UI.el("span", null, I18n.t("tree.resetView") || "Reset view")
    ]);
