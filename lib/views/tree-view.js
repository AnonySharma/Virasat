...
(function (global) {
  ...
  function drawEdges(positions) {
    ...
    groups.forEach(({ parents: pp, children: cc }) => {
      ...
      // If every child in this group is a pet, dash the whole connector
      // (trunk + rail + risers) so the visual reads consistently as
      // "bonded, not biological". A mixed group keeps the solid trunk/rail
      // and only dashes the per-pet risers.
      const allPets = ordered.every((cpos) => cpos.person.isPet);
      const groupCls = "t-edge" + (allPets ? " t-edge--pet" : "");

      // Trunk: from parent anchor down to the rail
      const trunk = `M ${anchorX} ${anchorY} L ${anchorX} ${railY}`;
      edgesG.appendChild(svgEl_("path", { class: groupCls, d: trunk, "data-edge-ids": groupIds }));
      ...
      edgesG.appendChild(svgEl_("path", { class: groupCls, d: rail, "data-edge-ids": groupIds }));

      // Risers — one per child, each with a small rounded corner where the
      // riser meets the rail. Pets get .t-edge--pet (dashed gold) so they
      // read as "bonded" rather than biological.
      ordered.forEach((cpos, i) => {
        const c = childTops[i];
        const riserIds = parentIds.concat([cpos.person.id]).join(",");
        const isPet = !!cpos.person.isPet;
        const cls = "t-edge" + (isPet ? " t-edge--pet" : "");
