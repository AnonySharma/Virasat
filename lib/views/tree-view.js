...
(function (global) {
  ...
  function resetComparePathState() {
    ...
    pathIds = pathEdgePairs = pathSpousePairs = null;
    pathLen = 0;
  }
  ...
  // "Viewing as" state (same position, same "· Reset" affordance). Content
  ...
  function updateCompareBanner() {
    ...
    if (!pathAId) {
      ...
      // Direct hop → show the relation word ("wife", "father"); longer chains →
      // a hop count ("3 steps apart"). relationLabel is undefined for non-
      // adjacent pairs, so only trust it at length 2.
      let detail = "";
      if (pathLen === 2 && FamilyStore.relationLabel) {
        detail = FamilyStore.relationLabel(pathAId, pathBId);
      }
