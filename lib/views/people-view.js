 ...
 * mount() builds the static scaffold (header, search, facet bar, grid host) once.
 * render() refreshes the grid based on current FamilyStore state, the search
 * filter, the alive/deceased filter, and the place/occupation/decade facets.
 */
(function (global) {
  "use strict";
  ...
  // Faceted search — narrow the list by birth place, occupation, or birth
  // decade, on top of the free-text query and the global Living/Deceased
  // filter (all dimensions combine with AND). "" = "any". Place/occupation
  // hold a lower-cased match key (language-independent — we index BOTH scripts
  // so a facet chosen in English still matches after switching to Hindi);
  // decade holds the decade-start as a string, e.g. "1980".
  let facetPlace = "";
  let facetOccupation = "";
  let facetDecade = "";
  let facetBar = null, placeSel = null, occSel = null, decadeSel = null, clearFacetsBtn = null;
  let lastFacetSig = "";          // guards facet-option rebuilds (see syncFacetOptions)
  let facetBarHasContent = false; // any facet dimension has ≥1 value → show the bar
  ...
  // typed or set programmatically (setSearch / setMissingFilter).
  function syncClearBtn() { if (clearBtn) clearBtn.hidden = !searchTerm; }

  function setSearch(s) { searchTerm = s || ""; missingFilter = null; if (searchInput) searchInput.value = searchTerm; syncClearBtn(); render(); }
  ...
  // Clear ALL filter dimensions back to defaults. Called on tree switch so
  // Tree B doesn't inherit Tree A's leftover search/filter (which would hide
  // most of the new tree with no visible explanation). Also drops the card
  // cache, whose entries are keyed to the previous tree's people. Does NOT
  // render — the switch path re-renders once via the store change.
  function reset() {
    searchTerm = "";
    filterMode = "all";
    missingFilter = null;
    if (searchInput) searchInput.value = "";
    clearFacets(true);
    lastFacetSig = "";   // force a rebuild against the new tree's facet universe
    cardCache.clear();
    syncClearBtn();
  }
  ...
  // ===== Faceted search (place / occupation / decade) =====
  // A facet's canonical key is language-independent: prefer the base field,
  // fall back to the Hindi-only value, lower-cased. This keeps a facet chosen
  // in English matching the same person after a switch to Hindi (their base
  // `birthPlace` is unchanged), while a place recorded ONLY in Hindi still
  // gets its own facet entry rather than vanishing.
  function facetKey(p, key) {
    const base = (p[key] || "").trim();
    const hi = (p[key + "_hi"] || "").trim();
    return (base || hi).toLowerCase();
  }
  function facetsActive() { return !!(facetPlace || facetOccupation || facetDecade); }
  function matchesFacets(p) {
    if (facetPlace && facetKey(p, "birthPlace") !== facetPlace) return false;
    if (facetOccupation && facetKey(p, "occupation") !== facetOccupation) return false;
    if (facetDecade) {
      const y = FamilyStore.getYear(p.birthDate);
      if (y == null || String(Math.floor(y / 10) * 10) !== facetDecade) return false;
    }
    return true;
  }

  // Walk ALL people (not the filtered set — narrowing by name mustn't empty the
  // place dropdown) to build the facet option universe. Places/occupations are
  // deduped by canonical key with a display-language label; decades are the
  // distinct birth-year decades on file.
  function computeFacetIndex(people) {
    const placeMap = new Map();   // key → display label
    ...
    people.forEach((p) => {
      ...
      const y = FamilyStore.getYear(p.birthDate);
      if (y != null) decadeSet.add(Math.floor(y / 10) * 10);
    });
    const byLabel = (a, b) => a.label.localeCompare(b.label);
    return {
      places: Array.from(placeMap, ([value, label]) => ({ value, label })).sort(byLabel),
      occupations: Array.from(occMap, ([value, label]) => ({ value, label })).sort(byLabel),
      decades: Array.from(decadeSet).sort((a, b) => a - b).map((d) => ({ value: String(d), label: I18n.t("people.facetDecade", { d }) }))
    };
  }

  // Build the facet bar once (mount). Three calm HeritageSelect popovers +
  // a "clear" button. Options are populated lazily by syncFacetOptions.
  function buildFacetBar() {
    if (!window.HeritageSelect) return el("div", { class: "people-facets", hidden: true });
    const mk = (ph, onCh) => HeritageSelect.create({ options: [{ value: "", label: ph }], value: "", placeholder: ph, onChange: onCh });
    placeSel = mk(I18n.t("people.facetAnyPlace"), (v) => { facetPlace = v; onFacetPick(); });
    occSel = mk(I18n.t("people.facetAnyOccupation"), (v) => { facetOccupation = v; onFacetPick(); });
    decadeSel = mk(I18n.t("people.facetAnyDecade"), (v) => { facetDecade = v; onFacetPick(); });
    clearFacetsBtn = el("button", {
      class: "btn btn--sm btn--ghost people-facets__clear", type: "button", hidden: true,
      onclick: () => clearFacets(false)
    }, [
      el("i", { class: "fa-solid fa-filter-circle-xmark", "aria-hidden": "true" }),
      // data-i18n so the global applyToDOM() on a language switch refreshes this
      // static label (the facet selects rebuild via syncFacetOptions' lang sig).
      el("span", { "data-i18n": "people.facetClear" }, I18n.t("people.facetClear"))
    ]);
    const wrap = (icon, sel) => el("div", { class: "people-facet" }, [
      el("i", { class: icon + " people-facet__icon", "aria-hidden": "true" }),
      sel.el
    ]);
    return el("div", { class: "people-facets", hidden: true }, [
      wrap("fa-solid fa-location-dot", placeSel),
      wrap("fa-solid fa-briefcase", occSel),
      wrap("fa-solid fa-calendar-day", decadeSel),
      clearFacetsBtn
    ]);
  }

  // A facet pick is a user filter action, like typing a search — it also drops
  // any active "needs attention" missing-field mode (that path ignores facets).
  function onFacetPick() { missingFilter = null; render(); }

  function clearFacets(silent) {
    facetPlace = ""; facetOccupation = ""; facetDecade = "";
    if (placeSel) placeSel.setValue("");
    if (occSel) occSel.setValue("");
    if (decadeSel) decadeSel.setValue("");
    if (clearFacetsBtn) clearFacetsBtn.hidden = true;
    if (!silent) { missingFilter = null; render(); }
  }

  // Refresh the facet dropdowns from current data. Rebuilds options only when
  // the facetable universe (or language) actually changes — keyed off a
  // signature — so a keystroke-driven re-render doesn't stomp an open menu, and
  // a facet whose value has since disappeared (person edited/deleted) is reset.
  function syncFacetOptions(all) {
    if (!placeSel) return;
    const idx = computeFacetIndex(all);
    facetBarHasContent = !!(idx.places.length || idx.occupations.length || idx.decades.length);
    const sig = I18n.getLang() + "||"
      + idx.places.map((o) => o.value).join(",") + "||"
      + idx.occupations.map((o) => o.value).join(",") + "||"
      + idx.decades.map((o) => o.value).join(",");
    if (sig === lastFacetSig) return;
    lastFacetSig = sig;

    if (facetPlace && !idx.places.some((o) => o.value === facetPlace)) facetPlace = "";
    if (facetOccupation && !idx.occupations.some((o) => o.value === facetOccupation)) facetOccupation = "";
    if (facetDecade && !idx.decades.some((o) => o.value === facetDecade)) facetDecade = "";

    placeSel.setOptions([{ value: "", label: I18n.t("people.facetAnyPlace") }].concat(idx.places));
    occSel.setOptions([{ value: "", label: I18n.t("people.facetAnyOccupation") }].concat(idx.occupations));
    decadeSel.setOptions([{ value: "", label: I18n.t("people.facetAnyDecade") }].concat(idx.decades));
    placeSel.setValue(facetPlace);
    occSel.setValue(facetOccupation);
    decadeSel.setValue(facetDecade);
  }

  // Show/hide the whole bar (hidden when there's nothing to facet, or in the
  // missing-field / empty-tree modes) and keep the clear button in step.
  function syncFacetBar(visible) {
    if (!facetBar) return;
    facetBar.hidden = !visible;
    if (clearFacetsBtn) clearFacetsBtn.hidden = !facetsActive();
  }
  ...
  function renderGrid() {
    ...
    // Refresh the facet dropdowns against current data (rebuilds only when the
    // facetable universe or language changed — cheap on a keystroke).
    syncFacetOptions(all);
    ...
    // Maintenance deep-link: only show people missing a given field. Facets
    // don't apply here — this is a separate "needs attention" path — so hide
    // the bar to avoid implying it filters the banner list.
    ...
    // The facet bar rides above the grid whenever there's something to facet,
    // in both the search and default views (facets AND the text query compose).
    syncFacetBar(facetBarHasContent);
    ...
    if (q && FamilyStore.searchStories) {
      let storyHits = (function () {
        try { return FamilyStore.searchStories(q) || []; } catch (_) { return []; }
      })();
      // Active facets narrow story hits too, by their author — a story whose
      // author is filtered out shouldn't surface.
      if (facetsActive()) storyHits = storyHits.filter((hit) => hit.person && matchesFacets(hit.person));
      const personHits = all.filter((p) => {
        ...
        // …and the active facets (place / occupation / decade), same rule.
        ...
        // Facets widen the text search's reach: matching a facet-eligible field
        // (place / occupation) should count as a hit even without a facet pick,
        // per the feature — "search by place, occupation… not just name/story".
        if (p.birthPlace && p.birthPlace.toLowerCase().includes(q)) return true;
        if (p.birthPlace_hi && p.birthPlace_hi.toLowerCase().includes(q)) return true;
        if (p.occupation && p.occupation.toLowerCase().includes(q)) return true;
        if (p.occupation_hi && p.occupation_hi.toLowerCase().includes(q)) return true;
        return false;
      });
    ...
    // No query → the regular all-people grid. Facets HARD-filter (hide
    // non-matching, like a search); the global Living/Deceased filter stays
    // SOFT (dims within whatever facets left).
    gridHost.style.display = "";
    const base = facetsActive() ? all.filter(matchesFacets) : all;

    // A facet combination that matches nobody gets the same empty state a
    // fruitless search does — otherwise the grid just goes blank with no
    // explanation of why.
    if (facetsActive() && base.length === 0) {
      setResultCount(0);
      gridHost.style.display = "block";
      gridHost.appendChild(emptyStateNoMatches());
      return;
    ...
    // Report a count when the set is narrowed. A Living/Deceased filter counts
    // the non-dimmed cards (matchCount, computed over the facet-narrowed base) —
    // consistent whether or not facets are also active. Facets alone report the
    // whole facet set. With neither active the header keeps the full total.
    if (filterMode !== "all") setResultCount(matchCount);
    else if (facetsActive()) setResultCount(base.length);
  }
  ...
  global.PeopleView = { mount, render, openForm, setSearch, setFilter, setMissingFilter, reset };
})(window);
