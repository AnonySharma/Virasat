...
(function (global) {
  ...
  function render() {
    ...
    if (missingFilter && FamilyStore.peopleMissing) {
      ...
      if (!missing.length) {
        gridHost.appendChild(el("div", { class: "empty" }, [
          ...
          el("div", { class: "empty__title" }, "All set"),
          el("div", { class: "empty__text" }, "Every relative has this field filled in.")
        ]));
        return;
      }
      sortPeople(missing).forEach((p) => gridHost.appendChild(personCard(p)));
      return;
    }
    ...
    if (q && FamilyStore.searchStories) {
      ...
      personHits.forEach((p) => gridHost.appendChild(personCard(p)));
      return;
    }
  ...
  function emptyStateNoMatches() {
    return el("div", { class: "empty" }, [
      ...
      el("div", { class: "empty__text" }, "Try a different name, story, or tag.")
    ]);
  }
  ...
  function openForm(id, seed) {
    ...
    const removeBtn = el("button", {
      ...
      el("i", { class: "fa-regular fa-trash-can", "aria-hidden": "true" }),
    ...
    const cancelBtn = el("button", { class: "btn btn--ghost", type: "button" }, [
      ...
      el("span", null, I18n.t("actions.cancel"))
    ]);
    const saveBtn = el("button", { class: "btn btn--primary", type: "button" }, [
      el("i", { class: "fa-solid fa-floppy-disk", "aria-hidden": "true" }),
      el("span", null, I18n.t("actions.save"))
    ]);
