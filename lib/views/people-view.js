// @ts-check
/**
 * People view — list, search, add, edit, and delete family members.
 *
 * Exposes window.PeopleView =
 *   { mount(rootEl), render, openForm, setSearch, setFilter, setMissingFilter, reset }.
 * mount() builds the static scaffold (header, search, grid host) once.
 * render() refreshes the grid from current FamilyStore state + the free-text
 * search. Narrowing by status / gender / place / occupation / era / needs-
 * attention lives in the rail's shared faceted Filter panel (window.Filter);
 * this view consults Filter.matches() and HIDES non-matches.
 */
(function (global) {
  "use strict";

  const { el, clear, avatar, toast, openModal, confirm, field } = UI;

  let root = null;
  let gridHost = null;
  let subtitleEl = null;
  let searchInput = null;
  let clearBtn = null;
  let searchTerm = "";
  // Narrowing by status / gender / place / occupation / era / needs-attention
  // now lives in the rail's faceted Filter panel (window.Filter), shared by all
  // three views. The People view consults Filter.matches() and HIDES
  // non-matches (Tree & Timeline dim them). Free-text search still lives here.
  // Guards the one-time Inspector.onSelect subscription (see mount) so a
  // remount doesn't stack duplicate listeners.
  let selectionBound = false;
  // Cache of rendered person-cards keyed by person id. Lets render() reorder
  // / show / hide existing DOM rather than rebuilding the grid on every
  // keystroke. Cleared when the underlying person changes (data signature
  // mismatch) or when filter mode rotates.
  const cardCache = new Map();
  // Per-person sig of the bits the card displays; if it matches, we reuse
  // the cached node verbatim.
  function cardSig(p) {
    // Lang is part of the signature: the card's age chip, date range, and which
    // name is the subtitle are all language-dependent, so a switch must bust the
    // cache or the card renders in the previous language.
    return I18n.getLang() + "|" + p.id + "|" + (p.name || "") + "|" + (p.name_hi || "")
      + "|" + (p.birthDate || "") + "|" + (p.deathDate || "")
      + "|" + (p.birthPlace || "") + "|" + (p.photoId || (p.photo ? "p" : "_"))
      + "|" + (p.updatedAt || "");
  }
  // Search debounce — coalesce keystrokes so a 100-person grid doesn't
  // re-filter on every character.
  let searchTimer = null;
  const SEARCH_DEBOUNCE_MS = 120;

  // Search scope — which fields the free-text query reaches. All on by default;
  // the user can narrow it from the popover on the search bar (e.g. names only,
  // to stop stories flooding the results). Persisted per session like the
  // faceted filter. `names` always leads the result order regardless of scope.
  const SCOPE_KEYS = ["names", "occupation", "location", "stories"];
  const SCOPE_DEFAULT = { names: true, occupation: true, location: true, stories: true };
  const SCOPE_STORAGE_KEY = "virasat.searchScope";
  let searchScope = (function () {
    try {
      const raw = sessionStorage.getItem(SCOPE_STORAGE_KEY);
      if (!raw) return { ...SCOPE_DEFAULT };
      const o = JSON.parse(raw);
      const next = { ...SCOPE_DEFAULT };
      SCOPE_KEYS.forEach((k) => { if (o && typeof o[k] === "boolean") next[k] = o[k]; });
      return next;
    } catch (_) { return { ...SCOPE_DEFAULT }; }
  })();
  function saveScope() { try { sessionStorage.setItem(SCOPE_STORAGE_KEY, JSON.stringify(searchScope)); } catch (_) {} }
  // Never let every facet be off — an empty scope would make search silently
  // match nothing. Fall back to names (the primary signal) if the user clears
  // the last box.
  function scopeOrNames() {
    const any = SCOPE_KEYS.some((k) => searchScope[k]);
    return any ? searchScope : { ...searchScope, names: true };
  }
  // Popover elements (built lazily, one instance reused by both search bars).
  let scopePopover = null;
  let scopeToggleBtns = [];   // the little "scope" buttons inside each bar

  function mount(rootEl) {
    root = rootEl;
    clear(root);

    // Clear (×) button — hidden until there's text. Native search inputs only
    // render a cancel affordance on some WebKit builds (and we suppress that
    // in CSS to avoid a double ×), so this is the one consistent clear.
    clearBtn = el("button", {
      class: "searchbar__clear", type: "button", hidden: true,
      "aria-label": I18n.t("actions.clearSearch"),
      "data-i18n-aria-label": "actions.clearSearch",
      onclick: () => {
        if (searchTimer) { clearTimeout(searchTimer); searchTimer = null; }
        searchTerm = "";
        if (searchInput) searchInput.value = "";
        syncClearBtn();
        render();
        if (searchInput) searchInput.focus();
      }
    }, [el("i", { class: "fa-solid fa-xmark", "aria-hidden": "true" })]);

    searchInput = el("input", {
      type: "search",
      placeholder: I18n.t("people.searchPlaceholder"),
      "data-i18n-placeholder": "people.searchPlaceholder",
      "aria-label": I18n.t("people.searchAria"),
      "data-i18n-aria-label": "people.searchAria",
      oninput: (e) => {
        searchTerm = e.target.value || "";
        syncClearBtn();
        if (searchTimer) clearTimeout(searchTimer);
        searchTimer = setTimeout(render, SEARCH_DEBOUNCE_MS);
      }
    });
    const search = el("div", { class: "searchbar" }, [
      el("i", { class: "fa-solid fa-magnifying-glass searchbar__icon", "aria-hidden": "true" }),
      searchInput,
      clearBtn,
      scopeToggle()
    ]);

    const addBtn = el("button", {
      class: "btn btn--primary js-edit-only",
      type: "button",
      onclick: () => openForm(null)
    }, [
      el("i", { class: "fa-solid fa-user-plus", "aria-hidden": "true" }),
      el("span", { "data-i18n": "actions.add" }, I18n.t("actions.add"))
    ]);

    const head = el("div", { class: "view-head" }, [
      el("div", { class: "view-head__title-wrap" }, [
        el("h2", { class: "view-head__title", "data-i18n": "people.title" }, I18n.t("people.title")),
        (subtitleEl = el("span", { class: "view-head__sub" }, ""))
      ]),
      el("div", { class: "view-head__actions" }, [search, addBtn])
    ]);

    gridHost = el("div", { class: "people-grid" });

    root.appendChild(head);
    root.appendChild(gridHost);

    render();

    // Keep the "currently open in Inspector" card indicator in step without a
    // full grid rebuild — same pattern the tree uses (Inspector.onSelect →
    // applyHighlightClasses). Guarded so a minimal build without the Inspector
    // still mounts. Subscribe once per mount.
    if (!selectionBound && window.Inspector && Inspector.onSelect) {
      Inspector.onSelect(() => applySelection());
      selectionBound = true;
    }
  }

  // Keep the clear (×) affordance in step with whatever's in the box, whether
  // typed or set programmatically (setSearch).
  function syncClearBtn() { if (clearBtn) clearBtn.hidden = !searchTerm; }

  // ===== Search-scope popover =====
  // A little sliders button on the search bar opens a checklist of which fields
  // the query searches. Shared: both the in-view bar and the header bar mount
  // their own toggle button, but they open one popover and drive one state.
  const SCOPE_ICON = {
    names: "fa-solid fa-user",
    occupation: "fa-solid fa-briefcase",
    location: "fa-solid fa-location-dot",
    stories: "fa-solid fa-book-open"
  };
  function scopeActiveCount() { return SCOPE_KEYS.filter((k) => searchScope[k]).length; }
  // The trigger button that lives inside a search bar. Records itself so its
  // active-dot can be repainted when the scope changes from elsewhere.
  function scopeToggle() {
    const dot = el("span", { class: "searchbar__scope-dot", hidden: scopeActiveCount() === SCOPE_KEYS.length });
    const btn = el("button", {
      class: "searchbar__scope", type: "button",
      "aria-label": I18n.t("people.searchScopeAria"),
      "data-i18n-aria-label": "people.searchScopeAria",
      "aria-haspopup": "true", "aria-expanded": "false",
      title: I18n.t("people.searchScopeAria"),
      "data-i18n-title": "people.searchScopeAria",
      onclick: (e) => { e.preventDefault(); e.stopPropagation(); toggleScopePopover(btn); }
    }, [el("i", { class: "fa-solid fa-sliders", "aria-hidden": "true" }), dot]);
    btn._dot = dot;
    scopeToggleBtns.push(btn);
    return btn;
  }
  function paintScopeDots() {
    const all = scopeActiveCount() === SCOPE_KEYS.length;
    // Drop stale buttons (from a previous mount) so the array can't grow.
    scopeToggleBtns = scopeToggleBtns.filter((b) => b.isConnected);
    scopeToggleBtns.forEach((b) => { if (b._dot) b._dot.hidden = all; });
  }
  function closeScopePopover() {
    if (!scopePopover) return;
    document.removeEventListener("click", onDocClickScope, true);
    document.removeEventListener("keydown", onKeyScope, true);
    scopeToggleBtns.forEach((b) => b.setAttribute("aria-expanded", "false"));
    scopePopover.remove();
    scopePopover = null;
  }
  function onDocClickScope(e) {
    if (scopePopover && !scopePopover.contains(e.target) && !e.target.closest(".searchbar__scope")) closeScopePopover();
  }
  function onKeyScope(e) { if (e.key === "Escape") { e.stopPropagation(); closeScopePopover(); } }
  function toggleScopePopover(anchorBtn) {
    if (scopePopover) { closeScopePopover(); return; }
    const rows = SCOPE_KEYS.map((k) => {
      const cb = el("input", {
        type: "checkbox", class: "scope-menu__cb", checked: !!searchScope[k],
        onchange: (e) => {
          searchScope[k] = !!e.target.checked;
          saveScope();
          paintScopeDots();
          render();
        }
      });
      return el("label", { class: "scope-menu__item" }, [
        cb,
        el("i", { class: SCOPE_ICON[k] + " scope-menu__icon", "aria-hidden": "true" }),
        el("span", null, I18n.t("people.searchScope_" + k))
      ]);
    });
    scopePopover = el("div", { class: "kebab-menu scope-menu", role: "group", "aria-label": I18n.t("people.searchScopeAria") }, [
      el("div", { class: "scope-menu__head" }, I18n.t("people.searchScopeTitle")),
      ...rows
    ]);
    document.body.appendChild(scopePopover);
    // Anchor under the button, right-aligned, clamped to the viewport.
    const r = anchorBtn.getBoundingClientRect();
    scopePopover.style.top = (r.bottom + 6) + "px";
    const right = Math.max(8, window.innerWidth - r.right);
    scopePopover.style.right = right + "px";
    anchorBtn.setAttribute("aria-expanded", "true");
    // Defer the outside-click listener so this very click doesn't close it.
    setTimeout(() => {
      document.addEventListener("click", onDocClickScope, true);
      document.addEventListener("keydown", onKeyScope, true);
    }, 0);
  }
  // The header search bar is static HTML owned by app.js, so it can't build its
  // own scope toggle the way the in-view bar does. app.js calls this once at
  // boot to graft the same toggle (driving the same shared state + popover) into
  // it. Idempotent — a second call is a no-op.
  function mountHeaderScope(barEl) {
    if (!barEl || barEl.querySelector(".searchbar__scope")) return;
    barEl.appendChild(scopeToggle());
  }

  function setSearch(s) { searchTerm = s || ""; if (searchInput) searchInput.value = searchTerm; syncClearBtn(); render(); }
  // Called by the shared Filter (window.Filter) whenever any facet changes, and
  // by the store subscription. Takes no argument — the predicate lives in
  // Filter.matches(); this view just re-renders against it.
  function setFilter() { render(); }
  // "Needs attention" deep-link (rail): route through the shared Filter so the
  // panel reflects it, then ensure we're on People. Kept for back-compat with
  // any caller; the rail's own rows now drive Filter directly.
  function setMissingFilter(field) {
    searchTerm = "";
    if (searchInput) searchInput.value = "";
    syncClearBtn();
    if (window.Filter && Filter.set) Filter.set({ missing: field || "" });
    else render();
  }

  // Clear the view's own search on tree switch (the shared Filter is reset
  // separately, before hydrate, by the switch path). Drops the card cache,
  // whose entries are keyed to the previous tree's people. Does NOT render —
  // the switch path re-renders once via the store change.
  function reset() {
    searchTerm = "";
    if (searchInput) searchInput.value = "";
    cardCache.clear();
    syncClearBtn();
  }

  // The active filter predicate + whether anything is narrowing, read from the
  // shared rail panel. Safe if Filter isn't wired yet (minimal build / early
  // boot) — nothing is filtered.
  function filterActive() { return !!(window.Filter && Filter.isActive && Filter.isActive()); }
  function matchesFilter(p) { return (window.Filter && Filter.matches) ? Filter.matches(p) : true; }

  // Reflect how many cards a search / filter actually surfaced, instead of
  // leaving the header stuck on the full member total.
  function setResultCount(n) {
    subtitleEl.textContent = n === 0 ? I18n.t("people.resultNone")
      : n === 1 ? I18n.t("people.resultOne")
      : I18n.t("people.resultMany", { n });
  }

  // Reflect which card is the one currently open in the Inspector, mirroring
  // the tree's is-selected node (tree-view applyHighlightClasses). Scans the
  // live grid DOM so it's correct no matter which render() branch built it, and
  // is safe to call from the Inspector.onSelect subscription too.
  function applySelection() {
    if (!gridHost) return;
    const selectedId = window.Inspector && Inspector.getSelected ? Inspector.getSelected() : null;
    gridHost.querySelectorAll(".person-card[data-person-id]").forEach((c) => {
      c.classList.toggle("is-selected", !!selectedId && c.getAttribute("data-person-id") === selectedId);
    });
  }

  function render() {
    renderGrid();
    applySelection();
  }

  function renderGrid() {
    if (!root || !gridHost) return;
    const all = FamilyStore.getPeople();
    subtitleEl.textContent = all.length === 1 ? I18n.t("people.countOne") : I18n.t("people.countMany", { n: all.length });

    // Prune the card cache of entries whose person was deleted, so the
    // map doesn't grow unbounded over a long session.
    if (cardCache.size > all.length * 2) {
      const live = new Set(all.map((p) => p.id));
      cardCache.forEach((_, id) => { if (!live.has(id)) cardCache.delete(id); });
    }

    clear(gridHost);

    if (all.length === 0) {
      gridHost.style.display = "block";
      gridHost.appendChild(emptyStateInitial());
      return;
    }

    const q = searchTerm.trim().toLowerCase();
    const active = filterActive();
    // The rail's faceted Filter HIDES non-matching people in this view (Tree &
    // Timeline dim instead). Apply it first; free-text search composes on top.
    const base = active ? all.filter(matchesFilter) : all;

    // Scoped search: the query reaches only the fields the user left enabled in
    // the scope popover (names / occupation / location / stories). People whose
    // NAME matches always lead the results — that's the primary signal — then
    // people matched on other fields, then story hits. Falls through to the
    // plain grid when the query is empty.
    if (q) {
      const scope = scopeOrNames();
      const nameHit = (p) =>
        (p.name && p.name.toLowerCase().includes(q)) ||
        (p.name_hi && p.name_hi.toLowerCase().includes(q));
      const fieldHit = (p) =>
        (scope.occupation && (
          (p.occupation && p.occupation.toLowerCase().includes(q)) ||
          (p.occupation_hi && p.occupation_hi.toLowerCase().includes(q)))) ||
        (scope.location && (
          (p.birthPlace && p.birthPlace.toLowerCase().includes(q)) ||
          (p.birthPlace_hi && p.birthPlace_hi.toLowerCase().includes(q)))) ||
        // Notes ride along with the "names" (person-text) scope — they're
        // person prose, not a separate result type.
        (scope.names && (
          (p.notes && p.notes.toLowerCase().includes(q)) ||
          (p.notes_hi && p.notes_hi.toLowerCase().includes(q))));

      // Person hits, name-matches first (stable within each tier via sortPeople).
      const nameHits = scope.names ? sortPeople(base.filter(nameHit)) : [];
      const nameIds = new Set(nameHits.map((p) => p.id));
      const otherHits = sortPeople(base.filter((p) => !nameIds.has(p.id) && fieldHit(p)));

      // Story hits last — only when the stories scope is on.
      let storyHits = [];
      if (scope.stories && FamilyStore.searchStories) {
        try { storyHits = FamilyStore.searchStories(q) || []; } catch (_) { storyHits = []; }
        // The active filter narrows story hits too, by author — a story whose
        // author is filtered out shouldn't surface.
        if (active) storyHits = storyHits.filter((hit) => hit.person && matchesFilter(hit.person));
      }

      setResultCount(nameHits.length + otherHits.length + storyHits.length);

      if (!nameHits.length && !otherHits.length && !storyHits.length) {
        gridHost.style.display = "block";
        gridHost.appendChild(emptyStateNoMatches());
        return;
      }

      gridHost.style.display = "";
      // People first (name matches lead), then story cards.
      nameHits.forEach((p) => gridHost.appendChild(cachedPersonCard(p)));
      otherHits.forEach((p) => gridHost.appendChild(cachedPersonCard(p)));
      storyHits.forEach((hit) => gridHost.appendChild(storyResultCard(hit, q)));
      return;
    }

    // No query → the plain grid, already narrowed by the active filter.
    gridHost.style.display = "";

    // An active filter that matches nobody gets the same empty state a
    // fruitless search does — otherwise the grid just goes blank with no
    // explanation of why.
    if (active && base.length === 0) {
      setResultCount(0);
      gridHost.style.display = "block";
      gridHost.appendChild(emptyStateNoMatches());
      return;
    }

    sortPeople(base).forEach((p) => gridHost.appendChild(cachedPersonCard(p)));
    // Report the narrowed count when a filter is active; otherwise the header
    // keeps the full member total set above.
    if (active) setResultCount(base.length);
  }

  // Wrap matched substring of `text` with a <mark> element and return a
  // DocumentFragment. Case-insensitive, only the first match is highlighted
  // (we keep the snippet short anyway).
  function highlight(text, q) {
    const frag = document.createDocumentFragment();
    if (!text) return frag;
    if (!q) { frag.appendChild(document.createTextNode(String(text))); return frag; }
    const t = String(text);
    const lo = t.toLowerCase();
    const idx = lo.indexOf(q.toLowerCase());
    if (idx === -1) { frag.appendChild(document.createTextNode(t)); return frag; }
    if (idx > 0) frag.appendChild(document.createTextNode(t.slice(0, idx)));
    frag.appendChild(el("mark", { class: "search-mark" }, t.slice(idx, idx + q.length)));
    frag.appendChild(document.createTextNode(t.slice(idx + q.length)));
    return frag;
  }

  // Build a snippet around the first occurrence of `q` in `body`, capped at
  // ~180 chars total. Falls back to the start of the body if the query
  // doesn't actually appear there (story-search may have matched on title
  // or a tag, not the body).
  function snippetAround(body, q) {
    if (!body) return "";
    const t = String(body).replace(/\s+/g, " ").trim();
    if (!q) return t.slice(0, 180) + (t.length > 180 ? "…" : "");
    const idx = t.toLowerCase().indexOf(q.toLowerCase());
    if (idx === -1) return t.slice(0, 180) + (t.length > 180 ? "…" : "");
    const start = Math.max(0, idx - 60);
    const end = Math.min(t.length, idx + q.length + 120);
    return (start > 0 ? "…" : "") + t.slice(start, end) + (end < t.length ? "…" : "");
  }

  function storyResultCard(hit, q) {
    const p = hit.person;
    const s = hit.story;
    const displayName = FamilyStore.getField(p, "name") || p.name;
    const card = el("article", {
      class: "card story-result-card",
      tabindex: "0",
      role: "button",
      "aria-label": I18n.t("people.openStory", { title: s.title || I18n.t("people.untitledStory"), name: displayName }),
      onclick: () => openProfileAtStory(p.id, s.id),
      onkeydown: (ev) => {
        if (ev.key === "Enter" || ev.key === " ") {
          ev.preventDefault();
          openProfileAtStory(p.id, s.id);
        }
      }
    });

    // Top: avatar + author name + story title
    const head = el("div", { class: "story-result-card__head" }, [
      avatar(p, "sm"),
      el("div", { class: "story-result-card__author" }, [
        el("div", { class: "story-result-card__title" }, highlight(s.title || I18n.t("people.untitledStory"), q)),
        el("div", { class: "story-result-card__by" }, [
          el("i", { class: "fa-solid fa-feather", "aria-hidden": "true" }),
          el("span", null, displayName)
        ])
      ])
    ]);
    card.appendChild(head);

    // Snippet
    const snippet = snippetAround(s.body || "", q);
    if (snippet) {
      const body = el("p", { class: "story-result-card__snippet" });
      body.appendChild(highlight(snippet, q));
      card.appendChild(body);
    }

    // Tags
    if (s.tags && s.tags.length) {
      const tags = el("div", { class: "story-result-card__tags" },
        s.tags.map((t) => el("span", { class: "chip chip--gold" }, "#" + t))
      );
      card.appendChild(tags);
    }

    return card;
  }

  function openProfileAtStory(personId, storyId) {
    if (window.Inspector && Inspector.show) {
      Inspector.show(personId, { scrollToStoryId: storyId });
    } else {
      openProfile(personId);
    }
  }

  function sortPeople(people) {
    return people.slice().sort((a, b) => {
      // Living first
      const aliveA = FamilyStore.isAlive(a);
      const aliveB = FamilyStore.isAlive(b);
      if (aliveA !== aliveB) return aliveA ? -1 : 1;
      // Birth year ascending, nulls last
      const yA = FamilyStore.getYear(a.birthDate);
      const yB = FamilyStore.getYear(b.birthDate);
      if (yA == null && yB != null) return 1;
      if (yB == null && yA != null) return -1;
      if (yA != null && yB != null && yA !== yB) return yA - yB;
      // Name
      return a.name.localeCompare(b.name);
    });
  }

  // Cached version: returns the same DOM node across renders when the
  // visible bits (name / dates / place / photo / updatedAt) haven't
  // changed. Falls back to fresh build otherwise. Saves a name search
  // from rebuilding 14 DOM trees per keystroke.
  function cachedPersonCard(person) {
    const sig = cardSig(person);
    const hit = cardCache.get(person.id);
    if (hit && hit.sig === sig) return hit.node;
    const node = personCard(person);
    cardCache.set(person.id, { sig, node });
    return node;
  }

  function personCard(person) {
    const displayName = FamilyStore.getField(person, "name") || person.name;
    const displayPlace = FamilyStore.getField(person, "birthPlace") || person.birthPlace;
    const ageChip = buildAgeChip(person);
    const placeChip = displayPlace
      ? el("span", { class: "chip chip--muted" }, [
          el("i", { class: "fa-solid fa-location-dot", "aria-hidden": "true" }),
          el("span", null, displayPlace)
        ])
      : null;

    // .js-edit-only → hidden for viewers (body.is-viewer) alongside the guards
    // in openForm/deletePerson, so read-only users don't even see the controls.
    const editBtn = el("button", {
      class: "btn btn--icon btn--sm btn--ghost js-edit-only",
      type: "button",
      "aria-label": I18n.t("actions.edit") + " " + person.name,
      title: I18n.t("actions.edit"),
      onclick: (e) => { e.stopPropagation(); openForm(person.id); }
    }, [el("i", { class: "fa-solid fa-pen", "aria-hidden": "true" })]);

    const delBtn = el("button", {
      class: "btn btn--icon btn--sm btn--ghost js-edit-only",
      type: "button",
      "aria-label": I18n.t("actions.delete") + " " + person.name,
      title: I18n.t("actions.delete"),
      onclick: (e) => { e.stopPropagation(); deletePerson(person.id); }
    }, [el("i", { class: "fa-solid fa-trash-can", "aria-hidden": "true" })]);

    const card = el("div", {
      class: "card person-card",
      tabindex: "0",
      role: "button",
      "data-person-id": person.id,
      "aria-label": I18n.t("actions.openProfile") + ": " + person.name,
      onclick: () => openProfile(person.id),
      onkeydown: (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openProfile(person.id); } }
    }, [
      el("div", { class: "person-card__top" }, [
        avatar(person, "lg"),
        el("div", null, [
          el("div", { class: "person-card__name" }, [
            // Quiet memoriam glyph beside the name once a death date is set —
            // the same feather the inspector hero uses, kept small and gold.
            (FamilyStore.isDeceased && FamilyStore.isDeceased(person))
              ? el("i", { class: "fa-solid fa-feather person-card__memoriam", "aria-hidden": "true", title: I18n.t("profile.inMemoriam") })
              : null,
            displayName,
            // Show the alternate-script name as a small subtitle if both exist
            (I18n.getLang() === "hi" && person.name && person.name !== displayName)
              ? el("span", { class: "person-card__name-hi" }, person.name)
              : (I18n.getLang() !== "hi" && person.name_hi && person.name_hi !== displayName)
                ? el("span", { class: "person-card__name-hi", lang: "hi" }, person.name_hi)
                : null
          ]),
          el("div", { class: "person-card__dates" }, FamilyStore.formatDateRange(person))
        ])
      ]),
      el("div", { class: "person-card__meta" }, [ageChip, placeChip].filter(Boolean)),
      el("div", { class: "person-card__actions" }, [editBtn, delBtn])
    ]);

    return card;
  }

  function buildAgeChip(person) {
    const age = FamilyStore.calcAge(person);
    const alive = FamilyStore.isAlive(person);
    let label;
    if (age == null) {
      label = I18n.t(alive ? "people.living" : "people.deceased");
    } else if (alive) {
      label = I18n.t("people.ageLiving", { n: age });
    } else {
      label = I18n.t("people.ageLived", { n: age });
    }
    const cls = "chip person-card__age-chip " + (alive ? "chip--alive" : "chip--deceased");
    return el("span", { class: cls }, label);
  }

  function openProfile(id) {
    // Reveal on the canvas AND open the profile — same contract as the
    // Inspector "Open in tree" action (app.js activates the tree, pans/zooms to
    // the node, then Inspector.show runs). Falls back to a bare profile open
    // when the reveal seam or CustomEvent isn't available.
    try {
      window.dispatchEvent(new CustomEvent("virasat:reveal-in-tree", { detail: { id } }));
      return;
    } catch (_) {}
    if (window.Inspector && Inspector.show) Inspector.show(id);
    else openForm(id);
  }

  function emptyStateInitial() {
    return UI.emptyState({
      icon: "fa-solid fa-seedling",
      title: I18n.t("people.emptyTitle"),
      text: I18n.t("people.emptyText"),
      cta: {
        label: I18n.t("people.addFirst"),
        icon: "fa-solid fa-user-plus",
        onClick: () => openForm(null)
      }
    });
  }

  function emptyStateNoMatches() {
    return UI.emptyState({
      icon: "fa-solid fa-magnifying-glass",
      title: I18n.t("people.noMatchTitle"),
      text: I18n.t("people.noMatchText")
    });
  }

  // ===== Add/Edit form =====

  function openForm(id, seed) {
    // Hard gate: viewers must never reach the add/edit form. The buttons that
    // open it are hidden via .js-edit-only, but the tree node-menu, inspector,
    // and kebab all call openForm too — guard the entry point itself so a
    // viewer can't slip through and be shown a form that silently no-ops on
    // save (FamilyStore.add/updatePerson return null under read-only).
    if (window.FamilyStore && FamilyStore.isReadOnly && FamilyStore.isReadOnly()) {
      toast(I18n.t("people.readOnly"), "warning");
      return;
    }
    const isEdit = !!id;
    const existing = isEdit ? FamilyStore.getPerson(id) : null;
    if (isEdit && !existing) {
      toast(I18n.t("people.notFound"), "danger");
      return;
    }
    seed = seed || {};

    // Working copy of the person (only fields used by the form).
    const draft = {
      id: existing ? existing.id : null,
      name: existing ? existing.name : "",
      name_hi: existing ? existing.name_hi || "" : "",
      photoId: existing ? existing.photoId || null : null,
      photo: existing ? existing.photo : null, // base64 (legacy / freshly imported)
      photoCropAvatar: existing ? existing.photoCropAvatar || null : null,
      photoCropHero: existing ? existing.photoCropHero || null : null,
      // Additional photos beyond the avatar. Deep-copied so edits/removals in
      // the form don't mutate the live record until Save.
      gallery: existing && Array.isArray(existing.gallery)
        ? existing.gallery.map((g) => ({ id: g.id, photoId: g.photoId || null, photo: g.photo || null, caption: g.caption || "" }))
        : [],
      // Source documents (scans of certificates, letters, articles). Deep-copied
      // so form edits don't touch the live record until Save.
      documents: existing && Array.isArray(existing.documents)
        ? existing.documents.map((d) => ({ id: d.id, photoId: d.photoId || null, photo: d.photo || null, title: d.title || "", kind: d.kind || "other", date: d.date || null, note: d.note || "" }))
        : [],
      birthDate: existing ? existing.birthDate || "" : "",
      birthDatePrecision: (existing && existing.birthDatePrecision) || "exact",
      deathDate: existing ? existing.deathDate || "" : "",
      deathDatePrecision: (existing && existing.deathDatePrecision) || "exact",
      birthPlace: existing ? existing.birthPlace || "" : "",
      birthPlace_hi: existing ? existing.birthPlace_hi || "" : "",
      deathPlace: existing ? existing.deathPlace || "" : "",
      deathPlace_hi: existing ? existing.deathPlace_hi || "" : "",
      gender: existing ? existing.gender || "" : "",
      notes: existing ? existing.notes || "" : "",
      notes_hi: existing ? existing.notes_hi || "" : "",
      occupation: existing ? existing.occupation || "" : "",
      occupation_hi: existing ? existing.occupation_hi || "" : "",
      description: existing ? existing.description || "" : "",
      description_hi: existing ? existing.description_hi || "" : "",
      achievements: existing && existing.achievements ? existing.achievements.slice() : [],
      achievements_hi: existing && existing.achievements_hi ? existing.achievements_hi.slice() : [],
      education: existing && existing.education ? existing.education.slice() : [],
      education_hi: existing && existing.education_hi ? existing.education_hi.slice() : [],
      parents: existing ? existing.parents.slice() : (seed.parents ? seed.parents.slice() : []),
      unknownParents: existing && existing.unknownParents
        ? existing.unknownParents.slice()
        : (seed.unknownParents ? seed.unknownParents.slice() : []),
      spouses: existing ? existing.spouses.slice() : (seed.spouses ? seed.spouses.slice() : []),
      // Contact fields with per-field privacy flags. Existing record carries
      // its own contact object; new records start blank.
      contact: existing && existing.contact
        ? { phone: existing.contact.phone || "", email: existing.contact.email || "", address: existing.contact.address || "",
            privatePhone: !!existing.contact.privatePhone, privateEmail: !!existing.contact.privateEmail, privateAddress: !!existing.contact.privateAddress }
        : { phone: "", email: "", address: "", privatePhone: false, privateEmail: false, privateAddress: false },
      isPet: !!(existing && existing.isPet),
      petOwners: existing && existing.petOwners ? existing.petOwners.slice() : [],
      __addAsParentOf: seed.__addAsParentOf || null
    };

    // First person into an empty tree is almost always the signed-in user, so
    // pre-fill their name from the account (they can overwrite it). Guarded to
    // the genuinely-empty case: never on edit, never when a seed already named
    // them, and never once the tree has anyone — so it can't clobber a later add.
    if (!isEdit && !draft.name && FamilyStore.getPeople().length === 0) {
      const me = (window.Auth && Auth.getFirstName && Auth.getFirstName()) || "";
      if (me) draft.name = me;
    }

    // Photo-blob lifecycle. We do NOT delete the old IDB blob eagerly when the
    // user replaces or removes a photo — a subsequent Cancel (or close-X /
    // Escape / backdrop) would then leave the record pointing at a blob we
    // already destroyed. Instead we remember the original blob + every blob
    // created during this editing session, and reconcile exactly once on close
    // (see the openModal onClose below): keep the blob the persisted record
    // actually references, delete the rest. This also plugs a re-upload orphan
    // leak (upload A, upload B, Save previously left A pinned forever).
    const originalPhotoId = draft.photoId; // blob the person had when the form opened (or null)
    // Every blob the record referenced when the form opened — avatar + gallery.
    // On any cancel path these must all survive; only blobs created THIS
    // session (sessionBlobs) that the saved record no longer references get freed.
    const originalBlobs = new Set();
    if (originalPhotoId) originalBlobs.add(originalPhotoId);
    (draft.gallery || []).forEach((g) => { if (g && g.photoId) originalBlobs.add(g.photoId); });
    (draft.documents || []).forEach((d) => { if (d && d.photoId) originalBlobs.add(d.photoId); });
    const sessionBlobs = new Set();        // blobs created via upload during this session (avatar + gallery + documents)
    let committed = false;                 // flipped true only on a successful save

    // Snapshot the draft so we can detect unsaved edits on dismiss. The form
    // can hold minutes of work (a full biography) and the app has no undo, so a
    // stray Escape / backdrop-tap / close-X must not silently discard it.
    const openSnapshot = JSON.stringify(draft);
    let discardConfirming = false;         // guard against stacking confirm dialogs

    // ----- Photo uploader -----
    const fileInput = el("input", {
      type: "file",
      accept: "image/*",
      hidden: true,
      onchange: async (e) => {
        const file = e.target.files && e.target.files[0];
        e.target.value = "";
        if (!file) return;
        try {
          // Do NOT delete the previous blob here — Cancel must be able to
          // restore it. Cleanup of superseded blobs happens once on close
          // (onClose reconciler below), keyed off what was actually saved.
          if (window.PhotoStore) {
            const id = await PhotoStore.fileToPhotoId(file);
            sessionBlobs.add(id);
            draft.photoId = id;
            draft.photo = null;
          } else {
            const dataUrl = await FamilyStore.fileToDataURL(file);
            draft.photo = dataUrl;
          }
          // Crop frames are tied to the *previous* image's dimensions and
          // composition. A new photo deserves a fresh "fit, centred" default.
          draft.photoCropAvatar = null;
          draft.photoCropHero = null;
          refreshPhoto();
          // The crop tool is otherwise easy to miss, so open it immediately on a
          // fresh upload — the user lands straight on framing instead of hunting
          // for "Reframe". Still cancel-able: dismissing keeps the fit-centred
          // default.
          await openReframe(true);
        } catch (err) {
          toast(err.message, "danger");
        }
      }
    });

    const photoSlot = el("div", null);
    const removeBtn = el("button", {
      class: "btn btn--ghost btn--sm",
      type: "button",
      onclick: () => {
        // Detach only — the blob is reconciled on close. If the user removes
        // then cancels, the original is still intact in IDB.
        draft.photoId = null; draft.photo = null;
        draft.photoCropAvatar = null; draft.photoCropHero = null;
        refreshPhoto();
      }
    }, [
      el("span", null, I18n.t("actions.remove"))
    ]);
    // Open the crop editor on the current photo. Shared by the Reframe button and
    // the auto-open-after-upload path. `silent` skips the "reframe first" toast so
    // a just-uploaded photo (which always has a URL) doesn't nag if the user cancels.
    async function openReframe(silent) {
      if (!window.CropEditor) return;
      // Resolve current photo to a URL the editor can show.
      let url = null;
      if (draft.photo) url = draft.photo;
      else if (draft.photoId && window.PhotoStore) url = await PhotoStore.getUrl({ photoId: draft.photoId });
      if (!url) { if (!silent) toast(I18n.t("form.noPhotoReframe"), "danger"); return; }
      const out = await CropEditor.open(url, {
        initialAvatar: draft.photoCropAvatar,
        initialHero: draft.photoCropHero
      });
      if (!out) return;
      draft.photoCropAvatar = out.avatar;
      draft.photoCropHero = out.hero;
      refreshPhoto();
      toast(I18n.t("form.cropsSaved"), "success");
    }
    const reframeBtn = el("button", {
      class: "btn btn--sm", type: "button",
      onclick: () => openReframe()
    }, [
      el("i", { class: "fa-solid fa-crop-simple", "aria-hidden": "true" }),
      el("span", null, I18n.t("form.reframe"))
    ]);
    const buttonsCol = el("div", { class: "photo-uploader__buttons" }, [
      el("button", {
        class: "btn btn--sm",
        type: "button",
        onclick: () => fileInput.click()
      }, [
        el("i", { class: "fa-solid fa-arrow-up-from-bracket", "aria-hidden": "true" }),
        el("span", null, I18n.t("actions.upload"))
      ]),
      reframeBtn,
      removeBtn
    ]);
    const photoUploader = el("div", { class: "photo-uploader" }, [photoSlot, buttonsCol, fileInput]);

    function refreshPhoto() {
      clear(photoSlot);
      photoSlot.appendChild(avatar({
        name: draft.name || "?",
        photo: draft.photo, photoId: draft.photoId,
        photoCropAvatar: draft.photoCropAvatar
      }, "lg"));
      const has = !!(draft.photoId || draft.photo);
      removeBtn.style.display = has ? "" : "none";
      reframeBtn.style.display = has ? "" : "none";
    }
    refreshPhoto();

    // ----- Gallery: additional photos beyond the avatar -----
    // A thumbnail grid + an "Add photo" tile. Uploaded blobs join sessionBlobs
    // so the onClose reconciler frees them on Cancel; the reconciler already
    // keeps only what the SAVED record references (see keepIds below), so a
    // removed-then-cancelled gallery photo is restored intact.
    const galleryGrid = el("div", { class: "gallery-editor__grid" });
    const galleryFileInput = el("input", {
      type: "file", accept: "image/*", hidden: true,
      onchange: async (e) => {
        const file = e.target.files && e.target.files[0];
        e.target.value = "";
        if (!file) return;
        if (!window.PhotoStore) { toast(I18n.t("form.galleryNoStore"), "danger"); return; }
        try {
          const id = await PhotoStore.fileToPhotoId(file);
          sessionBlobs.add(id);
          draft.gallery.push({ id: "g_" + id, photoId: id, photo: null, caption: "" });
          renderGallery();
        } catch (err) {
          toast(err.message, "danger");
        }
      }
    });
    async function setThumb(imgEl, item) {
      let url = item.photo || null;
      if (!url && item.photoId && window.PhotoStore) {
        try { url = await PhotoStore.getUrl({ photoId: item.photoId }); } catch (_) {}
      }
      if (url) imgEl.src = url;
    }
    function renderGallery() {
      clear(galleryGrid);
      draft.gallery.forEach((item, idx) => {
        const img = el("img", { class: "gallery-editor__thumb-img", src: "", alt: item.caption || "" });
        setThumb(img, item);
        const caption = el("input", {
          class: "input gallery-editor__caption", type: "text",
          value: item.caption || "", placeholder: I18n.t("form.galleryCaptionPlaceholder"),
          "aria-label": I18n.t("form.galleryCaption"),
          oninput: (e) => { item.caption = e.target.value; }
        });
        const del = el("button", {
          class: "btn btn--icon btn--sm gallery-editor__remove", type: "button",
          title: I18n.t("actions.remove"), "aria-label": I18n.t("actions.remove"),
          onclick: () => { draft.gallery.splice(idx, 1); renderGallery(); }
        }, [el("i", { class: "fa-solid fa-xmark", "aria-hidden": "true" })]);
        galleryGrid.appendChild(el("figure", { class: "gallery-editor__cell" }, [
          el("div", { class: "gallery-editor__thumb" }, [img, del]),
          caption
        ]));
      });
      // Add tile — always last.
      galleryGrid.appendChild(el("button", {
        class: "gallery-editor__add", type: "button",
        onclick: () => galleryFileInput.click()
      }, [
        el("i", { class: "fa-solid fa-plus", "aria-hidden": "true" }),
        el("span", null, I18n.t("form.galleryAdd"))
      ]));
    }
    renderGallery();
    const galleryEditor = el("div", { class: "gallery-editor" }, [galleryGrid, galleryFileInput]);

    // ----- Documents / sources: scanned certificates, letters, articles -----
    // A stacked list rather than a thumbnail grid — each source carries a
    // title, a kind, an optional date, and a citation note ("what this
    // verifies"), so it needs more room than a caption. Scans store on the same
    // blob substrate as photos (via fileToDocumentId → larger, still EXIF-
    // stripped) and join sessionBlobs so the onClose reconciler frees them on
    // Cancel exactly like gallery/avatar photos.
    const DOC_KIND_OPTS = ["certificate", "letter", "article", "photo", "other"];
    const docList = el("div", { class: "doc-editor__list" });
    const docFileInput = el("input", {
      type: "file", accept: "image/*", hidden: true,
      onchange: async (e) => {
        const file = e.target.files && e.target.files[0];
        e.target.value = "";
        if (!file) return;
        if (!window.PhotoStore || !PhotoStore.fileToDocumentId) { toast(I18n.t("form.galleryNoStore"), "danger"); return; }
        try {
          const id = await PhotoStore.fileToDocumentId(file);
          sessionBlobs.add(id);
          draft.documents.push({ id: "doc_" + id, photoId: id, photo: null, title: "", kind: "certificate", date: null, note: "" });
          renderDocuments();
        } catch (err) {
          toast(err.message, "danger");
        }
      }
    });
    async function setDocThumb(imgEl, item) {
      let url = item.photo || null;
      if (!url && item.photoId && window.PhotoStore) {
        try { url = await PhotoStore.getUrl({ photoId: item.photoId }); } catch (_) {}
      }
      if (url) imgEl.src = url;
    }
    function renderDocuments() {
      clear(docList);
      draft.documents.forEach((item, idx) => {
        const img = el("img", { class: "doc-editor__thumb-img", src: "", alt: item.title || "" });
        setDocThumb(img, item);
        const thumb = el("div", { class: "doc-editor__thumb" }, [img]);

        const titleInput = el("input", {
          class: "input doc-editor__title", type: "text",
          value: item.title || "", placeholder: I18n.t("form.docTitlePlaceholder"),
          "aria-label": I18n.t("form.docTitle"),
          oninput: (e) => { item.title = e.target.value; }
        });
        const kindSelect = el("select", {
          class: "input doc-editor__kind", "aria-label": I18n.t("form.docKind"),
          onchange: (e) => { item.kind = e.target.value; }
        }, DOC_KIND_OPTS.map((k) => el("option", {
          value: k, selected: (item.kind || "other") === k ? true : undefined
        }, I18n.t("form.docKind_" + k))));
        const dateInput = el("input", {
          class: "input doc-editor__date", type: "text",
          value: item.date || "", placeholder: I18n.t("form.datePlaceholder"),
          "aria-label": I18n.t("form.docDate"),
          oninput: (e) => { item.date = e.target.value.trim() || null; }
        });
        const noteInput = el("textarea", {
          class: "input doc-editor__note", rows: "2",
          placeholder: I18n.t("form.docNotePlaceholder"),
          "aria-label": I18n.t("form.docNote"),
          oninput: (e) => { item.note = e.target.value; }
        });
        noteInput.value = item.note || "";
        const del = el("button", {
          class: "btn btn--icon btn--sm doc-editor__remove", type: "button",
          title: I18n.t("actions.remove"), "aria-label": I18n.t("actions.remove"),
          onclick: () => { draft.documents.splice(idx, 1); renderDocuments(); }
        }, [el("i", { class: "fa-solid fa-xmark", "aria-hidden": "true" })]);

        docList.appendChild(el("div", { class: "doc-editor__row" }, [
          thumb,
          el("div", { class: "doc-editor__fields" }, [
            el("div", { class: "doc-editor__fields-top" }, [titleInput, kindSelect, dateInput, del]),
            noteInput
          ])
        ]));
      });
      docList.appendChild(el("button", {
        class: "gallery-editor__add doc-editor__add", type: "button",
        onclick: () => docFileInput.click()
      }, [
        el("i", { class: "fa-solid fa-file-arrow-up", "aria-hidden": "true" }),
        el("span", null, I18n.t("form.docAdd"))
      ]));
    }
    renderDocuments();
    const documentsEditor = el("div", { class: "doc-editor" }, [docList, docFileInput]);

    // ----- Text fields -----
    const nameInput = el("input", {
      class: "input",
      type: "text",
      value: draft.name,
      required: true,
      placeholder: I18n.t("form.namePlaceholder"),
      oninput: (e) => {
        draft.name = e.target.value;
        if (!(draft.photoId || draft.photo)) refreshPhoto();
        if (typeof clearFieldError === "function") clearFieldError("name");
      }
    });
    const nameHiInput = el("input", {
      class: "input", type: "text", value: draft.name_hi,
      placeholder: "पूरा नाम", lang: "hi",
      oninput: (e) => { draft.name_hi = e.target.value; }
    });

    // Heritage date pickers — fall back to plain text inputs if the
    // module hasn't loaded for any reason.
    const birthDatePicker = window.HeritagePicker
      ? window.HeritagePicker.create({
          value: draft.birthDate, placeholder: "YYYY-MM-DD",
          allowYearOnly: true,
          ariaLabel: I18n.t("form.birthDate"),
          onChange: (iso) => { draft.birthDate = iso; clearFieldError("birth"); }
        })
      : null;
    const birthDateInput = birthDatePicker
      ? birthDatePicker.el
      : el("input", {
          class: "input", type: "text", value: draft.birthDate,
          placeholder: "YYYY-MM-DD or YYYY",
          oninput: (e) => { draft.birthDate = e.target.value; clearFieldError("birth"); }
        });

    const deathDatePicker = window.HeritagePicker
      ? window.HeritagePicker.create({
          value: draft.deathDate, placeholder: I18n.t("form.deathDatePlaceholder"),
          allowYearOnly: true,
          ariaLabel: I18n.t("form.deathDate"),
          onChange: (iso) => { draft.deathDate = iso; clearFieldError("death"); syncDeathPrecVisibility(); }
        })
      : null;
    const deathDateInput = deathDatePicker
      ? deathDatePicker.el
      : el("input", {
          class: "input", type: "text", value: draft.deathDate,
          placeholder: "YYYY-MM-DD or YYYY",
          oninput: (e) => { draft.deathDate = e.target.value; clearFieldError("death"); syncDeathPrecVisibility(); }
        });

    // Precision pickers — Exact / About / Before / After. Sit beside each
    // date input. The precision only means something once a date exists, so
    // the death picker is hidden while the death date is blank ("living").
    const PRECISION_OPTS = [
      { value: "exact",  label: I18n.t("form.precExact") },
      { value: "about",  label: I18n.t("form.precAbout") },
      { value: "before", label: I18n.t("form.precBefore") },
      { value: "after",  label: I18n.t("form.precAfter") }
    ];
    const birthPrecPicker = window.HeritageSelect ? HeritageSelect.create({
      options: PRECISION_OPTS, value: draft.birthDatePrecision || "exact",
      onChange: (v) => { draft.birthDatePrecision = v; }
    }) : null;
    const deathPrecPicker = window.HeritageSelect ? HeritageSelect.create({
      options: PRECISION_OPTS, value: draft.deathDatePrecision || "exact",
      onChange: (v) => { draft.deathDatePrecision = v; }
    }) : null;
    // Label the precision selects for screen readers (the visible field label
    // names the date input beside them, not the picker).
    if (birthPrecPicker) birthPrecPicker.el.setAttribute("aria-label", I18n.t("form.datePrecisionLabel", { field: I18n.t("form.birthDate") }));
    if (deathPrecPicker) deathPrecPicker.el.setAttribute("aria-label", I18n.t("form.datePrecisionLabel", { field: I18n.t("form.deathDate") }));

    // Per-field inline error spans, tied to their control via aria-describedby.
    // Keyed so onChange handlers and the save validator can clear/set by name.
    const fieldErrors = {};
    function makeErrorSpan(key) {
      const span = el("span", { class: "field__error", id: "pf-err-" + key, role: "alert" }, [
        el("i", { class: "fa-solid fa-circle-exclamation", "aria-hidden": "true" }),
        el("span", { class: "field__error-msg" }, "")
      ]);
      fieldErrors[key] = span;
      return span;
    }
    function controlFor(key) {
      if (key === "name") return nameInput;
      if (key === "birth") return birthDatePicker ? birthDatePicker.input : birthDateInput;
      if (key === "death") return deathDatePicker ? deathDatePicker.input : deathDateInput;
      return null;
    }
    function setFieldError(key, msg) {
      const span = fieldErrors[key];
      if (span) {
        span.querySelector(".field__error-msg").textContent = msg;
        span.classList.add("is-shown");
      }
      const ctl = controlFor(key);
      if (ctl) { ctl.setAttribute("aria-invalid", "true"); ctl.setAttribute("aria-describedby", "pf-err-" + key); ctl.classList.add("is-invalid"); }
    }
    function clearFieldError(key) {
      const span = fieldErrors[key];
      if (span) { span.classList.remove("is-shown"); span.querySelector(".field__error-msg").textContent = ""; }
      const ctl = controlFor(key);
      if (ctl) { ctl.removeAttribute("aria-invalid"); ctl.removeAttribute("aria-describedby"); ctl.classList.remove("is-invalid"); }
    }
    // Grid element per date row, so syncDeathPrecVisibility can collapse the
    // track when the precision picker is hidden (else the fixed 130px column
    // leaves an empty gutter beside every living person's death date).
    const precGrids = {};
    function syncDeathPrecVisibility() {
      if (!deathPrecPicker) return;
      const hasDeath = !!(draft.deathDate && String(draft.deathDate).trim());
      deathPrecPicker.el.style.display = hasDeath ? "" : "none";
      const grid = precGrids.death;
      if (grid) grid.style.gridTemplateColumns = hasDeath ? "1fr 130px" : "1fr";
    }
    function dateWithPrecisionField(label, key, dateEl, precPicker, hint) {
      const grid = el("div", { style: { display: "grid", gridTemplateColumns: "1fr 130px", gap: "8px" } }, [
        dateEl,
        precPicker ? precPicker.el : null
      ]);
      precGrids[key] = grid;
      const wrap = el("div", { class: "field" }, [
        el("span", { class: "field__label" }, label),
        grid,
        hint ? el("span", { class: "field__hint" }, hint) : null,
        makeErrorSpan(key)
      ]);
      return wrap;
    }

    const birthPlaceInput = el("input", {
      class: "input", type: "text", value: draft.birthPlace,
      placeholder: I18n.t("form.placePlaceholder"),
      oninput: (e) => { draft.birthPlace = e.target.value; }
    });
    const birthPlaceHiInput = el("input", {
      class: "input", type: "text", value: draft.birthPlace_hi,
      placeholder: "नगर, देश", lang: "hi",
      oninput: (e) => { draft.birthPlace_hi = e.target.value; }
    });

    const deathPlaceInput = el("input", {
      class: "input", type: "text", value: draft.deathPlace,
      placeholder: I18n.t("form.placePlaceholder"),
      oninput: (e) => { draft.deathPlace = e.target.value; }
    });
    const deathPlaceHiInput = el("input", {
      class: "input", type: "text", value: draft.deathPlace_hi,
      placeholder: "नगर, देश", lang: "hi",
      oninput: (e) => { draft.deathPlace_hi = e.target.value; }
    });

    const genderPicker = window.HeritageSelect
      ? HeritageSelect.create({
          options: [
            { value: "", label: I18n.t("form.genderNone") },
            { value: "m", label: I18n.t("form.genderM") },
            { value: "f", label: I18n.t("form.genderF") },
            { value: "o", label: I18n.t("form.genderO") }
          ],
          value: draft.gender || "",
          placeholder: I18n.t("form.genderNone"),
          ariaLabel: I18n.t("form.gender"),
          onChange: (v) => { draft.gender = v; }
        })
      : null;
    const genderSelect = genderPicker
      ? genderPicker.el
      : el("select", { class: "select", onchange: (e) => { draft.gender = e.target.value; } }, [
          el("option", { value: "" }, I18n.t("form.genderNone")),
          el("option", { value: "m" }, I18n.t("form.genderM")),
          el("option", { value: "f" }, I18n.t("form.genderF")),
          el("option", { value: "o" }, I18n.t("form.genderO"))
        ]);
    if (!genderPicker) genderSelect.value = draft.gender || "";

    const notesInput = el("textarea", {
      class: "textarea",
      placeholder: I18n.t("form.notesPlaceholder"),
      oninput: (e) => { draft.notes = e.target.value; }
    }, draft.notes);
    const notesHiInput = el("textarea", {
      class: "textarea", placeholder: "कहानियाँ, यादें…", lang: "hi",
      oninput: (e) => { draft.notes_hi = e.target.value; }
    }, draft.notes_hi);

    // Contact fields — phone / email / address. Each with a small "private"
    // checkbox that strips the field from JSON / PNG / poster exports.
    function contactRow(field, type, placeholder, icon) {
      const input = el("input", {
        class: "input", type, value: draft.contact[field], placeholder,
        oninput: (e) => { draft.contact[field] = e.target.value; }
      });
      const privKey = "private" + field.charAt(0).toUpperCase() + field.slice(1);
      const privateCb = el("input", { type: "checkbox" });
      privateCb.checked = !!draft.contact[privKey];
      privateCb.addEventListener("change", () => { draft.contact[privKey] = privateCb.checked; });
      const privateLabel = el("label", { class: "form-private-toggle", title: I18n.t("form.privateHint") }, [
        privateCb,
        el("i", { class: "fa-solid fa-lock", "aria-hidden": "true" }),
        el("span", null, I18n.t("form.private"))
      ]);
      return el("div", { class: "form-contact-row" }, [
        el("i", { class: icon + " form-contact-row__icon", "aria-hidden": "true" }),
        input,
        privateLabel
      ]);
    }
    const contactRows = el("div", { class: "form-contact" }, [
      contactRow("phone", "tel", I18n.t("form.phonePlaceholder"), "fa-solid fa-phone"),
      contactRow("email", "email", "name@example.com", "fa-solid fa-envelope"),
      contactRow("address", "text", I18n.t("form.addressPlaceholder"), "fa-solid fa-house")
    ]);

    // Pet toggle — companion animal? When on, the tree paints a paw badge AND
    // reveals an owner picker (below): a pet is anchored beneath its owners the
    // way a child sits beneath parents, so without an owner it floats rootless.
    // Elements are built here; the owner rows are populated later by
    // rebuildPetOwnerRows() (defined alongside the spouse rows), because the
    // pickers need relationOptions/others/NEW_PERSON, which are declared below.
    const petCb = el("input", { type: "checkbox" });
    petCb.checked = !!draft.isPet;
    petCb.addEventListener("change", () => {
      draft.isPet = petCb.checked;
      // Show/hide the owner picker in step with the toggle, and (re)populate it
      // the first time it's revealed. Hidden via inline display so the field
      // keeps its place in the flow when toggled back on.
      if (petOwnersField) petOwnersField.style.display = petCb.checked ? "" : "none";
      if (petCb.checked) rebuildPetOwnerRows();
    });
    const petToggle = el("label", { class: "form-pet-toggle" }, [
      petCb,
      el("i", { class: "fa-solid fa-paw", "aria-hidden": "true" }),
      el("span", null, I18n.t("form.petLabel"))
    ]);
    // Owner picker — a dynamic list of rows (a pet can belong to a couple), the
    // same shape as the spouse list. Populated by rebuildPetOwnerRows() below.
    const petOwnersHost = el("div", { class: "form-stack", style: { gap: "8px" } });
    let petOwnerList = (draft.petOwners || []).slice(); // working array of ids
    const petOwnersField = field(I18n.t("form.petOwners"), petOwnersHost, I18n.t("form.petOwnersHint"));
    petOwnersField.style.display = draft.isPet ? "" : "none";

    const occupationInput = el("input", {
      class: "input", type: "text", value: draft.occupation,
      placeholder: I18n.t("form.occupationPlaceholder"),
      oninput: (e) => { draft.occupation = e.target.value; }
    });
    const occupationHiInput = el("input", {
      class: "input", type: "text", value: draft.occupation_hi,
      placeholder: "अभियंता, शिक्षक…", lang: "hi",
      oninput: (e) => { draft.occupation_hi = e.target.value; }
    });

    const descriptionInput = el("textarea", {
      class: "textarea",
      placeholder: I18n.t("form.descriptionPlaceholder"),
      oninput: (e) => { draft.description = e.target.value; }
    }, draft.description);
    const descriptionHiInput = el("textarea", {
      class: "textarea", placeholder: "संक्षिप्त परिचय…", lang: "hi",
      oninput: (e) => { draft.description_hi = e.target.value; }
    }, draft.description_hi);

    const achievementsInput = el("textarea", {
      class: "textarea",
      placeholder: I18n.t("form.achievementsPlaceholder"),
      oninput: (e) => { draft.achievements = e.target.value.split(/\n+/).map((s) => s.trim()).filter(Boolean); }
    }, (draft.achievements || []).join("\n"));
    const achievementsHiInput = el("textarea", {
      class: "textarea", placeholder: "उपलब्धियाँ (प्रत्येक पंक्ति में एक)", lang: "hi",
      oninput: (e) => { draft.achievements_hi = e.target.value.split(/\n+/).map((s) => s.trim()).filter(Boolean); }
    }, (draft.achievements_hi || []).join("\n"));

    const educationInput = el("textarea", {
      class: "textarea",
      placeholder: I18n.t("form.educationPlaceholder"),
      oninput: (e) => { draft.education = e.target.value.split(/\n+/).map((s) => s.trim()).filter(Boolean); }
    }, (draft.education || []).join("\n"));
    const educationHiInput = el("textarea", {
      class: "textarea", placeholder: "शिक्षा (प्रत्येक पंक्ति में एक)", lang: "hi",
      oninput: (e) => { draft.education_hi = e.target.value.split(/\n+/).map((s) => s.trim()).filter(Boolean); }
    }, (draft.education_hi || []).join("\n"));

    // ----- Relations (father / mother / spouses) -----
    // No native pickers; everything goes through HeritageSelect.
    const others = FamilyStore.getPeople()
      .filter((p) => p.id !== draft.id)
      .sort((a, b) => (a.name || "").localeCompare(b.name || "", "en"));

    // Descendants of the person being edited must not be offered as a parent —
    // picking one would create an ancestor cycle (e.g. set your own son as
    // your father), which corrupts buildGenerations' generation numbering and
    // makes drawEdges render a backward-pointing edge. The pickers previously
    // only excluded self + narrowed by gender, so this was reachable in two
    // clicks through the ordinary form. BFS over children from the draft id;
    // for a NEW person (draft.id null) there are no descendants → empty set,
    // so this is a safe no-op there.
    const descendantIds = (function collectDescendants() {
      const set = new Set();
      if (!draft.id) return set;
      const queue = [draft.id];
      while (queue.length) {
        const cur = queue.shift();
        (FamilyStore.getChildrenOf(cur) || []).forEach((child) => {
          if (child && !set.has(child.id)) { set.add(child.id); queue.push(child.id); }
        });
      }
      return set;
    })();

    function personLabel(p) {
      const dn = FamilyStore.getField(p, "name") || p.name;
      const yr = FamilyStore.getYear(p.birthDate);
      return yr ? `${dn} · ${yr}` : dn;
    }

    // Sentinel option value for the inline "add a new person" row present in
    // every relation picker. Prefixed so it can never collide with a real
    // person id (genId uses "p_") or the empty "— None —" value.
    const NEW_PERSON = "__add_new_person__";

    function relationOptions(filterFn, noneLabel) {
      const opts = [{ value: "", label: noneLabel || "—" }];
      // Inline "add a new person" row, kept at the top so it's visible without
      // scrolling a long list. Selecting it is intercepted in each picker's
      // onChange (never stored as a value) — it opens quickCreatePerson().
      opts.push({ value: NEW_PERSON, label: I18n.t("form.relAddNew") });
      others.filter(filterFn || (() => true)).forEach((p) => {
        opts.push({ value: p.id, label: personLabel(p) });
      });
      return opts;
    }

    // Open a small stacked dialog capturing just a name, and create a person
    // with `gender` pre-set from the slot that asked (father→m, mother→f,
    // spouse→the opposite of this record's gender). Lets a relative be entered
    // "as remembered" without abandoning the half-filled form — the backlog's
    // "no create-from-relation-picker" gap. Resolves with the created person,
    // or null if dismissed. The person is persisted immediately (like "Save &
    // add another"); the *link* to this record is only saved when the form is,
    // so cancelling the form leaves an unlinked person, not a lost one.
    // `title` (already localised) names the slot. On success the newcomer is
    // spliced into `others` (the list every picker reads) so it appears at once.
    function quickCreatePerson(gender, title) {
      return new Promise((resolve) => {
        let settled = false;
        const settle = (v) => { if (!settled) { settled = true; resolve(v); } };

        const input = el("input", {
          class: "input", type: "text", required: true,
          placeholder: I18n.t("form.namePlaceholder")
        });
        const errSpan = el("span", { class: "field__error", role: "alert" }, [
          el("i", { class: "fa-solid fa-circle-exclamation", "aria-hidden": "true" }),
          el("span", { class: "field__error-msg" }, "")
        ]);
        input.addEventListener("input", () => errSpan.classList.remove("is-shown"));
        const nameField = field(I18n.t("form.name"), input, I18n.t("form.required"));
        nameField.appendChild(errSpan);

        const cancelBtn = UI.cancelBtn(I18n.t("actions.cancel"));
        const createBtn = UI.saveBtn(I18n.t("actions.add"), { icon: "fa-solid fa-user-plus" });

        function doCreate() {
          const nm = (input.value || "").trim();
          if (!nm) {
            errSpan.querySelector(".field__error-msg").textContent = I18n.t("form.nameRequired");
            errSpan.classList.add("is-shown");
            input.focus();
            return;
          }
          const person = FamilyStore.addPerson({ name: nm, gender: gender || null });
          // addPerson no-ops (returns null) under read-only; the outer form is
          // already gated, but never claim success on a null write.
          if (!person) { toast(I18n.t("people.readOnly"), "warning"); settle(null); dlg.close(); return; }
          // Slot the newcomer into the candidate list every picker reads from,
          // kept name-sorted so the pickers stay alphabetical.
          others.push(person);
          others.sort((a, b) => (a.name || "").localeCompare(b.name || "", "en"));
          settle(person);
          dlg.close();
        }

        createBtn.addEventListener("click", doCreate);
        cancelBtn.addEventListener("click", () => { settle(null); dlg.close(); });

        const dlg = openModal({
          title,
          body: el("div", { class: "form-stack" }, [nameField]),
          footer: [cancelBtn, createBtn],
          onEnter: () => doCreate(),
          onClose: () => settle(null)
        });
        setTimeout(() => { try { input.focus(); } catch (_) {} }, 50);
      });
    }

    // Gender-aware filters. We accept "no gender on file" everywhere so
    // people whose gender we haven't recorded aren't excluded.
    const isMale   = (p) => p.gender === "m" || !p.gender;
    const isFemale = (p) => p.gender === "f" || !p.gender;
    function spouseGenderFilter() {
      // Whoever is being edited has draft.gender. Spouses are typically the
      // opposite — but again only narrow when we *have* a gender on file.
      if (draft.gender === "m") return (p) => p.gender === "f" || !p.gender;
      if (draft.gender === "f") return (p) => p.gender === "m" || !p.gender;
      // "Other" / unspecified → don't narrow.
      return () => true;
    }

    // Slot the existing parents into Father / Mother for editing. A male parent
    // fills Father, a female parent fills Mother; any still-unplaced parent
    // (ungendered, or a same-gender second) drops into the first empty slot.
    // This is just for the form — the schema still stores `parents: [id, id]`.
    //
    // Slotting MUST respect gender, not position: a sole female parent assigned
    // to Father positionally would be excluded by the male-only Father filter
    // (and by the Mother filter, which excludes whoever's in Father), leaving
    // her invisible in both pickers and silently droppable on the next edit.
    let fatherId = "", motherId = "";
    if (draft.parents && draft.parents.length) {
      const ps = draft.parents.map(FamilyStore.getPerson).filter(Boolean);
      const male = ps.find((p) => p.gender === "m");
      const female = ps.find((p) => p.gender === "f");
      if (male) fatherId = male.id;
      if (female && female.id !== fatherId) motherId = female.id;
      // Fill remaining empty slots with any parent not already placed.
      ps.forEach((p) => {
        if (p.id === fatherId || p.id === motherId) return;
        if (!fatherId) fatherId = p.id;
        else if (!motherId) motherId = p.id;
      });
    }
    function recomputeParents() {
      const out = [];
      if (fatherId) out.push(fatherId);
      if (motherId && motherId !== fatherId) out.push(motherId);
      draft.parents = out;
    }

    // "Explicitly unknown" parent roles — the family recorded that a father
    // and/or mother exists but their identity is unknown (adoption, lost line).
    // Distinct from an empty picker (= not filled in yet). Held as a working
    // Set of roles and mirrored into draft.unknownParents on every change;
    // mutually exclusive with a chosen person in the same slot.
    const unknownRoles = new Set(
      (draft.unknownParents || []).filter((r) => r === "father" || r === "mother")
    );
    function syncUnknownParents() {
      draft.unknownParents = ["father", "mother"].filter((r) => unknownRoles.has(r));
    }
    // Slot UI handles ({ cb, wrap }), populated when the parent slots are built
    // below the pickers. `clearUnknownRole` is called from a picker's onChange:
    // choosing a real person and marking that same slot "unknown" are mutually
    // exclusive, so a fresh pick silently un-checks the Unknown box.
    const unknownSlots = { father: null, mother: null };
    function clearUnknownRole(role) {
      if (!unknownRoles.has(role)) return;
      unknownRoles.delete(role);
      syncUnknownParents();
      const slot = unknownSlots[role];
      if (slot) { slot.cb.checked = false; slot.wrap.classList.remove("parent-slot--unknown"); }
    }

    // Gender-narrow the parent dropdowns: fathers are male-or-unspecified,
    // mothers are female-or-unspecified. Showing every person in both lists
    // is confusing — users expect "Father" to mean a male candidate.
    // BUT always keep whoever is currently in this slot, even if their gender
    // doesn't match the heuristic — otherwise an already-assigned parent (from
    // legacy data or a same-gender couple) would vanish from their own picker
    // and couldn't be re-selected or cleared.
    // A person can't be both a spouse and a parent of the same record — that's
    // a structurally nonsensical relationship the form previously allowed
    // silently (neither filter excluded the other's picks). Exclude anyone
    // currently listed as a spouse from the parent pickers, EXCEPT whoever
    // already occupies this slot (so a legacy bad record stays visible and can
    // be cleared). Reads draft.spouses — kept in sync by the spouse onChange —
    // NOT spouseList, which isn't declared until after these filters are first
    // invoked (picker creation below).
    const spouseIdsNow = () => (draft.spouses || []);
    const fatherFilter = (p) => p.id !== motherId && (p.id === fatherId || isMale(p)) && !descendantIds.has(p.id) && (p.id === fatherId || !spouseIdsNow().includes(p.id));
    const motherFilter = (p) => p.id !== fatherId && (p.id === motherId || isFemale(p)) && !descendantIds.has(p.id) && (p.id === motherId || !spouseIdsNow().includes(p.id));

    // Confirm clearing a picked parent to "— None —" (no undo otherwise, same as
    // the spouse ×). Returns true to proceed. Not "danger": re-picking restores it.
    function confirmClearParent(prevId) {
      const nm = FamilyStore.getField(FamilyStore.getPerson(prevId), "name") || (FamilyStore.getPerson(prevId) || {}).name || "";
      return confirm({
        title: I18n.t("form.unlinkParentTitle"),
        message: I18n.t("form.unlinkParentMsg", { name: nm }),
        confirmLabel: I18n.t("form.unlinkConfirm")
      });
    }

    const fatherPicker = HeritageSelect.create({
      options: relationOptions(fatherFilter, I18n.t("form.fatherNone") || "— None —"),
      value: fatherId,
      placeholder: I18n.t("form.fatherNone") || "— None —",
      ariaLabel: I18n.t("form.father"),
      onChange: async (v) => {
        // Inline "add a new person" → capture a name, create a male parent, and
        // select them without leaving the form. Revert the display to the prior
        // pick up front (setValue doesn't re-fire onChange, so this is loop-safe)
        // so the form behind the sub-dialog never shows the sentinel as a value;
        // on cancel that revert stands, on success we re-select the newcomer.
        if (v === NEW_PERSON) {
          fatherPicker.setValue(fatherId);
          const created = await quickCreatePerson("m", I18n.t("inspector.addParent"));
          if (!created) return;
          fatherId = created.id;
          clearUnknownRole("father");
          // The newcomer isn't in this picker's options yet — refresh, then select.
          fatherPicker.setOptions(relationOptions(fatherFilter, I18n.t("form.fatherNone") || "— None —"));
          fatherPicker.setValue(fatherId);
          motherPicker.setOptions(relationOptions(motherFilter, I18n.t("form.motherNone") || "— None —"));
          recomputeParents();
          refreshSpouseRowsSafe();
          return;
        }
        // Clearing a real parent to "— None —" is a silent unlink with no undo;
        // confirm it (mirrors the spouse ×). Swapping one father for another is
        // a deliberate correction, so it isn't gated. setValue() doesn't re-fire
        // onChange, so reverting on cancel is loop-safe.
        if (!v && fatherId && !(await confirmClearParent(fatherId))) {
          fatherPicker.setValue(fatherId);
          return;
        }
        fatherId = v;
        // Picking a real father contradicts "father unknown" — clear that flag.
        if (v) clearUnknownRole("father");
        motherPicker.setOptions(relationOptions(motherFilter, I18n.t("form.motherNone") || "— None —"));
        recomputeParents();
        // A new parent must disappear from the spouse pickers (and vice-versa).
        refreshSpouseRowsSafe();
      }
    });
    const motherPicker = HeritageSelect.create({
      options: relationOptions(motherFilter, I18n.t("form.motherNone") || "— None —"),
      value: motherId,
      placeholder: I18n.t("form.motherNone") || "— None —",
      ariaLabel: I18n.t("form.mother"),
      onChange: async (v) => {
        if (v === NEW_PERSON) {
          motherPicker.setValue(motherId);
          const created = await quickCreatePerson("f", I18n.t("inspector.addParent"));
          if (!created) return;
          motherId = created.id;
          clearUnknownRole("mother");
          motherPicker.setOptions(relationOptions(motherFilter, I18n.t("form.motherNone") || "— None —"));
          motherPicker.setValue(motherId);
          fatherPicker.setOptions(relationOptions(fatherFilter, I18n.t("form.fatherNone") || "— None —"));
          recomputeParents();
          refreshSpouseRowsSafe();
          return;
        }
        if (!v && motherId && !(await confirmClearParent(motherId))) {
          motherPicker.setValue(motherId);
          return;
        }
        motherId = v;
        if (v) clearUnknownRole("mother");
        fatherPicker.setOptions(relationOptions(fatherFilter, I18n.t("form.fatherNone") || "— None —"));
        recomputeParents();
        refreshSpouseRowsSafe();
      }
    });

    // Re-narrow both parent pickers against the current spouse list, preserving
    // each picker's own current value. Called when a spouse changes so a newly
    // added spouse drops out of Father/Mother (mutual exclusion). Defined here
    // (after both pickers exist) but only invoked at interaction time.
    function refreshParentPickers() {
      fatherPicker.setOptions(relationOptions(fatherFilter, I18n.t("form.fatherNone") || "— None —"));
      motherPicker.setOptions(relationOptions(motherFilter, I18n.t("form.motherNone") || "— None —"));
    }

    // Spouses — dynamic list of HeritageSelect rows
    const spouseRowsHost = el("div", { class: "form-stack", style: { gap: "8px" } });
    let spouseList = (draft.spouses || []).slice(); // working array of ids (with possible "")

    // `focusRow`: after rebuilding, move focus onto the picker at that row
    // index (clamped). HeritageSelect.pick() focuses its own element then fires
    // onChange → this rebuild, which clear()s the host and destroys that very
    // element, dropping focus to <body> on every spouse pick/add/remove.
    // Restoring focus to the equivalent rebuilt picker keeps keyboard flow.
    function rebuildSpouseRows(focusRow) {
      clear(spouseRowsHost);
      // Always show at least one row so the section isn't empty
      const items = spouseList.length ? spouseList : [""];
      const genderOk = spouseGenderFilter();
      const pickers = [];
      items.forEach((id, i) => {
        const used = items.filter((x, j) => x && j !== i);
        // Mirror the parent-picker exclusion: don't offer a current parent as a
        // spouse (except this row's own value, so a legacy record stays
        // editable). draft.parents is kept in sync by recomputeParents().
        const parentIds = draft.parents || [];
        const picker = HeritageSelect.create({
          options: relationOptions((p) => !used.includes(p.id) && genderOk(p) && (p.id === id || !parentIds.includes(p.id)), I18n.t("form.spouseNone") || "— None —"),
          value: id || "",
          placeholder: I18n.t("form.spouseNone") || "— None —",
          ariaLabel: items.length > 1 ? I18n.t("form.spouse") + " " + (i + 1) : I18n.t("form.spouse"),
          onChange: async (v) => {
            // Inline "add a new person" → create a spouse (gendered opposite to
            // this record where known) and slot them into THIS row. Revert the
            // display to the row's prior value up front so the sentinel never
            // shows behind the sub-dialog; on cancel that revert stands.
            if (v === NEW_PERSON) {
              picker.setValue(id || "");
              const g = draft.gender === "m" ? "f" : draft.gender === "f" ? "m" : null;
              const created = await quickCreatePerson(g, I18n.t("inspector.addSpouse"));
              if (!created) return;
              v = created.id;
            }
            spouseList[i] = v;
            // Keep draft.spouses clean (no empties, no dupes)
            draft.spouses = spouseList.filter((x, j, arr) => x && arr.indexOf(x) === j);
            // Re-render so the other rows can hide the new pick
            spouseList = draft.spouses.slice();
            // A new spouse must disappear from the parent pickers (mutual
            // exclusion — a spouse can't also be a parent of this record).
            refreshParentPickers();
            // Keep keyboard focus on the picker the user just committed.
            rebuildSpouseRows(i);
          }
        });
        pickers.push(picker);
        const row = el("div", { class: "hrel-row" }, [
          el("span", { class: "hrel-row__pic" }, [
            id ? UI.avatar(FamilyStore.getPerson(id), "sm") : el("span", {
              class: "avatar avatar--sm avatar--mist",
              style: { background: "var(--surface-2)", color: "var(--text-3)" }
            }, "♥")
          ]),
          el("div", { class: "hrel-row__sel" }, [picker.el]),
          (items.length > 1 || id)
            ? el("button", {
                class: "hrel-row__remove",
                type: "button",
                "aria-label": I18n.t("form.removeSpouse"),
                onclick: async () => {
                  // Confirm before dropping a REAL picked spouse — a single stray
                  // click on the × had no undo. An empty/unpicked row has nothing
                  // to lose, so it's removed silently (keeps quick row cleanup
                  // frictionless). Not "danger": unlinking is reversible by
                  // re-picking, unlike deleting a person.
                  if (id) {
                    const nm = FamilyStore.getField(FamilyStore.getPerson(id), "name") || (FamilyStore.getPerson(id) || {}).name || "";
                    const ok = await confirm({
                      title: I18n.t("form.unlinkSpouseTitle"),
                      message: I18n.t("form.unlinkSpouseMsg", { name: nm }),
                      confirmLabel: I18n.t("form.unlinkConfirm")
                    });
                    if (!ok) return;
                  }
                  spouseList.splice(i, 1);
                  draft.spouses = spouseList.filter(Boolean);
                  spouseList = draft.spouses.slice();
                  // A removed spouse can now reappear in the parent pickers.
                  refreshParentPickers();
                  // Focus the picker that slid into this slot (or the last one).
                  rebuildSpouseRows(i);
                }
              }, [el("i", { class: "fa-solid fa-xmark" })])
            : null
        ]);
        spouseRowsHost.appendChild(row);
      });
      // "+ Add another spouse" button (only meaningful if there are unused
      // candidates of a compatible gender — otherwise pressing + would just
      // add an empty row with nothing to pick).
      const usedIds = new Set(spouseList.filter(Boolean));
      const canAdd = others.some((p) => !usedIds.has(p.id) && genderOk(p));
      if (canAdd) {
        spouseRowsHost.appendChild(el("button", {
          class: "hrel-add", type: "button",
          onclick: () => {
            spouseList.push("");
            // Focus the freshly-added (last) empty picker.
            rebuildSpouseRows(spouseList.length - 1);
          }
        }, [
          el("i", { class: "fa-solid fa-plus" }),
          el("span", null, I18n.t("form.addSpouse") || "Add another spouse")
        ]));
      }
      // Restore keyboard focus after an interaction-driven rebuild (pick / add /
      // remove all pass a row index). Clamp to the pickers that actually exist
      // now — a remove can shrink the list. The initial build passes nothing, so
      // focus is left where the form put it. Deferred a tick so the freshly
      // appended element is laid out before .focus().
      if (focusRow != null && pickers.length) {
        const target = pickers[Math.min(focusRow, pickers.length - 1)];
        if (target && target.focus) setTimeout(() => { try { target.focus(); } catch (_) {} }, 0);
      }
    }
    // Rebuild the spouse rows WITHOUT moving focus — used when a parent change
    // needs the spouse pickers to re-exclude the new parent. The parent picker
    // the user just touched isn't inside spouseRowsHost, so its focus survives.
    function refreshSpouseRowsSafe() {
      try { rebuildSpouseRows(); } catch (_) {}
    }
    rebuildSpouseRows();

    // Pet-owner rows — the write path for `petOwners`. Structurally a clone of
    // the spouse row builder (a pet can belong to a couple), but candidates are
    // narrowed to non-pets: a companion's owner is a person, never another pet.
    // Owners are anchors, not kin, so there's no gender/descendant filtering.
    // Declared as a hoisted function so the pet toggle's change handler (above,
    // before `others`/relationOptions exist) can call it safely at click time.
    function rebuildPetOwnerRows(focusRow) {
      clear(petOwnersHost);
      const items = petOwnerList.length ? petOwnerList : [""];
      const pickers = [];
      const notPet = (p) => !p.isPet;
      items.forEach((id, i) => {
        const used = items.filter((x, j) => x && j !== i);
        const picker = HeritageSelect.create({
          options: relationOptions((p) => notPet(p) && !used.includes(p.id), I18n.t("form.petOwnerNone") || "— No owner —"),
          value: id || "",
          placeholder: I18n.t("form.petOwnerNone") || "— No owner —",
          ariaLabel: items.length > 1 ? I18n.t("form.petOwners") + " " + (i + 1) : I18n.t("form.petOwners"),
          onChange: async (v) => {
            // Inline "add a new person" → create an owner (no gender implied for
            // a pet's owner) and slot them into THIS row. Revert the display to
            // the prior value up front so the sentinel never shows behind the
            // sub-dialog; on cancel that revert stands.
            if (v === NEW_PERSON) {
              picker.setValue(id || "");
              const created = await quickCreatePerson(null, I18n.t("form.petOwners"));
              if (!created) return;
              v = created.id;
            }
            petOwnerList[i] = v;
            draft.petOwners = petOwnerList.filter((x, j, arr) => x && arr.indexOf(x) === j);
            petOwnerList = draft.petOwners.slice();
            rebuildPetOwnerRows(i);
          }
        });
        pickers.push(picker);
        const row = el("div", { class: "hrel-row" }, [
          el("span", { class: "hrel-row__pic" }, [
            id ? UI.avatar(FamilyStore.getPerson(id), "sm") : el("span", {
              class: "avatar avatar--sm avatar--mist",
              style: { background: "var(--surface-2)", color: "var(--text-3)" }
            }, el("i", { class: "fa-solid fa-paw", "aria-hidden": "true" }))
          ]),
          el("div", { class: "hrel-row__sel" }, [picker.el]),
          (items.length > 1 || id)
            ? el("button", {
                class: "hrel-row__remove",
                type: "button",
                "aria-label": I18n.t("form.removeSpouse"),
                onclick: async () => {
                  // Confirm before dropping a REAL picked owner (mirrors the
                  // spouse ×). An empty row has nothing to lose → removed silently.
                  if (id) {
                    const nm = FamilyStore.getField(FamilyStore.getPerson(id), "name") || (FamilyStore.getPerson(id) || {}).name || "";
                    const ok = await confirm({
                      title: I18n.t("form.unlinkOwnerTitle"),
                      message: I18n.t("form.unlinkOwnerMsg", { name: nm }),
                      confirmLabel: I18n.t("form.unlinkConfirm")
                    });
                    if (!ok) return;
                  }
                  petOwnerList.splice(i, 1);
                  draft.petOwners = petOwnerList.filter(Boolean);
                  petOwnerList = draft.petOwners.slice();
                  rebuildPetOwnerRows(i);
                }
              }, [el("i", { class: "fa-solid fa-xmark" })])
            : null
        ]);
        petOwnersHost.appendChild(row);
      });
      // "+ Add another owner" — only when an unused non-pet candidate remains.
      const usedIds = new Set(petOwnerList.filter(Boolean));
      const canAdd = others.some((p) => notPet(p) && !usedIds.has(p.id));
      if (canAdd) {
        petOwnersHost.appendChild(el("button", {
          class: "hrel-add", type: "button",
          onclick: () => {
            petOwnerList.push("");
            rebuildPetOwnerRows(petOwnerList.length - 1);
          }
        }, [
          el("i", { class: "fa-solid fa-plus" }),
          el("span", null, I18n.t("form.addPetOwner") || "Add another owner")
        ]));
      }
      if (focusRow != null && pickers.length) {
        const target = pickers[Math.min(focusRow, pickers.length - 1)];
        if (target && target.focus) setTimeout(() => { try { target.focus(); } catch (_) {} }, 0);
      }
    }
    // Populate initially if this record is already a pet (edit case); otherwise
    // the rows build lazily when the toggle is switched on.
    if (draft.isPet) rebuildPetOwnerRows();

    // Build one parent slot: label + picker + an "Unknown" toggle. Checking
    // Unknown means "this parent is known to exist but unidentified" (adoption,
    // a lost line) — it's mutually exclusive with a chosen person, so it clears
    // and visually disables that slot's picker (HeritageSelect has no disabled
    // API, hence the `--unknown` class dims it + blocks pointer events). The
    // reverse exclusion — picking a person un-checks Unknown — lives in the
    // picker onChange via clearUnknownRole().
    function parentSlot(role, labelText, picker, onClearPicked) {
      const cb = el("input", { type: "checkbox" });
      cb.checked = unknownRoles.has(role);
      const wrap = el("div", { class: "field parent-slot" }, [
        el("span", { class: "field__label" }, labelText),
        el("div", { class: "parent-slot__picker" }, [picker.el])
      ]);
      if (cb.checked) wrap.classList.add("parent-slot--unknown");
      cb.addEventListener("change", () => {
        if (cb.checked) {
          unknownRoles.add(role);
          wrap.classList.add("parent-slot--unknown");
          // Clear whatever was picked in this slot — the two states can't coexist.
          picker.setValue("");
          onClearPicked();
        } else {
          unknownRoles.delete(role);
          wrap.classList.remove("parent-slot--unknown");
        }
        syncUnknownParents();
      });
      wrap.appendChild(el("label", { class: "parent-unknown-toggle", title: I18n.t("form.parentUnknownHint") }, [
        cb,
        el("i", { class: "fa-solid fa-user-slash", "aria-hidden": "true" }),
        el("span", null, I18n.t("form.parentUnknown") || "Unknown")
      ]));
      unknownSlots[role] = { cb, wrap };
      return wrap;
    }

    const parentsControl = others.length
      ? el("div", { class: "form-grid" }, [
          parentSlot("father", I18n.t("form.father") || "Father", fatherPicker, () => {
            fatherId = "";
            motherPicker.setOptions(relationOptions(motherFilter, I18n.t("form.motherNone") || "— None —"));
            recomputeParents();
            refreshSpouseRowsSafe();
          }),
          parentSlot("mother", I18n.t("form.mother") || "Mother", motherPicker, () => {
            motherId = "";
            fatherPicker.setOptions(relationOptions(fatherFilter, I18n.t("form.fatherNone") || "— None —"));
            recomputeParents();
            refreshSpouseRowsSafe();
          })
        ])
      : el("div", { class: "relations-list" }, []);

    const spousesControl = others.length
      ? spouseRowsHost
      : el("div", { class: "relations-list" }, []);

    // ----- Body layout -----
    // pair(label, en, hi, hint) renders an English field and its Hindi twin.
    // The Hindi half is collapsed behind a "+ हिन्दी" toggle unless it already
    // holds a value (editing an existing record) or the UI itself is in Hindi
    // — so English-only entry doesn't pay the doubled two-column card height on
    // every field. Mirrors moreDetails' content-keyed progressive disclosure.
    function pair(label, en, hi, hint) {
      const enField = field(label, en, hint);
      const hiField = el("label", { class: "field field--hi" }, [
        el("span", { class: "field__label field__label--hi" }, [
          el("span", { lang: "hi" }, "हिन्दी"),
          el("span", { class: "field__label-tag" }, I18n.t("form.optional"))
        ]),
        hi
      ]);

      const hasHi = !!(hi && typeof hi.value === "string" && hi.value.trim());
      const startOpen = hasHi || I18n.getLang() === "hi";

      const wrap = el("div", { class: "field-pair" + (startOpen ? "" : " field-pair--solo") }, [enField]);
      if (startOpen) {
        wrap.appendChild(hiField);
      } else {
        const addHi = el("button", {
          class: "field-pair__add-hi", type: "button",
          "aria-label": I18n.t("form.addHindiAria", { label }),
          onclick: () => {
            wrap.classList.remove("field-pair--solo");
            wrap.replaceChild(hiField, addHi);
            try { hi.focus(); } catch (_) {}
          }
        }, [
          el("i", { class: "fa-solid fa-plus", "aria-hidden": "true" }),
          el("span", { lang: "hi" }, I18n.t("form.addHindi"))
        ]);
        wrap.appendChild(addHi);
      }
      return wrap;
    }

    const nameField = pair(I18n.t("form.name"), nameInput, nameHiInput, I18n.t("form.required"));
    // Attach the name error span into the English half of the name pair.
    nameField.querySelector(".field")?.appendChild(makeErrorSpan("name"));

    // Essentials — always visible. The identity a record can't do without:
    // photo, name, dates, gender. Everything else lives under More details so
    // a new person isn't a flat wall of ~15 groups.
    const essentials = el("div", { class: "form-stack" }, [
      photoUploader,
      nameField,
      el("div", { class: "form-grid" }, [
        dateWithPrecisionField(I18n.t("form.birthDate"), "birth", birthDateInput, birthPrecPicker, I18n.t("form.datePlaceholder")),
        dateWithPrecisionField(I18n.t("form.deathDate"), "death", deathDateInput, deathPrecPicker, I18n.t("form.deathDatePlaceholder"))
      ]),
      field(I18n.t("form.gender"), genderSelect)
    ]);

    // More details — the rest, folded into a native <details> disclosure so
    // it's keyboard-accessible with no JS. Opens by default when editing a
    // record that already has data in these fields (so nothing is hidden), and
    // stays collapsed for a brand-new person.
    const moreInner = el("div", { class: "form-stack" }, [
      pair(I18n.t("form.birthPlace"), birthPlaceInput, birthPlaceHiInput),
      pair(I18n.t("form.deathPlace"), deathPlaceInput, deathPlaceHiInput),
      pair(I18n.t("form.occupation"), occupationInput, occupationHiInput),
      pair(I18n.t("form.description"), descriptionInput, descriptionHiInput, I18n.t("form.descriptionHint")),
      field(I18n.t("form.gallery"), galleryEditor, I18n.t("form.galleryHint")),
      field(I18n.t("form.documents"), documentsEditor, I18n.t("form.documentsHint")),
      pair(I18n.t("form.achievements"), achievementsInput, achievementsHiInput, I18n.t("form.achievementsHint")),
      pair(I18n.t("form.education"), educationInput, educationHiInput, I18n.t("form.educationHint")),
      others.length
        ? el("div", { class: "field" }, [
            el("span", { class: "field__label" }, I18n.t("form.parents")),
            parentsControl
          ])
        : field(I18n.t("form.parents"), parentsControl, I18n.t("form.relationsEmpty")),
      others.length
        ? el("div", { class: "field" }, [
            el("span", { class: "field__label" }, I18n.t("form.spouses")),
            spousesControl
          ])
        : field(I18n.t("form.spouses"), spousesControl),
      pair(I18n.t("form.notes"), notesInput, notesHiInput),
      field(I18n.t("form.contact"), contactRows, I18n.t("form.contactHint")),
      el("div", { class: "field" }, [petToggle]),
      petOwnersField
    ]);
    const hasMoreData = !!(draft.birthPlace || draft.birthPlace_hi || draft.deathPlace || draft.deathPlace_hi
      || draft.occupation || draft.occupation_hi || draft.description || draft.description_hi
      || (draft.gallery && draft.gallery.length)
      || (draft.documents && draft.documents.length)
      || (draft.achievements && draft.achievements.length) || (draft.education && draft.education.length)
      || (draft.parents && draft.parents.length) || (draft.unknownParents && draft.unknownParents.length) || (draft.spouses && draft.spouses.length)
      || draft.notes || draft.notes_hi
      || (draft.contact && (draft.contact.phone || draft.contact.email || draft.contact.address))
      || draft.isPet);
    const moreDetails = el("details", { class: "form-more" }, [
      el("summary", { class: "form-more__summary" }, [
        el("i", { class: "fa-solid fa-chevron-right form-more__chev", "aria-hidden": "true" }),
        el("span", null, I18n.t("form.moreDetails"))
      ]),
      moreInner
    ]);
    if (isEdit && hasMoreData) moreDetails.open = true;

    const body = el("div", { class: "form-stack" }, [essentials, moreDetails]);
    // Reflect the initial death-date state (hide precision when "living").
    syncDeathPrecVisibility();

    // ----- Footer -----
    const cancelBtn = UI.cancelBtn(I18n.t("actions.cancel"));
    const saveBtn = UI.saveBtn(I18n.t("actions.save"));
    // "Save & add another" only makes sense while adding — editing an existing
    // person has nothing to "add another" of. Rendered as a secondary button.
    const addAnotherBtn = isEdit ? null : UI.el("button", { class: "btn btn--ghost", type: "button" }, [
      el("i", { class: "fa-solid fa-user-plus", "aria-hidden": "true" }),
      el("span", null, I18n.t("form.saveAddAnother"))
    ]);

    const dlg = openModal({
      title: isEdit
        ? I18n.t("form.editTitle")
        : (seed && seed.parents && seed.parents.length
            ? I18n.t("inspector.addChild")
            : seed && seed.spouses && seed.spouses.length
              ? I18n.t("inspector.addSpouse")
              : seed && seed.__addAsParentOf
                ? I18n.t("inspector.addParent")
                : I18n.t("form.addTitle")),
      body,
      footer: addAnotherBtn ? [cancelBtn, addAnotherBtn, saveBtn] : [cancelBtn, saveBtn],
      // Enter in any single-line field saves — matches every other form on the
      // web. openModal excludes textareas and the date picker's own input (which
      // commits the typed date on its own Enter), so this won't fire mid-date.
      onEnter: () => saveBtn.click(),
      // Unsaved-changes guard. Runs on every dismiss route (Cancel, close-X,
      // Escape, backdrop). If the draft differs from the opening snapshot and
      // the user hasn't saved, veto the close and ask to confirm the discard.
      beforeClose: () => {
        if (committed) return true;                    // a save is never "discarded"
        // Fold in any date typed but not yet committed so the diff is honest.
        if (birthDatePicker && birthDatePicker.flush) birthDatePicker.flush();
        if (deathDatePicker && deathDatePicker.flush) deathDatePicker.flush();
        if (JSON.stringify(draft) === openSnapshot) return true;  // untouched → let it close
        if (discardConfirming) return false;           // confirm already open
        discardConfirming = true;
        UI.confirm({
          title: I18n.t("form.discardTitle"),
          message: I18n.t("form.discardMsg"),
          confirmLabel: I18n.t("actions.discard"),
          danger: true
        }).then((ok) => {
          discardConfirming = false;
          if (ok) dlg.close(true);                     // force past this guard
        });
        return false;
      },
      // Single reconciliation point for photo blobs, covering ALL dismiss
      // routes (Save, Cancel, close-X, Escape, backdrop) since they all funnel
      // through openModal's close(). The blob the surviving record points at is
      // kept; every other blob we touched this session is deleted.
      onClose: () => {
        if (!window.PhotoStore) return;
        // Reconcile ALL blobs touched this session (avatar + gallery) against
        // what survives. On save the survivor set is the saved record's blobs
        // (avatar + every gallery photoId); on any cancel it's the record's
        // original blobs. Delete every session blob not in the keep-set, plus —
        // on save — any original blob the saved record dropped.
        const keep = new Set();
        if (committed) {
          if (draft.photoId) keep.add(draft.photoId);
          (draft.gallery || []).forEach((g) => { if (g && g.photoId) keep.add(g.photoId); });
          (draft.documents || []).forEach((d) => { if (d && d.photoId) keep.add(d.photoId); });
        } else {
          originalBlobs.forEach((id) => keep.add(id));
        }
        const toDelete = new Set(sessionBlobs);
        if (committed) {
          // Any original blob the saved record no longer references is orphaned.
          originalBlobs.forEach((id) => { if (!keep.has(id)) toDelete.add(id); });
        }
        keep.forEach((id) => toDelete.delete(id)); // never delete a still-referenced blob
        toDelete.forEach((id) => { if (id) PhotoStore.delete(id).catch(() => {}); });
      }
    });

    cancelBtn.addEventListener("click", () => dlg.close());

    function focusField(key) {
      if (key === "name") { nameInput.focus(); return; }
      if (key === "birth") { if (birthDatePicker) birthDatePicker.focus(); else birthDateInput.focus && birthDateInput.focus(); return; }
      if (key === "death") { if (deathDatePicker) deathDatePicker.focus(); else deathDateInput.focus && deathDateInput.focus(); }
    }
    // Show an inline error on a field and focus it. Death-date errors live
    // under More details, so open that disclosure first or the message is
    // invisible. Returns false so callers can `return fail(...)`.
    function fail(key, msg) {
      setFieldError(key, msg);
      if (key === "death" && moreDetails && !moreDetails.open) moreDetails.open = true;
      focusField(key);
      return false;
    }

    // Validate + persist the draft. Returns the saved record on success, or
    // null on any validation/read-only/exception failure (having already shown
    // the inline error or toast). Shared by "Save" and "Save & add another" so
    // the two entry points can't drift.
    function commitDraft() {
      // Commit any date the user typed but didn't confirm with Enter — the
      // blur→commit can be deferred on Safari/Firefox and would otherwise run
      // after this handler, silently dropping the typed date.
      if (birthDatePicker && birthDatePicker.flush) birthDatePicker.flush();
      if (deathDatePicker && deathDatePicker.flush) deathDatePicker.flush();
      clearFieldError("name"); clearFieldError("birth"); clearFieldError("death");

      // A person needs a name in EITHER script — someone recording only in
      // Hindi shouldn't be forced to romanise. If the Latin name is blank we
      // resolve the Hindi one into `name` so every downstream reader (which
      // keys off `name`) still works.
      let name = (draft.name || "").trim();
      const nameHi = (draft.name_hi || "").trim();
      if (!name && nameHi) name = nameHi;
      if (!name) { fail("name", I18n.t("form.nameRequired")); return null; }

      const birth = draft.birthDate ? FamilyStore.parseDate(draft.birthDate) : null;
      if (draft.birthDate && !birth) { fail("birth", I18n.t("form.dateInvalid")); return null; }
      const death = draft.deathDate ? FamilyStore.parseDate(draft.deathDate) : null;
      if (draft.deathDate && !death) { fail("death", I18n.t("form.dateInvalid")); return null; }

      // Sanity: birth can't come after death, and neither can be in the future.
      // Compare by year so a same-year birth/death (common when only years are
      // known) isn't flagged by the Jan-1 normalisation of a bare "YYYY".
      const nowYear = new Date().getFullYear();
      if (birth && birth.getFullYear() > nowYear) { fail("birth", I18n.t("form.dateFutureInvalid")); return null; }
      if (death && death.getFullYear() > nowYear) { fail("death", I18n.t("form.dateFutureInvalid")); return null; }
      if (birth && death && birth.getFullYear() > death.getFullYear()) {
        fail("death", I18n.t("form.dateOrderInvalid")); return null;
      }

      const payload = {
        name,
        name_hi: draft.name_hi || "",
        photo: draft.photo || null,
        photoId: draft.photoId || null,
        photoCropAvatar: draft.photoCropAvatar || null,
        photoCropHero: draft.photoCropHero || null,
        // Additional photos. Persist a clean copy (id/photoId/photo/caption);
        // normalizePerson drops any entry with no usable photo ref.
        gallery: (draft.gallery || []).map((g) => ({ id: g.id, photoId: g.photoId || null, photo: g.photo || null, caption: (g.caption || "").trim() })),
        // Source documents. normalizePerson drops any entry with no scan ref.
        documents: (draft.documents || []).map((d) => ({ id: d.id, photoId: d.photoId || null, photo: d.photo || null, title: (d.title || "").trim(), kind: d.kind || "other", date: d.date || null, note: (d.note || "").trim() })),
        birthDate: draft.birthDate || null,
        birthDatePrecision: draft.birthDate ? (draft.birthDatePrecision || "exact") : null,
        deathDate: draft.deathDate || null,
        deathDatePrecision: draft.deathDate ? (draft.deathDatePrecision || "exact") : null,
        birthPlace: draft.birthPlace || "",
        birthPlace_hi: draft.birthPlace_hi || "",
        deathPlace: draft.deathPlace || "",
        deathPlace_hi: draft.deathPlace_hi || "",
        gender: draft.gender || null,
        notes: draft.notes || "",
        notes_hi: draft.notes_hi || "",
        occupation: draft.occupation || "",
        occupation_hi: draft.occupation_hi || "",
        description: draft.description || "",
        description_hi: draft.description_hi || "",
        achievements: (draft.achievements || []).slice(),
        achievements_hi: (draft.achievements_hi || []).slice(),
        education: (draft.education || []).slice(),
        education_hi: (draft.education_hi || []).slice(),
        parents: draft.parents.slice(),
        unknownParents: draft.unknownParents ? draft.unknownParents.slice() : [],
        spouses: draft.spouses.slice(),
        contact: {
          phone: (draft.contact.phone || "").trim(),
          email: (draft.contact.email || "").trim(),
          address: (draft.contact.address || "").trim(),
          privatePhone: !!draft.contact.privatePhone,
          privateEmail: !!draft.contact.privateEmail,
          privateAddress: !!draft.contact.privateAddress
        },
        isPet: !!draft.isPet,
        petOwners: draft.isPet ? (draft.petOwners || []).filter(Boolean) : []
      };

      try {
        let saved;
        if (isEdit) saved = FamilyStore.updatePerson(draft.id, payload);
        else saved = FamilyStore.addPerson(payload);
        // Both add/updatePerson return null under read-only (and updatePerson
        // also returns null for a vanished id). Never claim success on a null —
        // otherwise a viewer, or an editor whose role was revoked mid-form, sees
        // a "Saved" toast for a write that never persisted.
        if (!saved) {
          toast(I18n.t("people.readOnly"), "warning");
          return null;
        }
        // If we were asked to attach this new person as a parent of someone,
        // do that now that we have an id.
        if (!isEdit && draft.__addAsParentOf) {
          const child = FamilyStore.getPerson(draft.__addAsParentOf);
          if (child) {
            const parents = (child.parents || []).slice();
            // A child has at most two parents. Without this cap, "Save & add
            // another" from the add-parent path reopens with the SAME
            // __addAsParentOf seed, so a 3rd/4th "parent" gets pushed onto the
            // child's parents array (dedupe-only) with no slot in the child's
            // own form to see or remove it — a silent, orphaned link. Cap here,
            // and warn instead of silently dropping the link.
            if (parents.includes(saved.id)) {
              // already linked — nothing to do
            } else if (parents.length >= 2) {
              const childName = FamilyStore.getField(child, "name") || child.name;
              toast(I18n.t("form.parentsFull", { name: childName }), "warning");
            } else {
              parents.push(saved.id);
              // Adding a real parent contradicts an "unknown" marker for that
              // same role — clear it so the child can't hold both a named
              // father and "father unknown". Match the marker by the new
              // parent's gender (male→father, female→mother); an unspecified
              // gender leaves any marker untouched (we can't know which slot).
              const patch = { parents };
              const clearRole = saved.gender === "m" ? "father" : saved.gender === "f" ? "mother" : null;
              if (clearRole && Array.isArray(child.unknownParents) && child.unknownParents.includes(clearRole)) {
                patch.unknownParents = child.unknownParents.filter((r) => r !== clearRole);
              }
              FamilyStore.updatePerson(child.id, patch);
            }
          }
        }
        // Persisted successfully — tell the onClose reconciler to keep the
        // saved blob and free the superseded ones. Must be set before any
        // close(), which fires onClose synchronously.
        committed = true;
        return saved;
      } catch (err) {
        toast(err.message || I18n.t("form.saveFailed"), "danger");
        return null;
      }
    }

    // "Save": persist, close, and reveal the new node on the canvas.
    saveBtn.addEventListener("click", () => {
      const saved = commitDraft();
      if (!saved) return;
      dlg.close();
      toast(I18n.t("form.saved"), "success");
      // After adding a NEW person, select them and pan the tree so the fresh
      // node is never left off-screen. revealPerson() selects via Inspector
      // and pans/zooms if the tree is showing; fall back to plain selection
      // when TreeView isn't available (e.g. a minimal build).
      if (!isEdit) {
        if (window.TreeView && TreeView.revealPerson) TreeView.revealPerson(saved.id);
        else if (window.Inspector && Inspector.show) Inspector.show(saved.id);
      }
    });

    // "Save & add another" (new-person forms only): persist, then reopen a
    // blank form seeded like this one — so entering a whole branch in one
    // sitting doesn't pay a tree-reveal + Inspector context switch per person.
    // Passes the original seed so add-child/-spouse/-parent flows keep their
    // relationship context for the next entry.
    if (addAnotherBtn) {
      addAnotherBtn.addEventListener("click", () => {
        const saved = commitDraft();
        if (!saved) return;
        dlg.close();
        toast(I18n.t("form.savedAddAnother"), "success");
        // Carry the seed forward — EXCEPT drop __addAsParentOf once the target
        // child's two parent slots are full, so the next entry isn't offered as
        // a phantom 3rd parent (commitDraft caps it anyway, but this avoids the
        // repeated "already has two parents" warning on each subsequent save).
        let nextSeed = seed;
        if (seed && seed.__addAsParentOf) {
          const child = FamilyStore.getPerson(seed.__addAsParentOf);
          if (!child || (child.parents || []).length >= 2) {
            nextSeed = Object.assign({}, seed);
            delete nextSeed.__addAsParentOf;
          }
        }
        openForm(null, nextSeed);
      });
    }

    setTimeout(() => nameInput.focus(), 50);
  }

  // ===== Delete =====

  async function deletePerson(id) {
    // Same hard gate as openForm — viewers can't delete. FamilyStore.deletePerson
    // no-ops under read-only, so without this a viewer would see a "Removed"
    // success toast for a delete that never happened.
    if (window.FamilyStore && FamilyStore.isReadOnly && FamilyStore.isReadOnly()) {
      toast(I18n.t("people.readOnly"), "warning");
      return;
    }
    const person = FamilyStore.getPerson(id);
    if (!person) return;
    const displayName = FamilyStore.getField(person, "name") || person.name;
    // A record with a death date is the highest-stakes data in the tree — the
    // person can't be re-interviewed. Deleting it gets a distinct, heavier
    // confirm than removing a living entry.
    const deceased = FamilyStore.isDeceased && FamilyStore.isDeceased(person);
    const ok = await confirm(deceased ? {
      title: I18n.t("actions.memoriamDeleteTitle"),
      message: I18n.t("actions.memoriamDeleteMsg", { name: displayName }),
      confirmLabel: I18n.t("actions.remove"),
      danger: true
    } : {
      title: I18n.t("form.deleteTitle") + " — " + displayName,
      message: I18n.t("form.deleteMsg"),
      confirmLabel: I18n.t("actions.remove"),
      danger: true
    });
    if (!ok) return;
    // Blob cleanup (own photo + marriage photos + petOwners strip) is owned by
    // FamilyStore.deletePerson now — don't duplicate it here.
    FamilyStore.deletePerson(id);
    toast(I18n.t("form.removed"), "success");
  }

  if (window.I18n && I18n.onChange) I18n.onChange(() => { if (root) render(); });

  global.PeopleView = { mount, render, openForm, setSearch, setFilter, setMissingFilter, reset, mountHeaderScope };
})(window);
