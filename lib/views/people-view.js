...
(function (global) {
  ...
  function openForm(id, seed) {
    ...
    const fatherFilter = (p) => p.id !== motherId && isMale(p) && !descendantIds.has(p.id);
    const motherFilter = (p) => p.id !== fatherId && isFemale(p) && !descendantIds.has(p.id);
    ...
    const body = el("div", { class: "form-stack" }, [
      photoUploader,
      pair(I18n.t("form.name"), nameInput, nameHiInput, I18n.t("form.required")),
      el("div", { class: "form-grid" }, [
        dateWithPrecisionField(I18n.t("form.birthDate"), birthDateInput, birthPrecPicker, I18n.t("form.datePlaceholder")),
        dateWithPrecisionField(I18n.t("form.deathDate"), deathDateInput, deathPrecPicker, I18n.t("form.deathDatePlaceholder"))
      ]),
