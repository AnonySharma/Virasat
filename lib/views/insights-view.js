...
(function (global) {
  ...
  // Reveal a person from any anniversary surface (Coming-up row, calendar day):
  // put them on the tree canvas AND open the profile, the same seam Timeline/
  ...
  // One clickable person row, shared by the Coming-up list, the "+N more" modal,
  // and a calendar day tap. `onClick` defaults to opening the person; the modal
  // passes a variant that closes itself first so the tree reveal is visible.
  function eventRow(ev, onClick) {
    ...
    row.addEventListener("click", onClick || (() => openPerson(ev.person)));
    return row;
  }

  // Open a modal listing a set of anniversary events as clickable rows. Used by
  // the Coming-up "+N more" (whole year ahead) and by a calendar day with events
  // — both would overflow their panel inline, so they overflow into a scrollable
  // sheet instead. Each row closes the modal, then reveals its person.
  function openEventsModal(title, evs) {
    if (!window.UI || !UI.openModal || !evs || !evs.length) return;
    let handle = null;
    const rows = evs.map((ev) => eventRow(ev, () => { if (handle) handle.close(); openPerson(ev.person); }));
    const closeBtn = el("button", { class: "btn btn--primary", type: "button" }, I18n.t("actions.close"));
    handle = UI.openModal({
      title: title,
      body: el("div", { class: "coming-up__list coming-up__list--modal" }, rows),
      footer: [closeBtn]
    });
  ...
  function comingUpPanel() {
    ...
    const rows = evs.slice(0, COMING_UP_MAX).map((ev) => eventRow(ev));

    // "+N more" opens the whole year ahead in a scrollable modal rather than
    // stretching the panel — the panel stays a fixed-height glance.
    const hiddenCount = Math.max(0, evs.length - COMING_UP_MAX);
    if (hiddenCount) {
      const moreBtn = el("button", { class: "coming-up__more", type: "button" },
        I18n.t("insights.comingUpMore", { n: hiddenCount }));
      moreBtn.addEventListener("click", () => openEventsModal(I18n.t("insights.comingUpTitle"), evs));
      rows.push(moreBtn);
    }
  ...
  // "Family calendar" — a compact month grid (mac-style) marking every birthday
  // and memorial from the same whole-family Anniversaries source as "Coming up".
  // A marked day carries a small type icon (cake for a birthday, feather for a
  // memorial); tapping it opens the shared events modal for that day rather than
  // expanding inline, so the grid stays a fixed-size glance. Prev/next walk
  // months; the offset is sticky across re-renders so a store change / language
  // switch doesn't reset the view.
  let calMonthOffset = 0;      // displayed month relative to the current one
  function calPad2(n) { return String(n).padStart(2, "0"); }
  function calendarPanel() {
    if (!global.Anniversaries) return null;
    const evs = Anniversaries.events();
    const head = el("h3", { class: "insight-panel__title" }, I18n.t("insights.calendarTitle"));

    // Bucket every anniversary by month-day. events() spans the next 365 days,
    // so each recurring date appears exactly once — bucketing by MM-DD covers
    // any month we might display, with no duplicates.
    const byKey = {};
    evs.forEach((ev) => {
    ...
    const firstWeekday = shown.getDay();                  // 0 = Sunday
    ...
    // Month nav — prev / label / next, plus a "This month" reset shown only when
    // we've walked away from the current month.
    const prevBtn = el("button", { class: "cal-nav__btn", type: "button", "aria-label": I18n.t("insights.calendarPrev") },
      [el("i", { class: "fa-solid fa-chevron-left", "aria-hidden": "true" })]);
    const nextBtn = el("button", { class: "cal-nav__btn", type: "button", "aria-label": I18n.t("insights.calendarNext") },
      [el("i", { class: "fa-solid fa-chevron-right", "aria-hidden": "true" })]);
    prevBtn.addEventListener("click", () => { calMonthOffset--; render(); });
    nextBtn.addEventListener("click", () => { calMonthOffset++; render(); });
    const navChildren = [prevBtn, el("span", { class: "cal-nav__label" }, monthLabel), nextBtn];
    if (!isCurrentMonth) {
      const todayBtn = el("button", { class: "cal-nav__today", type: "button" }, I18n.t("insights.calendarThisMonth"));
      todayBtn.addEventListener("click", () => { calMonthOffset = 0; render(); });
      navChildren.push(todayBtn);
    }
    const nav = el("div", { class: "cal-nav" }, navChildren);

    // Weekday header.
    const weekdays = I18n.t("insights.calendarWeekdays");
    const wdRow = el("div", { class: "cal-weekdays" },
      (Array.isArray(weekdays) ? weekdays : ["S", "M", "T", "W", "T", "F", "S"])
        .map((w) => el("span", { class: "cal-weekday" }, w)));

    // Day cells: leading blanks to align the 1st under its weekday, then each
    // day — a real button where something is marked, a plain cell otherwise. A
    // marked day shows the day number plus one small icon per kind present.
    const cells = [];
    for (let i = 0; i < firstWeekday; i++) cells.push(el("span", { class: "cal-cell cal-cell--blank" }));
    let monthHasMarks = false;
    for (let d = 1; d <= daysInMonth; d++) {
      const key = monthKey + "-" + calPad2(d);
      const dayEvs = byKey[key] || [];
      const isToday = isCurrentMonth && d === todayDate;
      const inner = [el("span", { class: "cal-cell__num" }, String(d))];

      if (dayEvs.length) {
        monthHasMarks = true;
        const icons = [];
        if (dayEvs.some((e) => e.kind !== "death"))
          icons.push(el("i", { class: "fa-solid fa-cake-candles cal-cell__icon cal-cell__icon--birth", "aria-hidden": "true" }));
        if (dayEvs.some((e) => e.kind === "death"))
          icons.push(el("i", { class: "fa-solid fa-feather cal-cell__icon cal-cell__icon--death", "aria-hidden": "true" }));
        inner.push(el("span", { class: "cal-cell__icons" }, icons));

        const dayLabel = Anniversaries.dateLabel(year + "-" + key);
        const cell = el("button", {
          class: "cal-cell cal-cell--marked" + (isToday ? " is-today" : ""),
          type: "button",
          title: I18n.t("insights.calendarDayMarked", { date: dayLabel, n: dayEvs.length })
        }, inner);
        cell.addEventListener("click", () => openEventsModal(dayLabel, byKey[key] || []));
        cells.push(cell);
      } else {
        cells.push(el("span", { class: "cal-cell" + (isToday ? " is-today" : "") }, inner));
      }
    }
    const grid = el("div", { class: "cal-grid" }, cells);

    const children = [head, nav, wdRow, grid];

    // A month with nothing marked reads as broken without a word — say so.
    if (!monthHasMarks) children.push(el("p", { class: "cal-nothing" }, I18n.t("insights.calendarNothing")));

    const legend = el("div", { class: "cal-legend" }, [
      el("span", { class: "cal-legend__item" }, [
        el("i", { class: "fa-solid fa-cake-candles cal-cell__icon cal-cell__icon--birth", "aria-hidden": "true" }),
        el("span", null, I18n.t("insights.calendarBirthdayLegend"))
      ]),
      el("span", { class: "cal-legend__item" }, [
        el("i", { class: "fa-solid fa-feather cal-cell__icon cal-cell__icon--death", "aria-hidden": "true" }),
        el("span", null, I18n.t("insights.calendarMemorialLegend"))
      ])
    ]);
    children.push(legend);

    return el("div", { class: "insight-panel insight-panel--wide insight-panel--calendar" }, children);
  }
  ...
  function render() {
    ...
    // born-by-decade, name trends, roots, elders.
    ...
    // The whole-family "coming up" and the month calendar are full-width bands,
    // stacked in their own flow — kept out of the multicol above so the column
    // balancer can't wedge a tall gap beside them.
    body.appendChild(el("div", { class: "insight-bands" }, [
      comingUpPanel(),
      calendarPanel()
    ]));
  }
