...
(function (global) {
  ...
  function comingUpPanel() {
    ...
    // Each row opens its person (shared openPerson above). A row is a real
    // button for keyboard + screen-reader access.
    const rowFor = (ev) => {
  ...
  // "Family calendar" — a month grid marking every birthday (gold) and memorial
  // (olive) from the same whole-family Anniversaries source as "Coming up".
  // Prev/next walk months; clicking a marked day expands its people below the
  // grid, each clickable to open on the tree. Both offsets are sticky across
  // re-renders so a store change / language switch doesn't reset the view.
  let calMonthOffset = 0;      // displayed month relative to the current one
  let calSelectedKey = null;   // "MM-DD" of the expanded day, or null
  ...
  function calendarPanel() {
    ...
    prevBtn.addEventListener("click", () => { calMonthOffset--; calSelectedKey = null; render(); });
    nextBtn.addEventListener("click", () => { calMonthOffset++; calSelectedKey = null; render(); });
    ...
    if (!isCurrentMonth) {
      ...
      todayBtn.addEventListener("click", () => { calMonthOffset = 0; calSelectedKey = null; render(); });
    ...
    // day — a real button where something is marked, a plain cell otherwise.
    const cells = [];
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const dots = [];
      if (dayEvs.some((e) => e.kind !== "death")) dots.push(el("span", { class: "cal-dot cal-dot--birth" }));
      if (dayEvs.some((e) => e.kind === "death")) dots.push(el("span", { class: "cal-dot cal-dot--death" }));
      const inner = [el("span", { class: "cal-cell__num" }, String(d))];
      if (dots.length) inner.push(el("span", { class: "cal-cell__dots" }, dots));

      if (dayEvs.length) {
        monthHasMarks = true;
        const sel = calSelectedKey === key;
        const cell = el("button", {
          class: "cal-cell cal-cell--marked" + (isToday ? " is-today" : "") + (sel ? " is-selected" : ""),
          type: "button",
          "aria-pressed": sel ? "true" : "false",
          title: I18n.t("insights.calendarDayMarked", { date: Anniversaries.dateLabel(year + "-" + key), n: dayEvs.length })
        }, inner);
        cell.addEventListener("click", () => { calSelectedKey = sel ? null : key; render(); });
        cells.push(cell);
      } else {
    ...
    // Selected-day detail: that day's people as clickable rows (shared openPerson,
    // same row styling as "Coming up"). Guarded so a stale selection after a data
    // change silently collapses instead of throwing.
    if (calSelectedKey && byKey[calSelectedKey]) {
      const detail = el("div", { class: "cal-detail" },
        byKey[calSelectedKey].map((ev) => {
          const row = el("button", {
            class: "coming-up__row",
            type: "button",
            title: I18n.t("insights.comingUpOpen", { name: Anniversaries.eventTitle(ev) })
            ...
            el("div", { class: "coming-up__body" }, [
              el("div", { class: "coming-up__title" }, Anniversaries.eventTitle(ev)),
              el("div", { class: "coming-up__meta" }, Anniversaries.eventMeta(ev) || Anniversaries.dateLabel(ev.date))
            ])
          ]);
          row.addEventListener("click", () => openPerson(ev.person));
          return row;
  ...
  function render() {
    ...
    // Panels: gender ratio, born-by-decade, name trends, roots, elders, the
    // whole-family "coming up", then the month calendar (both wide, spanning all
    // columns, and both carry their own chrome so they sit as full-width bands).
