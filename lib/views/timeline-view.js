 ...
 *
 * Exposes window.TimelineView = { mount(rootEl), render, setFilter }.
...
(function (global) {
  ...
  // Minimap: how many stacked lanes the tiny bars cycle through, and each
  // lane's vertical step in px. LANES × LANE_H (+ padding) sets the strip height
  // in the CSS; keep them in sync with .timeline-minimap there.
  const MINIMAP_LANES = 4;
  const MINIMAP_LANE_H = 5;
  ...
  let minimapGroup = null;   // caption + strip wrapper (hidden when nothing overflows)
  let minimap = null;        // the strip container (hidden when nothing overflows)
  let minimapTrack = null;   // holds the tiny lifespan bars + now tick
  let minimapWindow = null;  // the draggable viewport rectangle
  ...
  let lastSig = null;
  let mqList = null;
  // Last computed range + rows, kept so the minimap's drag/keyboard handlers can
  // map viewport ↔ year-fraction without recomputing.
  ...
  function mount(rootEl) {
    ...
    // Keep the minimap viewport rectangle in lockstep with native scrolling.
    scroll.addEventListener("scroll", syncMinimapWindow, { passive: true });

    // Minimap — a compact overview of the whole span with a draggable viewport
    // window. For trees covering many generations the main canvas is far wider
    // than the screen; this gives a one-glance "where am I / jump there" strip.
    // Coordinates are percentages of the total year-range, so it's independent
    // of pxPerYear and never needs its own pixel math kept in sync with zoom.
    minimapWindow = el("div", { class: "timeline-minimap__window" });
    minimapTrack = el("div", { class: "timeline-minimap__track" }, [minimapWindow]);
    minimap = el("div", {
      class: "timeline-minimap",
      role: "slider",
      tabindex: "0",
      "aria-label": I18n.t("timeline.minimapLabel"),
      "data-i18n-aria-label": "timeline.minimapLabel"
    }, [minimapTrack]);
    minimap.addEventListener("pointerdown", onMinimapPointerDown);
    minimap.addEventListener("keydown", onMinimapKey);
    // Scrolling anywhere over the strip pans the timeline horizontally — the
    // strip itself has nothing to scroll, so without this a wheel/trackpad
    // gesture over it does nothing (the reported "scroll doesn't work here").
    minimap.addEventListener("wheel", onMinimapWheel, { passive: false });

    // Caption so the strip reads as a navigator, not mystery chrome. Grouped
    // with the strip so both hide together when nothing overflows.
    const minimapCaption = el("div", {
      class: "timeline-minimap__caption",
      "data-i18n": "timeline.minimapHint"
    }, I18n.t("timeline.minimapHint"));
    minimapGroup = el("div", { class: "timeline-minimap-group", hidden: true }, [minimapCaption, minimap]);

    // The overview sits directly under the zoom controls, above the scroll
    // area: read top-down as "here's the whole span → drag to jump → the
    // timeline below follows."
    const wrap = el("div", { class: "timeline-wrap" }, [controls, minimapGroup, scroll]);
  ...
  function render() {
    ...
    if (!range) {
      ...
      lastSig = null;
      currentRange = null;
      if (minimapGroup) minimapGroup.hidden = true;
      return;
    }
    currentRange = range;
    ...
    lastSig = sig;
    applyFilterClasses();
    renderMinimap(rows, range);
  }

  // Draw the tiny lifespan bars and position the viewport window. Each bar is
  // placed by percentage across the total year-span, so the minimap needs no
  // pxPerYear math and stays correct across zoom. Shown only when the canvas
  // actually overflows its viewport (otherwise there's nothing to navigate).
  function renderMinimap(rows, range) {
    if (!minimap || !minimapTrack || !scroll) return;
    const overflowing = scroll.scrollWidth - scroll.clientWidth > 4;
    if (!rows.length || !overflowing) { if (minimapGroup) minimapGroup.hidden = true; return; }
    if (minimapGroup) minimapGroup.hidden = false;

    // Rebuild the bars but keep the persistent window element.
    while (minimapTrack.firstChild && minimapTrack.firstChild !== minimapWindow) {
      minimapTrack.removeChild(minimapTrack.firstChild);
    }
    const span = range.maxYear - range.minYear || 1;
    const frag = document.createDocumentFragment();
    rows.forEach((r, i) => {
      const startPct = ((r.startYear - range.minYear) / span) * 100;
      const widthPct = Math.max(0.4, ((r.endYear - r.startYear) / span) * 100);
      const deceased = FamilyStore.isDeceased(r.person);
      frag.appendChild(el("div", {
        class: "timeline-minimap__bar" + (deceased ? " is-deceased" : ""),
        style: {
          left: startPct + "%",
          width: widthPct + "%",
          // Stack bars into a few lanes so a tall tree still reads as a band
          // rather than one overwritten line; cheap modulo lanes are enough.
          top: (2 + (i % MINIMAP_LANES) * MINIMAP_LANE_H) + "px"
        }
      }));
    });
    // "Now" tick at today's position.
    const nowPct = ((range.currentYear - range.minYear) / span) * 100;
    frag.appendChild(el("div", { class: "timeline-minimap__now", style: { left: nowPct + "%" } }));
    minimapTrack.insertBefore(frag, minimapWindow);

    syncMinimapWindow();
  }

  // Position the viewport rectangle to mirror the horizontal scroll fraction.
  // The minimap track spans the year-range only; the name column is a sticky
  // gutter (width ncw) that overlays the viewport's left edge and isn't part of
  // the year-span, so it's subtracted from both the scrollable content and the
  // visible width before converting to a fraction of the timeline proper.
  function syncMinimapWindow() {
    if (!minimap || (minimapGroup && minimapGroup.hidden) || !minimapWindow || !scroll) return;
    const ncw = nameColWidth();
    const contentW = scroll.scrollWidth - ncw;   // year-span width in px
    const viewW = scroll.clientWidth - ncw;       // visible year-span width in px
    if (contentW <= 0 || viewW <= 0) return;
    const startFrac = UI.clamp(scroll.scrollLeft / contentW, 0, 1);
    const widthFrac = UI.clamp(viewW / contentW, 0, 1);
    minimapWindow.style.left = (startFrac * 100) + "%";
    minimapWindow.style.width = (widthFrac * 100) + "%";
  }

  // Map a clientX over the minimap track (a fraction of the year-span) to a
  // scrollLeft that centres the visible window on that fraction. Shared by
  // click and drag. Inverse of the fraction math in syncMinimapWindow.
  function scrollToMinimapX(clientX) {
    if (!minimapTrack || !scroll) return;
    const rect = minimapTrack.getBoundingClientRect();
    if (!rect.width) return;
    const ncw = nameColWidth();
    const contentW = scroll.scrollWidth - ncw;
    const viewW = scroll.clientWidth - ncw;
    const frac = UI.clamp((clientX - rect.left) / rect.width, 0, 1);
    // Centre the window: put `frac` at the middle of the visible year-span.
    const target = frac * contentW - viewW / 2;
    scroll.scrollLeft = Math.max(0, target);
    syncMinimapWindow();
  }

  // Wheel over the strip pans the timeline horizontally. Trackpads send the
  // intended horizontal delta on deltaX; a plain mouse wheel only has deltaY,
  // so fall back to it — either axis moves along the lifeline. Keep the window
  // rectangle in step, and preventDefault so the page/rows don't also scroll.
  function onMinimapWheel(e) {
    if (!scroll || (minimapGroup && minimapGroup.hidden)) return;
  ...
  function onMinimapPointerDown(e) {
    if (!minimap || (minimapGroup && minimapGroup.hidden)) return;
  ...
  // Arrow keys nudge the viewport a page-ish step; Home/End jump to the extremes.
  function onMinimapKey(e) {
    if (!scroll) return;
    const step = scroll.clientWidth * 0.5;
    let handled = true;
    switch (e.key) {
      case "ArrowLeft": scroll.scrollLeft -= step; break;
      case "ArrowRight": scroll.scrollLeft += step; break;
      case "Home": scroll.scrollLeft = 0; break;
      case "End": scroll.scrollLeft = scroll.scrollWidth; break;
      default: handled = false;
    }
    if (handled) { e.preventDefault(); syncMinimapWindow(); }
  }
  ...
  function openPersonModal(person) {
    ...
    if (person.birthPlace) meta.push(placeRow(I18n.t("timeline.born"), person.birthPlace));
    if (person.deathPlace) meta.push(placeRow(I18n.t("timeline.died"), person.deathPlace));
        ...
        el("div", null, [
          ...
          el("div", { class: "person-card__dates" },
            dateRange + (ageNow != null
              ? " · " + (FamilyStore.isAlive(person) ? I18n.t("timeline.ageNow", { n: ageNow }) : I18n.t("timeline.lived", { n: ageNow }))
              : ""))
        ])
      ]),
  ...
  // The rail's faceted Filter panel owns the predicate; the timeline DIMS
  // (never hides) non-matching rows. setFilter() re-reads window.Filter.
  function setFilter() {
    applyFilterClasses();
  }
  function applyFilterClasses() {
    ...
    canvas.querySelectorAll(".timeline-row").forEach((row) => {
      ...
      row.classList.toggle("is-dim", active && !!p && !match(p));
    });
  }
