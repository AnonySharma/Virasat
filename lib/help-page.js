...
(function (global) {
  ...
  function helpToast(msg) {
    ...
    if (!root) {
      // el() is declared `any` (UI.el fallback), so annotate the element type
      // to keep `root` non-null for the appendChild calls here and below.
      root = /** @type {HTMLElement} */ (el("div", { id: "help-toast-root", class: "help-toast-root", "aria-live": "polite" }));
  ...
  // Copy a tile's deep link to the clipboard, then toast. The copy itself
  // (async Clipboard API + textarea/execCommand fallback) is UI.copyText;
  // here we own the feedback: a toast on success, a manual-copy prompt on
  // failure. dom.js is always loaded alongside this page (help.html), so UI
  // is present.
  function copyTileLink(item) {
    ...
    UI.copyText(url).then((ok) => { if (ok) done(); else fail(); });
  }
  ...
  function wireFab(fab) {
    ...
    function onScroll() {
      if (ticking) return;
      ticking = true;
      // The fallback's `f` is contextually typed as FrameRequestCallback
      // (time => void), so it must be invoked with an arg; computeActive
      // ignores it. `0` stands in for the missing rAF timestamp.
      (global.requestAnimationFrame || ((f) => f(0)))(computeActive);
    }
